...
public class AdminStateStoreKeysTests
{
    ...
    [InlineData("tenant1", "")]
    [InlineData("tenant1", " ")]
    public void CommandStatusKey_ThrowsOnNullOrWhitespace(string? tenantId, string? correlationId)
    {
        Should.Throw<ArgumentException>(() => AdminStateStoreKeys.CommandStatusKey(tenantId!, correlationId!));
    }
    ...
    [InlineData("tenant1", "")]
    [InlineData("tenant1", " ")]
    public void CommandArchiveKey_ThrowsOnNullOrWhitespace(string? tenantId, string? correlationId)
    {
        Should.Throw<ArgumentException>(() => AdminStateStoreKeys.CommandArchiveKey(tenantId!, correlationId!));
    }
}
