...
public class KnownActorTypesTests {
    [Fact]
    public void Types_ContainsThreeKnownActorTypes() => KnownActorTypes.Types.Count.ShouldBe(3);

    [Theory]
    [InlineData("AggregateActor")]
    [InlineData("ETagActor")]
    [InlineData("ProjectionActor")]
    public void Types_ContainsExpectedActorType(string typeName) => KnownActorTypes.Types.ShouldContainKey(typeName);
    ...
    public void IsDynamicKeyFamily_ClassifiesCorrectly(string stateKey, bool expected) => KnownActorTypes.IsDynamicKeyFamily(stateKey).ShouldBe(expected);
    ...
    public void ResolveStateKey_ResolvesCorrectly(string stateKey, string actorId, string expected) => KnownActorTypes.ResolveStateKey(stateKey, actorId).ShouldBe(expected);

    [Theory]
    [InlineData("idempotency:{causationId}", "any-id")]
    [InlineData("drain:{correlationId}", "any-id")]
    [InlineData("{actorId}:events:{N}", "t:d:a")]
    public void ResolveStateKey_DynamicFamilies_ReturnsNull(string stateKey, string actorId) => KnownActorTypes.ResolveStateKey(stateKey, actorId).ShouldBeNull();

    [Fact]
    public void IsDynamicKeyFamily_NullInput_ThrowsArgumentNullException() => Should.Throw<ArgumentNullException>(() => KnownActorTypes.IsDynamicKeyFamily(null!));

    [Fact]
    public void ResolveStateKey_NullStateKey_ThrowsArgumentNullException() => Should.Throw<ArgumentNullException>(() => KnownActorTypes.ResolveStateKey(null!, "id"));

    [Fact]
    public void ResolveStateKey_NullActorId_ThrowsArgumentNullException() => Should.Throw<ArgumentNullException>(() => KnownActorTypes.ResolveStateKey("key", null!));
}
