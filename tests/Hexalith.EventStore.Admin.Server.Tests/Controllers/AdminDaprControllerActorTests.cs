...
public class AdminDaprControllerActorTests {
    ...
    public AdminDaprControllerActorTests() => _sut = new AdminDaprController(_service, NullLogger<AdminDaprController>.Instance) {
    ...
    public async Task GetActorRuntimeInfoAsync_DelegatesToService() {
        DaprActorRuntimeInfo expected = new(
            ...
            "http://localhost:3501");
        _ = _service.GetActorRuntimeInfoAsync(Arg.Any<CancellationToken>())
            .Returns(expected);
    ...
    public async Task GetActorInstanceStateAsync_DelegatesToService() {
        ...
        _ = _service.GetActorInstanceStateAsync("ETagActor", "Proj:T1", Arg.Any<CancellationToken>())
            .Returns(expected);
    ...
    public async Task GetActorInstanceStateAsync_Returns404_WhenActorTypeUnknown() {
        _ = _service.GetActorInstanceStateAsync("Unknown", "id", Arg.Any<CancellationToken>())
    ...
    public async Task GetActorInstanceStateAsync_Returns503_WhenServiceUnavailable() {
        _ = _service.GetActorInstanceStateAsync("ETagActor", "id", Arg.Any<CancellationToken>())
            .ThrowsAsync(new HttpRequestException("Unavailable"));
    ...
    public async Task GetActorInstanceStateAsync_ColonActorId_RoundTrips() {
        ...
        DaprActorInstanceState expected = new(
            ...
            0,
            DateTimeOffset.UtcNow);
        _ = _service.GetActorInstanceStateAsync("AggregateActor", actorId, Arg.Any<CancellationToken>())
            .Returns(expected);

        IActionResult result = await _sut.GetActorInstanceStateAsync("AggregateActor", actorId);

        OkObjectResult okResult = result.ShouldBeOfType<OkObjectResult>();
        var state = okResult.Value as DaprActorInstanceState;
