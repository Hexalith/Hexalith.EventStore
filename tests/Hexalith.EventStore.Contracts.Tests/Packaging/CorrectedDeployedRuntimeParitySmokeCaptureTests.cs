...
public sealed class CorrectedDeployedRuntimeParitySmokeCaptureTests
{
    ...
    /// <summary>
    /// Verifies a <c>TimeoutExpired</c> during <c>docker run</c> inspects and force-removes the
    /// uuid-named container. <c>container_created</c> is set only after a successful detach, so a
    /// hung run that did create the container previously leaked it. A non-zero <c>docker run</c>
    /// exit still must not remove a coincidentally same-named container; that contract stays in
    /// <see cref="FailedDockerRunNeverRemovesAContainerTheCaptureDidNotCreate"/>.
    /// </summary>
    [Fact]
    public void TimedOutDockerRunInspectsAndRemovesTheUuidNamedContainer()
    {
        RequireUnixHost();
        string root = FindRepositoryRoot();
        string temporary = Directory.CreateTempSubdirectory("eventstore-story315-smoke-run-timeout-").FullName;
        try
        {
            string fakeBin = Path.Combine(temporary, "bin");
            Directory.CreateDirectory(fakeBin);
            string dockerLog = Path.Combine(temporary, "docker-argv.log");
            string curlLog = Path.Combine(temporary, "curl-argv.log");
            WriteExecutable(Path.Combine(fakeBin, "docker"), DockerFake("run-timeout"));
            WriteExecutable(Path.Combine(fakeBin, "curl"), CurlFake("200 0"));
