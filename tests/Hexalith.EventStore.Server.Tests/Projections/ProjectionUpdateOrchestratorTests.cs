...
public class ProjectionUpdateOrchestratorTests {
    ...
    [Fact]
    public async Task CoordinatedEraseAsync_RecreatedSequenceOne_DeliversAndPersistsFreshCheckpoint() {
        const string readModelKey = "test-tenant:projection:test-domain:agg-001";
        object? persistedReadModel = new();
        ...
        DaprClient stateDaprClient = Substitute.For<DaprClient>();
        _ = stateDaprClient.GetStateAsync<ProjectionCheckpoint>(
                "statestore",
            ...
            .Returns(_ => persistedCheckpoint!);
        _ = stateDaprClient.GetStateAndETagAsync<ProjectionCheckpoint>(
                "statestore",
                ProjectionCheckpointTracker.GetStateKey(TestIdentity),
                consistencyMode: Arg.Any<ConsistencyMode?>(),
                metadata: Arg.Any<IReadOnlyDictionary<string, string>>(),
                cancellationToken: Arg.Any<CancellationToken>())
            .Returns(_ => ((ProjectionCheckpoint, string))(persistedCheckpoint!, persistedCheckpoint is null ? string.Empty : "checkpoint-etag"));
        _ = stateDaprClient.TrySaveStateAsync(
                "statestore",
                ProjectionCheckpointTracker.GetStateKey(TestIdentity),
                ...
                cancellationToken: Arg.Any<CancellationToken>())
            .Returns(call => {
                persistedCheckpoint = call.ArgAt<ProjectionCheckpoint>(2);
                return true;
            });
        _ = stateDaprClient.ExecuteStateTransactionAsync(
                "statestore",
            ...
            .Returns(call => {
                ...
                operations.ShouldContain(operation => operation.Key == readModelKey && operation.OperationType == StateOperationType.Delete);
                operations.ShouldContain(operation => operation.Key == ProjectionCheckpointTracker.GetStateKey(TestIdentity) && operation.OperationType == StateOperationType.Delete);
        ...
        var eraser = new ProjectionStateEraser(
            stateDaprClient,
            new DaprReadModelStore(stateDaprClient),
            tracker,
            Options.Create(new ProjectionOptions()));

        bool erased = await eraser.TryEraseAsync(
            TestIdentity,
            [new ReadModelEraseTarget(TestIdentity.TenantId, "statestore", readModelKey, "read-model-etag")],
            "checkpoint-etag");

        erased.ShouldBeTrue();
        persistedReadModel.ShouldBeNull();
        persistedCheckpoint.ShouldBeNull();

        var entries = new List<LogEntry>();
        var handler = new CountingProjectionResponseHandler();
        IHttpClientFactory httpClientFactory = CreateHttpClientFactory(handler);
        DaprClient invocationDaprClient = new DaprClientBuilder().Build();
        (ProjectionUpdateOrchestrator sut, IActorProxyFactory actorProxyFactory, _, IDomainServiceResolver resolver, _) = CreateSut(
            tracker,
            invocationDaprClient,
            httpClientFactory,
            new TestLogger<ProjectionUpdateOrchestrator>(entries));
        _ = resolver.ResolveAsync(Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<CancellationToken>())
            .Returns(new DomainServiceRegistration("counter-service", "project", "test-tenant", "test-domain", "v1"));
        IAggregateActor aggregateActor = Substitute.For<IAggregateActor>();
        _ = aggregateActor.GetEventsAsync(0).Returns([CreateTestEnvelope(1)]);
        _ = actorProxyFactory.CreateActorProxy<IAggregateActor>(Arg.Any<ActorId>(), "AggregateActor")
            .Returns(aggregateActor);
        ...
        entries.ShouldNotContain(entry => entry.EventId.Id == 1143);
        persistedCheckpoint.ShouldNotBeNull();
        persistedCheckpoint.LastDeliveredSequence.ShouldBe(1);
    }
