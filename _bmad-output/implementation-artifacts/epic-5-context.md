...
- Every Admin surface other than the explicit health probes must authenticate the caller, enforce current role and tenant scope, and reject before protected work. Denials must not disclose tenant or resource existence. The recent-commands count defaults to `1000` when omitted and supplied values are clamped to `1..1000` before service invocation. JSON bodies are bounded at the HTTP boundary: ordinary bodies default to 1 MiB, backup import is limited to 10 MiB, and oversized requests return bounded `413` Problem Details without partial work or payload echo.
- The EventStore gateway and Admin Server Host fail startup outside Development when trusted authority, issuer, audience, HTTPS metadata, signature/lifetime validation, or a nonempty explicitly configured asymmetric signing-algorithm allowlist is absent or unsafe. No implicit production algorithm default is permitted; Development and `AllowInsecureSymmetricKey` break-glass symmetric mode accept only HS256, preserve every other validation, emit a support-safe audit signal, and use exactly 60 seconds of clock skew. No committed configuration, including Development configuration, contains a forgeable administrator identity, signing key, username, password, credential, bearer token, secret, or decoded token data; Development and test credentials are injected at runtime.
