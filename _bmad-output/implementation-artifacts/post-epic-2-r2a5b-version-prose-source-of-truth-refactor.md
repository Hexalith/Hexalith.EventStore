...
- [x] Task 7: Conventional-commit-formatted PR / merge commit (AC: #12, #13)
    ...
    - [ ] 7.3 Squash-merge step (post-PR-approval): confirm subject preserves `chore:` prefix; verify semantic-release no-op (no NuGet bump, no `CHANGELOG.md` update, no GitHub Release on the 6 packages). Emergency-revert path per AC #12 if a bump fires.
