...
- [ ] [Review][Patch] Resolve the retained FrontComposer full Shell test blockers before Story 3.1 completion — restore/build at `afb39847` succeeds with 0 warnings/errors and 2,348/2,351 tests pass, but two governance tests require the forbidden/uninitialized nested Builds checkout and `SemanticReleaseAnalyzer_ConventionalCommitsMatrix_SelectsExpectedReleaseTypes` reports `fixBreakingHeader=false`. [references/Hexalith.FrontComposer/tests/Hexalith.FrontComposer.Shell.Tests/Governance/CiGovernanceTests.cs:589]
...
- All eight re-review patches were applied and their focused checks pass. The former FrontComposer NU1902 build blocker is resolved, but its complete Shell test project still has three independent failures, so Story 3.1 remains `in-progress` and does not claim `done`.
