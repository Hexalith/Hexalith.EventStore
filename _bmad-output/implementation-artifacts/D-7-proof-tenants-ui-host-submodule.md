...
### Review Findings — Re-review 2026-07-04 (round 2)

Second adversarial pass (Blind Hunter + Edge Case Hunter + Acceptance Auditor, all Opus, all findings independently re-verified against source). Scope: are the 7 committed patches correct, are the 6 deferrals safe, and did anything regress. **Auditor confirmed all 7 prior patches (C1/H1/H2/H3/M1/M2/M3/LOW-docs) are correctly and completely implemented in the committed code.** New triage: 1 decision-needed, 1 patch, 4 defer, 6 dismissed.

- [x] [Review][Patch] **MEDIUM — generator does not validate the `{tenantId}` route segment against the body under `RestTenantSource.System`** (decision-needed resolved 2026-07-05: patch the generator) [src/Hexalith.EventStore.RestApi.Generators/RestApiControllerEmitter.cs:1143 `IsTenantParameter` / :347 `ResolveTenant`]. `IsTenantParameter` excludes both `tenant` and `tenantId` from the route/body mismatch check, and `ResolveTenant` only compares route tenant values under `"Route"` source. So under `System` source the ~9 tenant-domain commands routed as `/api/tenants/{tenantId}/…` bind `[FromRoute] tenantId` but never compare it to `body.AggregateId`: `POST /api/tenants/acme/disable` with body `{"tenantId":"other-corp"}` dispatches against `other-corp` and returns 202, not 400. No privilege escalation (authz is enforced on the body tenant via the forwarded bearer), but AC2 ("route/body ID mismatches must remain 400 problem details") is a hard requirement the Auditor argues is unmet. This is the SAME issue the prior review deferred (its deferred item #2); the Auditor escalates it to an AC2 violation now that the CRITICAL is fixed. **Resolved 2026-07-05 (patch):** extend the generator to validate tenant-named route params against the body `AggregateId` when tenant source ≠ Route, returning 400 on mismatch — a reusable, platform-wide generator change.

- [x] [Review][Patch] **MEDIUM — sentinel-prefixed 400 validation errors are hijacked to 404/501 by the earlier fuzzy substring checks** [src/Hexalith.EventStore.Server/Pipeline/SubmitQueryHandler.cs:61]. In `CreateQueryFailureException` the fuzzy `IsNotFound` (`.Contains("not found")`, line 61) and `IsNotImplemented` (line 65) run BEFORE `IsSentinel(InvalidCursor)` (82) and `IsSentinel(InvalidEnvelope)` (94). The audit handler emits `"invalid-envelope: Invalid audit category: {categoryValue}"` with caller-supplied `categoryValue` (`TenantQueryHandlerBase.cs:320-325`); a `category` value containing "not found"/"not implemented" (e.g. `category=not found`) matches the fuzzy check first → HTTP 404/501 instead of the intended 400 `MalformedRequest`. Shared platform code, so every domain using the `"sentinel: detail"` convention is affected — directly defeating the patch's own comment ("without a fuzzy substring match that could misclassify"). New regression introduced by the H3/M1 patch. **Fix (unambiguous):** order the exact/prefix sentinel checks (InvalidCursor, InvalidEnvelope) before the fuzzy IsNotFound/IsNotImplemented checks.

- [x] [Review][Defer] **MEDIUM — `ListByCursorAsync` lets a 200-coded gateway exception escape into the Blazor circuit** [references/Hexalith.Tenants/src/Hexalith.Tenants.UI/Services/Gateways/TenantQueryGateway.cs:482-488] — deferred, pre-existing pattern carried through the migration. The tenant-list path catches only `IsUnauthorized` (401/403) and `IsUnavailableOrInvalid` (`StatusCode >= 400`); the generic `SubmitQueryAsync<T>` throws `EventStoreGatewayException` with `StatusCode = 200` on a null/JSON-`null`/shape-mismatch payload, which matches neither filter and escapes → circuit teardown, undermining AC7 fail-closed. Sibling methods use an unfiltered catch-all. Trivially patchable (add a catch-all like the siblings), but the filters were unchanged by D7 and it needs a malformed 200 (projection/contract bug) to trigger, so low likelihood.

- [x] [Review][Defer] **LOW — tenant-list path has no invalid-cursor recovery; a 400 invalid cursor surfaces as "gateway unavailable"** [references/Hexalith.Tenants/src/Hexalith.Tenants.UI/Services/Gateways/TenantQueryGateway.cs:485-487, 936] — deferred, pre-existing asymmetry. `IsUnavailableOrInvalid` treats every `>= 400` identically, so unlike `GetTenantAuditAsync` (which detects `IsInvalidAuditCursor` and resets to page 1) the list path neither resets nor reports an input error. Low likelihood — list cursors are server-issued protected cursors.

- [x] [Review][Defer] **LOW — generator emits an empty aggregate/entity id for an empty `Constant` query binding with no diagnostic** [src/Hexalith.EventStore.RestApi.Generators/RestApiControllerEmitter.cs:376] — deferred, variant of the already-deferred D5 generator-diagnostic gap. `[RestQueryBinding(Constant, "")]` yields `AggregateId = ""` at runtime (the attribute's own `ThrowIfNullOrWhiteSpace` never runs because Roslyn reads the metadata instead of instantiating). The related GetHashCode-NRE sub-claim was refuted (`GetString` never returns null). Fold into the existing generator-diagnostic (HESREST) hardening item.

- [x] [Review][Defer] **LOW — generated freshness headers never carry real values in production; UI stale badge unreachable** [src/Hexalith.EventStore.Server/Queries/QueryRouterResult.cs; src/Hexalith.EventStore/Controllers/QueriesController.cs] — deferred, duplicate of the prior review's deferred HIGH and the tracked D6 read-model-freshness handoff. AC4 ("when available") keeps D7 compliant, but AC7's stale/freshness indicator is a live regression until the platform `/query` hop stops dropping `IsStale`/`ProjectionVersion`. Platform work, not this story.

**Dismissed (round 2, not defects):** ListTenants `EntityId: null` "regression" (refuted — `ListTenantsQueryHandler` reads a single fixed-key global index `TenantIndexProjectionKey` and scopes RBAC by `envelope.UserId`, never by `EntityId`; test-proven and previously dismissed) · invalid-cursor reason-code change `invalid-cursor` → `InvalidPage` (UI keys on the *detail*, which still carries `invalid-cursor`; the reason code is a minor semantic choice on a brand-new external surface with no consumers) · query `PageSize` default "dropped to 0" (refuted — `ClampStandardPageSize`/`ClampAuditPageSize` map `pageSize <= 0` to 20/100) · identifier regex/256-char cap removed (aligns Tenants with the platform "any non-whitespace / ULID" ID rule; the old regex was stricter than the contract; no injection risk) · `InboundBearerForwardingHandler` appends Authorization (mirrors the accepted Sample.Api pattern; outgoing gateway request is fresh so append == set) · File-List incompleteness + nested-gitlink bumps (already dismissed with verification; documentation hygiene, no code defect). Scoping note: the provided root diff range `496395f2..HEAD` also contains one non-D7 hunk (`SubmitCommandHandler.cs` from unrelated commit `e0ad0fbe`) — excluded from this review.

**Round-2 patches applied & verified 2026-07-05 (uncommitted root-repo working-tree changes — need committing).** Both patches are root-platform-only (no Tenants submodule source change; A1's effect is regenerated by the analyzer at build time):
- E1 — reordered the exact/prefix `IsSentinel(InvalidCursor/InvalidEnvelope)` checks ahead of the fuzzy `IsNotFound`/`IsNotImplemented` checks in `SubmitQueryHandler.CreateQueryFailureException`. Verified: `dotnet build src/Hexalith.EventStore.Server -c Release` → 0/0; `dotnet test tests/Hexalith.EventStore.Server.Tests --filter SubmitQueryHandlerTests` → 20/20 (added a bare-sentinel→400 theory and two hijack regressions where a `"sentinel: detail"` message whose detail contains "not found"/"not implemented" now stays 400).
- A1 — `AppendCommandRouteMismatchChecks` now reconciles tenant-named route params against the command body under non-Route tenant source (`RequiresRouteTenant(options) ? GetNonTenantParameters : routeParameters`). Verified: `dotnet test tests/Hexalith.EventStore.RestApi.Generators.Tests` → 48/48 (added a System-source tenantId-mismatch test and a Route-source exclusion guard); regenerated `Hexalith.Tenants.Api` controllers now emit 9 `{tenantId}` route/body 400 guards (was 0) with the 17-action surface (11 commands + 6 queries) unchanged.
