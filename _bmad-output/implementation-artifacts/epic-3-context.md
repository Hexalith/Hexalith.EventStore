...
Maintainers can build, test, package, publish, and verify EventStore independently of local checkout state, reject invalid candidates without granting authority, and prove exact package and deployed-runtime lineage for a conforming release. Deterministic release gates remain separate from live infrastructure coverage. Invalid candidates remain preserved but non-authorizing; a separately authorized corrective release and independent parity verification can select an immutable deployed-runtime identity. Planning and story completion grant no external publication, deployment, or consumer-removal authority.
...
`v3.94.1` remains rejected, immutable, non-authorizing evidence and cannot select a deployed identity. A corrective release requires durable one-use pre-publication authority bound to the exact workflow run, version, source, destinations, inventory, platforms, and shared tooling. Positive deployed-runtime parity independently binds one canonical source/package/workflow/OCI/smoke lineage. Public API and wire compatibility remain separately blocked under G-COMPAT until their inventory, baselines, and enforcing gate pass. FR36 also requires separate `release-available`, `production-promoted`, and per-consumer removal authority for the same immutable subject and applicable deployment profile. A complete negative disposition or positive Story 3.15 evidence grants none of those later authorities.
...
An immutable subject advances through `built`, `evidence-candidate-published`, `evidence-validated`, `release-available`, and `production-promoted`. Positive Story 3.15 closure requires content-bound receipts from the authenticated EventStore owner, Release owner, and Test Architect and establishes only `evidence-validated`; release availability and each production-profile promotion need separate authenticated owner records and exact-subject validation. Consumer removal further requires a complete all-consumer manifest, applicable-mode parity, and an authenticated receipt binding the consumer repository, commit, and removal subject. Catalog and root-submodule maintenance refreshes preserve unrelated work, use exact reachable gitlinks, and leave prior release-bound evidence identities unchanged.
