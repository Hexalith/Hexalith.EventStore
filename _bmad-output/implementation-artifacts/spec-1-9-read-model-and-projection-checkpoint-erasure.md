...
> **AMENDED 2026-07-13 (human-directed resolution).** The `## Review Triage Log` at the end of this spec records eight human-directed resolutions of the 2026-07-13 senior-review `[Review][Decision]` items. Those resolutions amend this contract: the read-model canonical **write-path** is scoped out of 1.9 (forward seam; a no-canonical-writer slot returns `Unsupported`, never a no-op `Success`); the caller-supplied slot manifest is **authoritative** (no platform-side enumeration of "every owned slot"); the authenticated boundary is `[Authorize]` + explicit **GlobalAdministrator** (named Admin policy + tenant-authorization filter not required); a **domain-wide** active-rebuild gate is accepted; resume must bypass the active-rebuild gate and re-classify recorded-`Complete` targets; and the TOCTOU write-after-erase (AC8) plus the live AC7/AC12 evidence are accepted as disclosed limitations with tracked follow-ups. **Where this contract and that log differ, the log governs.**
...
- 2026-07-13 -- Human-directed resolution of the 2026-07-13 senior-review's 8 open `[Review][Decision]` items (see the Review Triage Log below). Disposition: fix the real-correctness defects (D1 no-op-Success guard, D2 resume wedge, D7 resume convergence) now; scope the read-model canonical write-path out to a follow-up; accept the caller-authoritative slot manifest, the global-admin-only boundary, and the domain-wide rebuild gate; waive the TOCTOU (AC8) and the live AC7/AC12 evidence with tracked follow-ups. The intent-contract and Tasks & Acceptance above are amended accordingly; where they differ, the Review Triage Log governs.

## Review Triage Log

### 2026-07-13 -- Senior-review decision resolution (human-directed, interactive)

The 2026-07-13 Senior Developer Review (see the story file's "Senior Developer Review (AI)" section) applied 5 code patches (F1--F5, committed) and left eight `[Review][Decision]` architect items open. Those eight are resolved below. **These resolutions amend the intent-contract and Tasks & Acceptance above -- where they differ, this log governs.** 1.9 remains in-progress until the D1/D2/D7 fixes land.

- **D1 -- Read-model canonical write-path: SCOPED OUT + no-op guard.** _(Amends Execution "The same address must be used for writes and erasure" and the acceptance line "every aggregate-owned read-model target ... are durably absent".)_ Populating the canonical `readmodel:{tenant}:{domain}:{projection}:{aggregate}:{slot}` key from a projection WRITE path is a forward platform seam and is **not in 1.9's scope** (the only shipped read-model producer, Tenants, is a read-only submodule whose adoption is Story 4.7). 1.9 ships the erase primitive plus the proven checkpoint fix. **Required fix (this story):** coordinated erase of a slot that has no registered canonical read-model writer MUST return `Unsupported` (reason `no-canonical-writer`) before mutation -- it must never report `Success` for a no-op erase of an unpopulated address. **Follow-up story:** canonical read-model write-path adoption + a real write→erase conformance proof.
- **D2 -- Erase/rebuild wedge: FIX now.** _(Amends the acceptance line "the actor resumes its persisted manifest/progress and converges".)_ A **resume** of an in-flight erase (matching `operationId`, actor phase already `Erasing`) MUST NOT be re-blocked by the active-rebuild gate. The active-rebuild refusal applies only to a **fresh `Begin`** (a new `operationId` admitted from `Idle`); a `Resume` bypasses the gate so an interrupted erase always converges. A fresh `Begin` that coincides with an active rebuild is still refused (`ActiveRebuild`) without leaving a persisted `Erasing` phase behind.
- **D3 -- TOCTOU write-after-erase: WAIVE + follow-up.** _(Amends the acceptance line "no read model or checkpoint can be recreated during completed erasure", AC8.)_ The approved "gate the write" design admits a delivery write point-in-time; a write admitted at `Idle` before erase begins can, in a cross-replica race, land after a completed erase and recreate state. Accepted as a **disclosed residual** for 1.9. **Follow-up hardening story:** an in-flight-write fence/lease in the lifecycle actor so a completed erase cannot be overtaken.
- **D4 -- Live AC7/AC12 evidence: WAIVE with recorded waiver.** _(Amends the IntegrationTests task and the Verification lane.)_ Cross-replica actor exclusion (AC7) and the stitched authenticated-endpoint→sequence-one E2E (AC12) are accepted as proven by unit tests (`ProjectionLifecycleActorTests`, `AdminProjectionRebuildControllerTests`, `ProjectionUpdateOrchestratorTests`) plus the persisted Redis recovery mechanism, in place of one live Tier-3 run (which needs the Aspire host, not runnable in-session). The Redis-level recovery mechanism IS proven live. **Follow-up:** a full Aspire authenticated cross-replica E2E when the host is available.
- **D5 -- Slot manifest authority: caller-authoritative.** _(Amends the acceptance line "every aggregate-owned read-model target".)_ The caller-supplied slot list is authoritative (consistent with the logical-IDs-only REST request, AC11). Coordinated erase covers exactly the caller-listed **registered aggregate-owned** slots; the platform does NOT enumerate every owned slot in 1.9. `IProjectionReadModelAddressFactory.CreateAggregateOwnedManifest` and `ProjectionEraseOutcomeKind.Denied` stay unused until a future story (if ever) adopts platform-side enumeration.
- **D6 -- Admin authorization surface: ACCEPT deviation.** _(Amends Execution "Require the Admin authorization policy, tenant authorization filter, forwarded application credentials, and an explicit EventStore GlobalAdministrator decision".)_ `[Authorize]` + an explicit `EnsureGlobalAdministrator()` denial-before-resolution is the accepted boundary. The named Admin policy + tenant-authorization filter are **not required**: global-admin-only is strictly narrower (ordinary Operator is denied) and the request's tenant scopes the erase. AD-10 (security fails closed) is satisfied.
- **D7 -- Resume convergence: FIX now.** _(Amends the acceptance line "the actor resumes its persisted manifest/progress and converges".)_ On resume, a recorded-`Complete` target MUST be **re-classified** (re-read / re-erase) rather than blindly trusted, so a target that reappeared after being recorded `Complete` cannot loop `Incomplete` forever against the completion-time `VerifyAllAbsent` re-read.
- **D8 -- Active-rebuild gate scope: ACCEPT stricter.** _(Amends Execution "Reject the whole request before mutation when the target projection has an active rebuild".)_ A **domain-wide** active-rebuild gate (`HasActiveOperatorRebuildForDomainAsync`) is accepted; it is fail-closed and stricter than AC9's per-projection wording (it may over-refuse erasure of an unrelated projection while another projection in the same domain rebuilds). No change required in 1.9; a projection-scoped gate is an optional future refinement.
