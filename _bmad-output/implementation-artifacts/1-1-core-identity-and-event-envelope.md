...
- [ ] Task 1: Audit EventMetadata fields against FR11 14-field specification (AC: #2)
  - [ ] 1.1 Add `MessageId` field (event message ID) — currently missing
  - [ ] 1.2 Add `AggregateType` field (aggregate type, distinct from Domain) — currently missing
  - [ ] 1.3 Add `GlobalPosition` field (cross-aggregate monotonic position) — currently missing
  - [ ] 1.4 Add `MetadataVersion` field (integer, starting at 1, FR65) — currently missing
  - [ ] 1.5 Keep `SerializationFormat` on EventMetadata (decided: it's a property of the serialized event). No action needed beyond verifying its position in the parameter order.
- [ ] Task 2: Update EventEnvelope to reflect EventMetadata changes (AC: #2, #4, #5)
  - [ ] 2.1 Update ToString() to include new fields while keeping payload redacted
  - [ ] 2.2 Verify defensive copy of Extensions dictionary
- [ ] Task 3: Verify AggregateIdentity completeness (AC: #1)
  - [ ] 3.1 Confirm parse/format methods work correctly
  - [ ] 3.2 Confirm all derived keys (ActorId, EventStreamKeyPrefix, MetadataKey, SnapshotKey, PubSubTopic) are correct
  - [ ] 3.3 Confirm validation (regex, length, colon prohibition)
- [ ] Task 4: Verify ULID integration (AC: #3)
  - [ ] 4.1 Confirm `Hexalith.Commons.UniqueIds` dependency is referenced
  - [ ] 4.2 Confirm all ULID fields are `string`-typed (not custom value objects)
- [ ] Task 5: Update Tier 1 tests (AC: #8, #9)
  - [ ] 5.1 Update EventMetadataTests for new fields (construction, validation). **Explicitly update `EventMetadata_HasExactly11Fields` test → change assertion to 15.**
  - [ ] 5.2 Add MetadataVersion validation test: values < 1 must throw `ArgumentOutOfRangeException`
  - [ ] 5.3 Add GlobalPosition validation test: values < 0 must throw `ArgumentOutOfRangeException`
  - [ ] 5.4 Update EventEnvelopeTests for new fields and updated ToString()
  - [ ] 5.5 Verify AggregateIdentityTests pass (existing)
  - [ ] 5.6 Verify IdentityParserTests pass (existing)
  - [ ] 5.7 Run full `dotnet test tests/Hexalith.EventStore.Contracts.Tests/`
- [ ] Task 6: Verify XML documentation on all public types (AC: #7)
- [ ] Task 7: Fix all compilation errors across the ENTIRE solution caused by EventMetadata changes (AC: #9)
  - [ ] 7.1 Grep `EventMetadata` across ALL projects (not just listed ones): `src/`, `tests/`, `samples/`
  - [ ] 7.2 Update all callers of EventMetadata constructor (new parameters added)
  - [ ] 7.3 Update any callers that destructure or pattern-match EventMetadata
  - [ ] 7.4 Verify `dotnet build Hexalith.EventStore.slnx --configuration Release` succeeds with zero warnings
  - [ ] 7.5 Run ALL Tier 1 test projects (Contracts.Tests, Client.Tests, Sample.Tests, Testing.Tests)
- [ ] Task 8: Update Server EventEnvelope to match new metadata fields (AC: #9)
  - [ ] 8.1 Add `MessageId`, `AggregateType`, `GlobalPosition`, `MetadataVersion` parameters to `src/Hexalith.EventStore.Server/Events/EventEnvelope.cs`
  - [ ] 8.2 Update ToString() to include new fields
  - [ ] 8.3 Update `PayloadProtectionTests.cs` ServerEventEnvelope construction (line 167)
  - [ ] 8.4 Grep `new ServerEventEnvelope(` and `Server.Events.EventEnvelope` across all test files
  - [ ] 8.5 Check for any **conversion logic** between `Contracts.EventEnvelope` and `Server.EventEnvelope` in `EventPersister.cs`, `EventStreamReader.cs`, or `AggregateActor.cs` — update field mappings
