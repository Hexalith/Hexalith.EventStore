...
- [x] [Review][Patch] `run_subprocess_bounded` and `sha256_git_file` duplicate drainers have diverged: combined output cap 131072 vs blob cap 8 MiB [tools/validate-oq8-platform-evidence.py:813-880,913-990] — applied 2026-09-06: one bounded Popen/selector path; `git_file` / `sha256_git_file` share the 8 MiB blob cap.
...
- [x] [Review][Patch] `scan_json_protected_content` has no depth/size bound and skips `PLACEHOLDER_RE` / `FORBIDDEN_CLAIM_RE` [tools/validate-oq8-platform-evidence.py:757-774] — applied 2026-09-06: depth/node bounds plus placeholder and forbidden-claim scans.
- [x] [Review][Patch] `validate_pyyaml_dependency` workflow wiring is an unscoped substring search [tools/validate-oq8-platform-evidence.py:3324-3334] — applied 2026-09-06: three bootstrap fragments must appear in order with only whitespace between them.
