...
2. **Tenant/key identity.** Admission is partitioned by managed tenant plus a protected digest of a caller-supplied opaque idempotency key, independently of command MessageId and aggregate identity. Same-tenant reuse against another operation, aggregate, target, delegated task scope, or behavior-affecting credential scope cannot bypass conflict detection. The same raw key in another tenant remains isolated.
...
- [ ] **Task 5 — Complete the durable state machine, fencing, and execution integration** (AC: 4, 5, 7)
  ...
  - [ ] Issue and persist monotonically increasing fencing tokens whenever execution authority is newly granted or safely reissued. Reject zero, stale, missing, or forged fences.
  ...
  - [ ] Decide and test whether projection activation outbox creation is protected work; preserve the existing invariant that aggregate commit cannot become visible without recoverable projection activation.
...
- [ ] **Task 8 — Prove production behavior and produce the platform evidence packet** (AC: 1-8)
  ...
  - [ ] Inspect persisted before/after state and assert zero aggregate/domain/provider/repository/projection/audit/scheduling execution for replay, conflict, expired, denied, unavailable, corrupt, unsafe legacy, pending, recoverable-without-checkpoint, and unreconciled unknown outcomes.
