...
- **Stage G DONE (Tasks 8+9)** — Full erase DI graph verified (`ProjectionEraseRegistrationTests`); released-`IReadModelStore` shape guard added; `DomainModuleAuthoringGuardrailTests` unmodified (28); AC→test coverage map recorded. Client 645 / Testing 150 / Server 2423 / DomainService 88 green. **Only Task 10 remains** (real-Redis Tier-3 persisted proof — Docker + Dapr placement/redis/scheduler confirmed reachable).
