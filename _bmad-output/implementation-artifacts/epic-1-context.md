...
- Projection handlers are keyed by `(Domain, ProjectionType)`. Partial fan-out success preserves independently durable sibling work, but failed or indeterminate routes do not advance. Full-replay and incremental rebuild semantics are explicit, work is staged, and promotion occurs only after every required projection completes durably.
