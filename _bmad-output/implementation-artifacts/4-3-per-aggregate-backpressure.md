...
**Caveat (AC #5d):** **Resolved at <pending merge SHA> by R4-A2b** — `IBackpressureTracker` and `InMemoryBackpressureTracker` removed as dead code; actor-level Step 2b is the sole enforcement path. The `SubmitCommandHandler` constructor overloads accepting `IBackpressureTracker` are also removed.
