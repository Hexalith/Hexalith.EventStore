...
- [ ] [Review][Decision] Referenced-contract discovery scope is unbounded — `RestApiMessageParser.ParseReferenced` collects every `[RestRoute]` command/query from every referenced assembly that references the platform Contracts assembly, with no domain/assembly filter, so a host opting into one domain (`[assembly: RestApi("api/{tenant}/counter", "counter", …)]`) can emit unrelated referenced contracts under the `counter` prefix. Related facets: discovery runs off `CompilationProvider` (re-walks the reference graph on every edit — incrementality/perf) and the emitted descriptors are unsorted (generated-output determinism). Owner choice needed: (a) filter by the opted-in domain/tag, (b) require an explicit assembly opt-in list, or (c) declare multi-contract aggregation intentional and document/test it. [src/Hexalith.EventStore.RestApi.Generators/RestApiMessageParser.cs:52]
- [ ] [Review][Decision] Domain module `Hexalith.EventStore.Sample` is not SDK-only — unreconciled governance conflict — `Hexalith.EventStore.Sample.csproj:17` references `Sample.Contracts` (functionally required at HEAD: the aggregate `Handle` overloads consume the command contracts that now live there). This contradicts the D5 Project Structure Notes + CLAUDE.md "domain references only `DomainService`" rule and the completion note claiming "the domain remains SDK-only" (false at HEAD), yet matches proposal CP-2. Owner must rule which governs and reconcile the domain-authoring rule (permit a domain-owned contracts library or not); the false completion note must be corrected either way. [samples/Hexalith.EventStore.Sample/Hexalith.EventStore.Sample.csproj:17]

**Patch**

- [ ] [Review][Patch] `sample-api` DAPR access-control grants `/**` for GET+POST — the gateway client only ever `POST`s to the query path; GET is unused and `/**` exposes admin/command/health/actor endpoints. Scope to the query path + POST (add the command path in D6). Defense-in-depth only (eventstore JWT/RBAC stays authoritative). Confirms the earlier 2026-07-03 open patch, unaddressed. [src/Hexalith.EventStore.AppHost/DaprComponents/accesscontrol.yaml:60]
- [ ] [Review][Patch] `Sample.Api` inbound JWT trust-widening in Authority mode — when `Authority` is set, `ValidIssuer` is never overridden so `hexalith-dev` stays permanently trusted, and `RequireHttpsMetadata` defaults `true` (can block http-dev Keycloak discovery). Latent under `EnableKeycloak=false`. Override/clear `ValidIssuer` when Authority is configured and wire issuer/https-metadata via `WithEventStoreClientCredentials`. Confirms the earlier 2026-07-03 open patch, unaddressed. [samples/Hexalith.EventStore.Sample.Api/Program.cs:31]
- [ ] [Review][Patch] `Sample.Api` empty/whitespace `SigningKey` bypasses its guard — line 41 `?? throw` fires only on `null`; an empty or whitespace `SigningKey` flows into `new SymmetricSecurityKey(...)` → opaque startup `ArgumentException` (the sibling Authority branch correctly uses `IsNullOrWhiteSpace`). Use `IsNullOrWhiteSpace` + a ≥32-byte length check. New this review. [samples/Hexalith.EventStore.Sample.Api/Program.cs:41]
- [ ] [Review][Patch] D5 ACs/tasks/notes still describe the abandoned "generate into BlazorUI" design — AC2/AC3/AC4/AC5/AC9, Task 3, and Project Structure Notes reference emitting into `Sample.BlazorUI` and editing the domain query file, contradicting the shipped external `Sample.Api` + `Sample.Contracts` outcome. Rewrite to the post-correct-course scope. Confirms the earlier 2026-07-03 open patch, unaddressed. [_bmad-output/implementation-artifacts/D-5-proof-sample-blazorui-queries.md:54]
...
- [ ] [Review][Decision] Domain module `Hexalith.EventStore.Sample` is not SDK-only + false completion note — `Hexalith.EventStore.Sample.csproj:17` references `Sample.Contracts` (functionally required at HEAD: the aggregate `Handle` overloads consume the command contracts that now live there), contradicting the CLAUDE.md/project-context "domain references only `DomainService`" rule and story Project Structure Notes:376. Completion note :510 asserts "the domain remains SDK-only" — **false at HEAD**. Owner must rule whether a domain-owned contracts library is permitted (reconciling the domain-authoring rule) and the false completion note must be corrected either way. [samples/Hexalith.EventStore.Sample/Hexalith.EventStore.Sample.csproj:17] — STILL OPEN (re-confirmed).
...
- [ ] [Review][Patch→Decision] (from Decision 1, option a) Filter referenced-contract emission by the host's opted-in `[assembly: RestApi]` domain — **HELD 2026-07-03: this is not a mechanical patch.** Faithful option (a) requires the generator to know each contract's domain, but `RestApiMessageDescriptor` carries no domain, `RestApiOptions.Tag` is the OpenAPI tag (not a per-contract domain), and a contract's `Domain` is a static expression-bodied property (`public static string Domain => "counter";`) the generator **cannot read from referenced-assembly metadata** (no syntax; not a compile-time constant). A faithful fix needs a contract-seam change (e.g. a domain argument on `[RestRoute]` or a `[RestDomain]` attribute readable from metadata), touching D1/D6/D7. Owner must choose the discovery mechanism before implementation — or adopt Decision-1 option (b) explicit assembly opt-in, or defer to a dedicated generator-hardening story. [src/Hexalith.EventStore.RestApi.Generators/RestApiMessageParser.cs:52]
