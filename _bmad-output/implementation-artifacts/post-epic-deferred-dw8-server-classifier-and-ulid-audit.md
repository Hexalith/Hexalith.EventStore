...
- [x] [Review][Decision→Patch] Plain `DaprException` always classifies as `DaprUnavailable` — resolved 2026-05-09: inverted `ContainsDaprUnavailableSignal` default to return `false` when no inner availability signal is present, and added a trailing `DaprException => DrainReasonCodes.StateStoreFailure` arm. Plain `DaprException` (e.g. permanent state-store/component config error) now classifies as `drain_state_store_failure`, matching the doc semantics. [`src/Hexalith.EventStore.Server/Actors/AggregateActor.cs:864-895`]
- [x] [Review][Decision→Patch] `EventDeserializationException` mapped to `drain_state_store_failure` — resolved 2026-05-09: kept the mapping (deserialization happens at the controlled state-store boundary) and updated `docs/operations/drain-failure-reason-codes.md` row for `drain_state_store_failure` to call out payload-schema corruption alongside availability checks.
- [x] [Review][Decision→Patch] `IsUnavailableStatusCode` / HTTP set is broad — resolved 2026-05-09: dropped `StatusCode.Aborted` from the unavailable set so concurrency/transactional aborts no longer collapse into `drain_dapr_unavailable`; updated the doc row to acknowledge throttling/timeouts and to explicitly exclude `Aborted`. [`src/Hexalith.EventStore.Server/Actors/AggregateActor.cs:897-900`]
- [x] [Review][Decision→Patch] Publish wrapper catches `Exception` — resolved 2026-05-09: introduced `IsDrainPublishBoundaryFailure(...)` filter (`DaprException`, `RpcException`, `HttpRequestException`, `IOException`, `TimeoutException`); the publish try/catch now uses `catch when (IsDrainPublishBoundaryFailure(ex))`. Programmer-error exceptions (NRE/ArgumentException/etc.) propagate raw to the outer catch and classify as `unknown` so real defects surface. [`src/Hexalith.EventStore.Server/Actors/AggregateActor.cs:736-748, 884-890`]
- [x] [Review][Patch] Audit allowlist for `/Middleware/CorrelationIdMiddleware.cs` is suffix-only — resolved 2026-05-09: replaced the suffix match with an explicit two-path allowlist (`AllowedMiddlewarePaths`) anchored to `src/Hexalith.EventStore/Middleware/CorrelationIdMiddleware.cs` and `src/Hexalith.EventStore.Admin.Server.Host/Middleware/CorrelationIdMiddleware.cs`. [`tests/Hexalith.EventStore.Server.Tests/Governance/ProtectedIdentifierGuidParserAuditTests.cs:14-17, 109-114`]
- [x] [Review][Patch] Audit test-file allowlist is over-permissive — resolved 2026-05-09: replaced the 7-line context window with a same-line assertion rule (`SameLineAssertion`) that only allowlists test hits when the line itself carries a Shouldly fluent assertion (`ShouldBe...`/`ShouldThrow`/`ShouldNotThrow`), an `xUnit Assert.<Capital>` call, or the `bool isGuid = Guid.TryParse(...)` assertion-evidence shape. The optional `?` suffix in the previous Should-regex was removed.
- [x] [Review][Patch] `FindRepositoryRoot` brittleness — resolved 2026-05-09: renamed to `TryFindRepositoryRoot` with three fallbacks: `HEXALITH_REPOROOT` env var, walk from the test source file via `[CallerFilePath]`, walk from `AppContext.BaseDirectory`. When all three fail the test calls `Assert.Skip(...)` instead of throwing `DirectoryNotFoundException`, so missing source-tree access is reported as Skip rather than build failure.
- [x] [Review][Patch] `EventDrainRecoveryTests.CaptureDrainActivityAsync` lacks correlationId filtering — resolved 2026-05-09: added an overload `CaptureDrainActivityAsync(string correlationId, Func<Task> action)` that filters captured activities by `EventStoreActivitySource.TagCorrelationId`. The default-arg overload (`"corr-drain"`) preserves all existing call sites. Cross-test capture between EventDrainRecoveryTests and Dw8DrainReasonClassifierTests is now impossible. [`tests/Hexalith.EventStore.Server.Tests/Actors/EventDrainRecoveryTests.cs:165-194`]
- [x] [Review][Patch] `LoadPersistedEventsRangeAsync` wrapper missing explicit `catch (OperationCanceledException) { throw; }` — resolved 2026-05-09: added the cancellation-pass-through guard immediately before the filtered `catch when (IsDrainStateStoreBoundaryFailure)`, matching the sibling state-store-load and publish blocks. [`src/Hexalith.EventStore.Server/Actors/AggregateActor.cs:729-735`]
- [x] [Review][Defer] Outer catch's `ex.Message` is preserved into `LastFailureReason` on the persisted record [`src/Hexalith.EventStore.Server/Actors/AggregateActor.cs:841`] — pre-existing in DW1, surface widened by DW8 (raw RpcException/HttpRequestException text now flows through). Bound or sanitize to a known schema in a follow-up.
- [x] [Review][Defer] `DrainPublishException`/`DrainStateStoreException` derive from `InvalidOperationException` [`src/Hexalith.EventStore.Server/Actors/`] — an upstream `catch (InvalidOperationException)` could swallow these and lose the inner DAPR/HTTP exception. None detected today; consider basing on `Exception` for future-proofing.
- [x] [Review][Defer] Audit guard scans `*.cs` only [`tests/Hexalith.EventStore.Server.Tests/Governance/ProtectedIdentifierGuidParserAuditTests.cs:711`] — `.razor` files (and Razor-generated `.razor.g.cs` excluded as `.g.cs`) are skipped. No current Razor protected-field validators detected.
- [x] [Review][Defer] `sprint-status.yaml` "Previous last_updated detail preserved" comment chain has no ceiling [`_bmad-output/implementation-artifacts/sprint-status.yaml`] — governance smell, grows unboundedly each story.
- [x] [Review][Defer] Outer-catch comment "Drain infrastructure failure: increment retry, save, reminder continues" [`src/Hexalith.EventStore.Server/Actors/AggregateActor.cs:836`] — predates DW8 wrappers and now also applies to programmer-error wraps via `DrainPublishException`. Refresh the comment.
...
| --- | ---: | --- | --- |
| 2026-05-09 | 1.1 | Code review (bmad-code-review) applied 9 patches across `AggregateActor.cs`, `ProtectedIdentifierGuidParserAuditTests.cs`, `EventDrainRecoveryTests.cs`, and `drain-failure-reason-codes.md`: (1) plain `DaprException` now maps to `drain_state_store_failure`; (2) `EventDeserializationException` doc clarified for payload corruption; (3) `Aborted` removed from the unavailable status set; (4) publish-catch narrowed to infra exception types only; (5) audit middleware allowlist anchored to two exact paths; (6) audit test-file allowlist tightened to same-line assertions + `bool isGuid =` shape; (7) `FindRepositoryRoot` made resilient with `HEXALITH_REPOROOT` env, `[CallerFilePath]` walk, and `Assert.Skip` fallback; (8) `EventDrainRecoveryTests.CaptureDrainActivityAsync` filters by correlationId; (9) `LoadPersistedEventsRangeAsync` cancellation guard added. 5 deferred items recorded in `deferred-work.md`; 12 dismissed. Validation: focused DW8/guard/EventDrainRecovery 31/31 passed; full Server.Tests Release 1845 passed / 25 skipped. Story moved review → done. | Codex (code review) |
