...
- [Rejected][low] `BearerTokenRegex` matches ordinary prose beginning with `Bearer` and omits Basic/PEM/`Authorization:` — everyday Admin results do not carry those shapes; tightening the detector adds branches for a phrase that is not in the current models.
...
### Review Findings — Admin UI slice (2026-09-22, bmad-code-review)

_Story 5.4 Admin UI: `src/Hexalith.EventStore.Admin.UI` and `tests/Hexalith.EventStore.Admin.UI.Tests` versus `da5accfc` (25 files, +3442/−163). `AdminApiAccessTokenProvider` and its role tests are excluded as Story 5.3 work. All four layers reported. The dispatcher finding was reproduced by a scratch bUnit probe in a throwaway worktree: `waitForRender` was completed from a pool thread and `Snapshots.OnCreateSnapshotConfirm` threw `InvalidOperationException: The current thread is not associated with the Dispatcher` at `Snapshots.razor:885`._

- [x] [Review][Decision] An Operator's consistency-check confirmation does not name the tenant that will run — resolved 2026-09-22: option (a), mirror MCP and require a tenant for non-Admin users. Now tracked as the patch below.
- [x] [Review][Patch] Require an explicit tenant for non-Admin consistency triggers — With the field empty, `FormatTenantScope` shows "Authorized tenant scope" while `TryResolveTenantScopeForBody` runs the caller's first tenant claim. Mirror MCP `consistency-trigger`: when `_canUseOpaqueChecks` is false, make Tenant ID required, and fail an empty value as local validation (no request, exact initiator focus) so the facts always name the tenant that will run. Update the field label and cover this with tests. [src/Hexalith.EventStore.Admin.UI/Pages/Consistency.razor:1054]
- [x] [Review][Patch] Denial, validation, and stale-selection paths call `StateHasChanged()` off the Blazor dispatcher and kill the circuit — Handlers await `HideAsync`/`FocusService.RestoreAsync(...).ConfigureAwait(false)` (real async JS interop under Interactive Server), then call `StateHasChanged()` directly in `finally` or inline. Replace these calls with `await InvokeAsync(StateHasChanged)`, as Tenants/Consistency already do (DW12). Sites: Backups `finally` 977/1143/1373; Snapshots 629/714/793/885; DeadLetters 936/1064. Add a regression test whose `hexalithAdmin.waitForRender` completes asynchronously from a pool thread. [src/Hexalith.EventStore.Admin.UI/Pages/Snapshots.razor:885]
- [x] [Review][Patch] Focus restoration is unbounded and unguarded — `waitForRender` resolves only on `requestAnimationFrame`, which browsers pause in hidden tabs. The await stalls until the JS interop timeout, and the `TaskCanceledException`/`JSException` escapes every close/denial handler. Make the wait bounded (e.g. rAF raced with `setTimeout`) and make `RestoreAsync` best-effort. [src/Hexalith.EventStore.Admin.UI/Services/InitiatorFocusService.cs:22]
- [x] [Review][Patch] Projection reset/replay generic-failure and `Success != true` branches restore focus while `_isOperating` is still true — The helper re-renders the initiating button as `Disabled`, so the browser refuses focus (AC5). Clear `_isOperating`/`_operatingAction` before `Close…AndRestoreFocusAsync`, as the denial branches already do. [src/Hexalith.EventStore.Admin.UI/Components/ProjectionDetailPanel.razor:531]
- [x] [Review][Patch] Confirmation facts lag the value that will be submitted — The target inputs bind on `change`, not `Immediate` (Backups create tenant, Snapshots create-policy/create-snapshot fields, Consistency tenant/domain, Tenants add-user). When an operator types and clicks Confirm, the submitted target was never shown in `ConfirmationFacts` (AC4). Tenants create already sets `Immediate="true"`; apply that to the other inputs. [src/Hexalith.EventStore.Admin.UI/Pages/Backups.razor:240]
- [x] [Review][Patch] Projection reset/replay rejected-result and service-unavailable paths are untested — No test gives the mock a `null`/`Success=false` result or a `ServiceUnavailableException`. Changing `!= true` to `== false` would show "accepted" wording for a null result, and dropping `_isOperating = false` would lock the controls; both mutations stay green. Add theory cases asserting the error toast, no "accepted" copy, the closed dialog, exact focus, and re-enabled buttons. [tests/Hexalith.EventStore.Admin.UI.Tests/Components/ProjectionDetailPanelTests.cs]
- [x] [Review][Patch] Import schema is not pinned to the real exporter — The importer now requires camelCase `tenantId`/`domain`/`aggregateId` and an `events` array. No server test parses `BuildExportContent` output for those keys, so the exporter can drift and every export stops importing with no test failing. Add a `DaprBackupCommandServiceTests` round-trip assertion for both formats. [src/Hexalith.EventStore.Admin.Server/Services/DaprBackupCommandService.cs:533]
- [x] [Review][Patch] Dead-letter write 503 coverage was deleted — The `ServiceUnavailable, "temporarily unavailable"` row was removed from `RetryDeadLettersAsync_GenericFailure_PreservesStatusCategory` and not replaced; restore it. [tests/Hexalith.EventStore.Admin.UI.Tests/Services/AdminDeadLetterApiClientErrorTests.cs:46]
- [x] [Review][Patch] Projection facts and guard use `_detail.Name` while the call submits `ProjectionName` — The dialog and `IsExactAndSupportSafe` verify one value and execute another. Derive both from the value that is submitted. [src/Hexalith.EventStore.Admin.UI/Components/ProjectionDetailPanel.razor:484]
- [x] [Review][Patch] `throw MapWriteException(ex)` rethrows the same instance for non-401/403 statuses and resets its stack trace — Use an exception filter so other statuses propagate untouched. [src/Hexalith.EventStore.Admin.UI/Services/AdminDeadLetterApiClient.cs:116]
- [x] [Review][Patch] Dead `if (denied)` inside the dead-letter full-success branch — `denied` is set only alongside `failCount += …`, so this branch can never run; delete it. [src/Hexalith.EventStore.Admin.UI/Pages/DeadLetters.razor:1015]
- [x] [Review][Defer] No blocking browser test proves `hexalithAdmin.waitForRender` exists or that dialogs return `document.activeElement` to the initiator [src/Hexalith.EventStore.Admin.UI/wwwroot/js/interop.js:126] — deferred: bUnit Loose JS mode accepts any identifier; the E2E dialog tests are skipped/advisory; same blocker as the frontmatter `deferred` entry.
- [x] [Review][Defer] Unchanged toast paths still show raw `result?.Message` / `ex.Message` and Snapshots "created" completion wording [src/Hexalith.EventStore.Admin.UI/Pages/Tenants.razor:989] — deferred: pre-existing lines identical at `da5accfc` (Tenants 896–1079, Snapshots 549–756); not touched by this story.

#### Rejected

- [Rejected][false] Local validation closes the dialog and discards entered values — the spec requires exact initiator focus after validation failure, which a mounted modal's focus trap prevents (prior BH-12 fix); the message is still shown as a toast.
- [Rejected][low] Success paths never restore initiator focus — AC5 names cancellation, validation, and denial only; a fallback target for removed controls adds branches.
- [Rejected][low] The empty-selection dead-letter test asserts focus on a button that is no longer rendered — only reachable if a filter debounce clears the selection while the modal is open; a heading fallback adds a branch.
- [Rejected][low] `catch (Exception ex) { _ = ex; }` blocks do not log — the user copy is intentionally fixed; adding logging means injecting a logger into five components.
- [Rejected][low] Toasts/validation copy remain inline English, there is no resx-key completeness test, and bodies use raw `<p style>` — pre-existing repo idiom (38 raw `<p style>` bodies versus 7 `FluentText` uses in Admin.UI); the spec requires the three facts to be resource-backed, and they are.
- [Rejected][false] The facts panel omits role, interval, positions, and tenant names — the spec requires target, impact, and permission, which are shown; the other values sit in the same dialog, and bulk tenant IDs come from server-listed rows.
- [Rejected][false] Tenants focus-id encoding can collide — only one tenant is expanded at a time, and lifecycle (`disable`/`enable`) and user (`add`/`remove`/`change-role`) action prefixes are disjoint, so no two rendered ids collide.
- [Rejected][low] New tests reach private members through reflection — test style; it does not hide a demonstrated defect.
- [Rejected][low] `ConfirmationFacts` is not wired through `aria-describedby` — the facts are readable dialog content; wiring twelve dialogs is added surface for a cosmetic gain.
- [Rejected][low] `AddHttpClient` registration and `aspire run --apphost` text are unrelated to 5.4 — harmless, and `--apphost` is the current CLI flag.
- [Rejected][low] A post-acceptance toast `InvalidOperationException` now escapes `OnTriggerConfirm` — deliberate, pinned by `Trigger_PostAcceptanceUiInvalidOperationIsNotRelabeledAsInvalidRequest`; requires a toast-service fault.
- [Rejected][low] `ConfirmationFacts` truncation can split a surrogate pair — same boundary already rejected for the MCP `BoundText`.
- [Rejected][low] The import composite-length error blames identifier length — requires three near-limit identifiers.
- [Rejected][low] Duplicate `MessageId` across tenants blocks bulk actions — message ids are ULIDs.
- [Rejected][low] A typed tenant literally named "All tenants" looks like the all-tenant scope — contrived input.
- [Rejected][false] Older PascalCase export files are now rejected — the producer never emitted PascalCase (prior BH-09), so no real export was ever accepted by the old parser.
- [Rejected][false] Write-path 503 is not mapped to `ServiceUnavailableException` — write failures surface through bulk failure items, not the query exception mapping; this is a behavior choice, not a defect.
- [Rejected][low] `_replayValidationError` is dead state after the dialog closes — cleanup only.
