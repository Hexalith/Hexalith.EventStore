...
Code review of commit `4a56c7c` (post-epic-11-r11a3-apphost-projection-proof). Sources: blind hunter, edge case hunter, acceptance auditor.

- [x] [Review][Decision] aspire-run*.log files referenced but missing from commit — Story File List and Debug Log References (lines 99, 256-257) advertise `aspire-run.log`, `aspire-run-nokeycloak.log`, `aspire-run-nokeycloak-sampletenant.log` as canonical AppHost run-log evidence. None are in the commit; `.gitignore` matches `*.log`. Choose: (a) rename to `.txt` and commit them, or (b) remove the references from File List + Debug Log References. Sources: blind+edge+auditor.
- [x] [Review][Decision] AC #6 — projection actor write log was not emitted; substituted with surrogate evidence — `EventReplayProjectionActor.UpdateProjectionAsync` and `ProjectionUpdateOrchestrator.ProjectionStateUpdated` log at `LogLevel.Debug`; AppHost active log level is `Information`. Completion Notes admit this and substitute "post-command ProjectionActor query trace + count delta + ETag regeneration" as inferential proof. The spec wording requires direct write-side evidence. Choose: (a) rerun proof with `Logging:LogLevel:Hexalith.EventStore.Server=Debug` to capture the actor-write line, (b) accept the caveated PARTIAL with a deferred-work note routed to R11-A1/R11-A2, or (c) raise the log to `Information` in source and rerun. Sources: blind+edge+auditor.
- [x] [Review][Decision] AC #9 — SignalR delivery not directly logged; refresh-after-signal evidenced behaviorally — `EventStoreSignalRClient.OnProjectionChanged`, `SilentReloadPattern.OnProjectionChanged`, and `SignalRProjectionChangedBroadcaster.BroadcastSent` all emit at `Debug`. The artifact captures negotiate/connect/JoinGroup but no moment-of-delivery log. The page has no polling timer, so the observed `Refresh count: 1 → 2` after the direct API command is behavioral refresh-after-signal evidence the spec permits. Choose: (a) rerun with `SignalRProjectionChangedBroadcaster` at `Information` to capture an explicit broadcast log, (b) accept the behavioral refresh-after-signal evidence as documented PARTIAL with caveat, or (c) defer hardening to a follow-up. Sources: edge+auditor.
- [x] [Review][Patch] Query response correlationIds break the proof identity chain [src/Hexalith.EventStore/Controllers/QueriesController.cs:42-43] — `final-baseline-query-response.txt` and `final-after-query-response-5s.txt` carry plain GUIDs (`8e1f8968-…`, `04be0a50-…`) unrelated to the proof prefix `r11a3-…`. The query controller mints a fresh GUID per request; the proof is stitched via wall-clock and `trace-ids.txt`, not structural ID. Patch the README AC #2/#8 entries to explicitly call out trace-id (not correlationId) as the link for the query leg. Sources: edge.
- [x] [Review][Patch] Negative-log-search filter terms too narrow [_bmad-output/test-artifacts/post-epic-11-r11a3-apphost-projection-proof/negative-log-search.txt] — Misses real exception/log keys present in source: `InvalidProjectionResponse` (ProjectionUpdateOrchestrator.cs:208 — exactly the AC #11 "malformed projection response" failure mode), `NoDomainServiceRegistered`, `CheckpointSaveExhausted`, `CheckpointSaveFailed`, `ProjectionChangeNotificationFailed`, plus generic terms (`error`, `Error`, `exception`, `Exception`, `fail`, `warn`). Rerun the bounded search with the broader term set and update the file. Sources: blind+edge.
- [x] [Review][Patch] UI ETag display truncated identically across all snapshots [ui-silent-reload-*-snapshot.md (×4)] — Every snapshot shows `paragraph: "ETag: Y291bnRl..."` so the browser artifacts cannot prove ETag change. Add a one-line note to the README AC #7/#10 rows clarifying that ETag-change evidence is the API response headers, not the browser display. Sources: blind.
- [x] [Review][Patch] AC #2 evidence-matrix row imprecise [README.md AC matrix row 2] — Row reads as if `final-proof-identity.json`, `final-command-payload.json`, and `final-query-payload.json` together carry messageId, correlationId, and JWT claims. Each field actually lives in exactly one file. Fix the row to map each artifact to the field it actually carries. Sources: blind.
- [x] [Review][Patch] test-results.txt duration "84 ms" is mislabeled or transcribed — wall time for `dotnet test` over 63 xUnit tests is normally seconds, not 84 ms; full console output is not captured. Either capture the raw `dotnet test` output (or paste the trx summary) or label the field as "test execution time (excluding host startup)". Sources: blind.
- [x] [Review][Patch] README runbook missing AppHost-vs-test process-lock prerequisite [_bmad-output/test-artifacts/post-epic-11-r11a3-apphost-projection-proof/README.md] — `test-results.txt` records first attempt blocked by AppHost-locked DLLs in `samples/Hexalith.EventStore.Sample/bin/Debug/net10.0`. Add a Prerequisites step: "Stop the AppHost before running `dotnet test`" so the runbook is repeatable. Sources: edge.
- [x] [Review][Patch] README runbook missing Dapr CLI / placement / scheduler prerequisite [_bmad-output/test-artifacts/post-epic-11-r11a3-apphost-projection-proof/README.md] — `resources-before.txt` admits `placement.exe`/`scheduler.exe` were absent on Windows and Aspire-managed sidecars were used instead. CLAUDE.md mandates `dapr init` for Tier 2/3 work. Add a Prerequisites step covering Docker + `dapr init` (or `dapr init --slim` on Windows). Sources: edge.
- [x] [Review][Defer] AC #11: 5-second bounded wait not justified against actor turn model — deferred, owned by R11-A1 (checkpoint tracking) and R11-A2 (polling mode). The current run passed (delta=1) but the proof window does not handle the case where projection delivery legitimately exceeds 5s; the safe approach is poll-until-converged with timeout. Owners of the polling/checkpoint stories carry this hardening.
...
| 2026-05-02 | 1.1 | Code review applied: README AC evidence matrix rewritten with file/log-id precision and AC #6/#9 caveats; Prerequisites section added (Docker, `dapr init`, AppHost-stop-before-tests); negative-log-search.txt term list expanded with named log keys + generic terms; test-results.txt clarifies the 84 ms metric as xUnit assembly inner-time; aspire-run*.log references removed from File List and Debug Log References (files were ignored by `*.log` and contained only CLI startup banners). AC #11 5s-wait hardening deferred to R11-A1/R11-A2 in deferred-work.md. AC #6 and AC #9 accepted as caveated PARTIAL with deferred residual hardening. | Claude Opus 4.7 |
