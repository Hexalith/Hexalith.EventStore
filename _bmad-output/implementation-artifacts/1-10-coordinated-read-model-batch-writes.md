...
_Adversarial code review 2026-07-13 (`bmad-code-review`, 4 blind layers at Opus 4.8: blind-hunter, edge-case-hunter, verification-gap, acceptance-auditor). Reviewed diff = the isolated Story 1.10 code contribution `428ef1a1^..acc45f14` (40 files, +3343/-49), scoped away from the interleaved Story 1.9 erasure commits. 16 findings, 0 dismissed. Strong cross-layer convergence on the throw-on-race, the envelope substring scan, the hardcoded JSON options, and the unexecuted live lane._

**Decisions resolved 2026-07-13 (4):**

- [x] [Review][Decision→Defer] [HIGH] Live-sidecar lane never executed / omits Task-8 scenarios → **Accepted with tracked follow-up.** Deterministic evidence accepted now; the real Tier-3 live run (persisted old-view / abort / cancellation reconciliation + the transaction partial-commit probe) HARD-GATES wiring the batch into Story 1.12/1.13 production dispatch, per the Dev Notes contingency. Logged to `deferred-work.md`. [ReadModelBatchLiveSidecarTests.cs; 1-10-...md:231]
- [x] [Review][Decision→Patch] [MEDIUM] Hardcoded `JsonSerializerDefaults.Web` vs `daprClient.JsonSerializerOptions` → **Accept the default-JSON constraint** (keep the stable serializer for fingerprint stability; document + guard/test). Moved to Patch list.
- [x] [Review][Decision→Patch] [MEDIUM] Old-view read hands out the envelope wrapper's ETag → **Fail-safe ETag + reclaim** (opaque/non-matching ETag during a prepared batch; compaction reclaims a foreign-overwritten key). Moved to Patch list.
- [x] [Review][Decision→Patch] [LOW] `CleanupPending` unreachable → **Remove the dead member** and document cleanup as always-synchronous-before-`Completed`. Moved to Patch list.

**Patch (12):**

- [x] [Review][Patch] [HIGH] Concurrent same-identity flush throws `InvalidOperationException` out of `ExecuteAsync` instead of a structured result — when a sibling same-identity run advances the marker past `Prepared` (to Committed/Completed/Aborting) between this run's marker read and its prepare-CAS, `PrepareMarkerAsync` throws, and the outer catch deliberately excludes `InvalidOperationException`, so it escapes. Violates Decision 1 (recovery/concurrency outcomes must be structured, never thrown). Fix: on the lost-race re-read, fall through to the existing status handling (Completed→AlreadyCompleted, Committed→compact, Aborting/Aborted→reconcile). [ReadModelBatchProtocol.cs:237, 82]
- [x] [Review][Patch] [HIGH] Transaction partial-commit detection (`VerifyTransactionAsync` Incomplete branches) has zero executed coverage — both test doubles model transactions strictly all-or-nothing, so the single behavior AC2 exists for is never exercised; deleting the entire read-back loop keeps every executed test green. Fix (deterministic, no live env needed): extend a test double to apply a transaction partially (write receipt, drop/alter one op) and assert `VerifyTransactionAsync` returns `Incomplete`, not `Completed`. [ReadModelBatchProtocol.cs:558-580; RecordingDaprClient.cs; InMemoryReadModelStore batch accessor]
- [x] [Review][Patch] [MEDIUM] TransactionQualified ETag/create-only precondition rejection is misclassified as non-convergent `Incomplete` — an atomic transaction rejection throws → reconcile finds marker `Prepared` → `default` arm returns `Incomplete` ("retry converges"), but the retry re-issues the identical doomed transaction forever; the resumable path returns `Conflict` for the same condition (AC8 parity break; Decision 3 mandates `Indeterminate` unless conflict/completion proved). Fix: make the transaction-origin reconcile return `Indeterminate` (or `Conflict` on proven stale-ETag), not `Incomplete`. [ReadModelBatchProtocol.cs:523-548, 614-615]
- [x] [Review][Patch] [MEDIUM] TransactionQualified retry trusts the marker alone (`Completed → AlreadyCompleted`) without re-reading operation keys — re-introduces the "trust the void transaction" flaw the story exists to prevent: after a partial commit that wrote the receipt but not a data key, a retry returns success though the key is still missing. Fix: on a transaction-profile retry with marker `Completed`, re-verify operation keys (reuse `VerifyTransactionAsync` read-back) before returning `AlreadyCompleted`. [ReadModelBatchProtocol.cs:514-516]
- [x] [Review][Patch] [MEDIUM] Envelope detection is a raw substring scan for `"$hxrmb"`; a legacy/user value merely containing that token is misdecoded as a defaulted envelope and read back as absent — `IsEnvelope` matches any byte occurrence, and `FromBytes` leniently returns a defaulted envelope (empty ScopeHash) → `ResolveVisibleAsync` returns "not found" for a real legacy value, violating "preserve legacy stored JSON." Fix: after the substring prescreen, require a genuine envelope (top-level `$hxrmb` version property present + non-empty ScopeHash/Fingerprint) before treating it as one. [ReadModelBatchEnvelope.cs:51; ReadModelBatchProtocol.cs:102-131]
- [x] [Review][Patch] [MEDIUM] `CommitAsync` force-transitions the marker to `Committed` from any status (no `Prepared` guard) — under concurrent same-identity execution plus an external interfering writer, one run can move the marker to `Aborting`/`Aborted` (compensating) while another forces it to `Committed`, producing a torn committed view; sibling transitions all guard their source status, commit does not. Fix: guard commit to transition only from `Prepared`; on Aborting/Aborted return a structured `Indeterminate`/`OptimisticConflict`. [ReadModelBatchProtocol.cs:302-319]
- [x] [Review][Patch] [MEDIUM] Fingerprint golden-vector test is self-referential for value bytes/type name — both are derived via the same production helpers, so a silent change to value serialization moves fingerprint and expectation in lockstep; a cross-deployment idempotency break (stored receipts stop matching → `AlreadyCompleted` regresses to full re-application) passes green. Fix: add a frozen golden test pinning the ACTUAL fingerprint hash of a known typed value to a hardcoded hex constant. [ReadModelBatchFingerprintTests.cs:43-56]
- [x] [Review][Patch] [LOW] AC7 "checkpoints unchanged" has no executed test — only the unexecuted live lane seeds/asserts a checkpoint key; no deterministic test proves a batch leaves an unrelated key untouched. Fix: seed a checkpoint-shaped key in the fake, run success/conflict/abort batches, assert byte-unchanged. [ReadModelBatchLiveSidecarTests.cs:84-85 only]
- [x] [Review][Patch] [LOW] Public `ReadModelBatchConcurrency` primary constructor accepts a null `ExpectedETag` — an `ExpectedETag`-mode policy with null etag bypasses the factory guards and NREs mid-install (leaked `Prepared` marker + partial envelopes + `Incomplete`) instead of failing before state access (AC1). Fix: `ArgumentNullException.ThrowIfNull(ExpectedETag)` in the record constructor. [ReadModelBatchConcurrency.cs:17; ReadModelBatchProtocol.cs:637]
- [x] [Review][Patch] [MEDIUM] (from Decision) Document + guard "batch requires default DAPR JSON options" — keep the stable canonical serializer for fingerprint+stored bytes (preserves Decision 2 fingerprint stability); add a guard/test asserting the effective `DaprClient.JsonSerializerOptions` matches the assumed Web defaults, and note the constraint in the read-model authoring docs so a consumer that customizes DAPR JSON is warned before values fail to round-trip through `GetAsync`. [ReadModelBatchCanonicalJson.cs:12; DaprReadModelStore.GetAsync]
- [x] [Review][Patch] [MEDIUM] (from Decision) Fail-safe ETag during a prepared batch + compaction reclaim — `ResolveVisibleAsync` must not surface the envelope wrapper's ETag as the previous value's ETag; return an opaque/non-matching ETag while the marker is prepared/aborting so a concurrent single-key CAS fails safely, and make `CompactAndCompleteAsync` reclaim a key whose envelope was overwritten so the batch converges instead of `Indeterminate` forever. [ReadModelBatchProtocol.cs:125-130, 345-364]
- [x] [Review][Patch] [LOW] (from Decision) Remove unreachable `ReadModelBatchResult.CleanupPending` — drop the param/member and document cleanup as always-synchronous-before-`Completed` in this story. [ReadModelBatchResult.cs:34]

**Deferred (3):**

- [x] [Review][Defer] [LOW] DAPR accessor infers key existence from ETag presence (ETag-less store/value reads as absent) [DaprReadModelBatchStateAccessor.cs:23] — deferred, backend-semantics assumption; masked on Redis (always returns ETags) and the resumable CAS protocol requires ETags anyway.
- [x] [Review][Defer] [LOW] Corrupt/tampered base64 in a stored envelope throws `FormatException` out of `GetAsync`/reconcile (only `JsonException` guarded) [ReadModelBatchEnvelope.cs:72-78] — deferred, out-of-contract storage-corruption path.
- [x] [Review][Defer] [LOW] Orphaned foreign envelope from an abandoned batch permanently blocks other batches on the same key (`OptimisticConflict` forever until the original retries) [ReadModelBatchProtocol.cs:262-269] — deferred, inherent to the resumable no-TTL design; Decision 5 defers bounded retention/cleanup to Story 1.13.
...
| 2026-07-13 | Adversarial code review (`bmad-code-review`, 4 blind layers): 16 findings, 0 dismissed. Applied 12 patches — concurrent same-identity flush now reconciles to a structured result instead of throwing (`ReadModelBatchRaceException`); transaction dispatch failure classifies ETag rejection as convergent `Conflict`/`Indeterminate` (not looping `Incomplete`); transaction retry re-verifies keys instead of trusting the receipt; `CommitAsync` guards against force-committing over an in-flight abort; envelope detection requires a real top-level discriminator (no legacy-data hiding); envelope-wrapped reads surface an empty ETag + compaction reclaims a foreign-overwritten key (fail-safe single-key coexistence); null-ETag concurrency fails before state access; removed unreachable `CleanupPending`; documented the default-DAPR-JSON constraint. Added deterministic tests: transaction partial-commit read-back proof, transaction completed-retry re-verification, frozen literal fingerprint golden, AC7 checkpoint-unchanged, null-ETag validation, DAPR default-JSON guard. Verified: Client.Tests 651, Testing.Tests 150, DomainService.Tests 88 — all green; full Release build clean. 4 items deferred (3 low-severity + the Tier-3 live-sidecar run, which HARD-GATES Story 1.12/1.13 production wiring per `deferred-work.md`). Status review → done. |
