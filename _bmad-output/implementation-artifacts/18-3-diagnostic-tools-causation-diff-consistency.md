...
- [ ] **Task 1: Extend AdminApiClient.Streams.cs with diagnostic methods** (AC: #1, #2, #7)
  - [ ] 1.1 Add `DiffAggregateStateAsync` method to existing `AdminApiClient.Streams.cs`:
    ```csharp
  ...
  - [ ] 1.2 Add `TraceCausationChainAsync` method to existing `AdminApiClient.Streams.cs`:
    ```csharp
...
- [ ] **Task 2: Create AdminApiClient.Consistency.cs** (AC: #3, #4, #7)
  - [ ] 2.1 Create `src/Hexalith.EventStore.Admin.Mcp/AdminApiClient.Consistency.cs` — partial class with:
  ...
  - [ ] 2.2 All methods use `System.Net.Http.Json` extensions. All methods are `async Task<T?>` or `async Task<IReadOnlyList<T>>` with `CancellationToken`. All path segments URI-encoded.

- [ ] **Task 3: Create Stream Diagnostic MCP tools** (AC: #1, #2, #5, #6)
  - [ ] 3.1 Create `src/Hexalith.EventStore.Admin.Mcp/Tools/DiagnosticTools.cs`:
    ```csharp
    [McpServerToolType]
    ...
    ```
  - [ ] 3.2 Both tools follow the error-handling pattern from story 18-2:
...
- [ ] **Task 4: Create Consistency MCP tools** (AC: #3, #4, #5, #6)
  - [ ] 4.1 Create `src/Hexalith.EventStore.Admin.Mcp/Tools/ConsistencyTools.cs`:
    ```csharp
    [McpServerToolType]
    internal static class ConsistencyTools
    {
        [McpServerTool(Name = "consistency-list")]
        [Description("List consistency check summaries with status, scope, and anomaly counts")]
        ...
        [Description("Get full consistency check results including anomaly details, severity, and affected streams")]
    ...
    ```
  - [ ] 4.2 `consistency-list` uses `ToolHelper.SerializeResult` for the list — empty array on null is fine (list endpoint).
  - [ ] 4.3 `consistency-detail` checks for null result and returns `ToolHelper.SerializeError("not-found", "...")` (single-entity endpoint).

- [ ] **Task 5: Create tests for AdminApiClient diagnostic methods** (AC: #7)
  - [ ] 5.1 Create `tests/Hexalith.EventStore.Admin.Mcp.Tests/AdminApiClientDiagnosticTests.cs`:
  ...
  - [ ] 5.2 Create `tests/Hexalith.EventStore.Admin.Mcp.Tests/AdminApiClientConsistencyTests.cs`:
...
- [ ] **Task 6: Create tests for MCP diagnostic tools** (AC: #1-#6)
  - [ ] 6.1 Create `tests/Hexalith.EventStore.Admin.Mcp.Tests/DiagnosticToolsTests.cs`:
  ...
  - [ ] 6.2 Create `tests/Hexalith.EventStore.Admin.Mcp.Tests/ConsistencyToolsTests.cs`:
  ...
  - [ ] 6.3 All tests use `MockHttpMessageHandler` from story 18-2 test infrastructure. Use xUnit `[Fact]`/`[Theory]`, Shouldly assertions, NSubstitute where needed.

- [ ] **Task 7: Verify end-to-end** (AC: #8)
  - [ ] 7.1 Build solution: `dotnet build Hexalith.EventStore.slnx --configuration Release` — zero warnings
  - [ ] 7.2 Run all Tier 1 tests — all green
  - [ ] 7.3 Verify all 4 new tools appear in `tools/list` response (discovered automatically via `WithToolsFromAssembly()`)
...
- **AdminApiClient.Streams.cs**: Already has 4 methods (`GetRecentlyActiveStreamsAsync`, `GetStreamTimelineAsync`, `GetAggregateStateAsync`, `GetEventDetailAsync`). New diagnostic methods follow the same pattern: build URL path with `Uri.EscapeDataString()` for path segments, call `GetAsync<T>`, return nullable result.
...
- Tool class pattern: `[McpServerToolType] internal sealed class`, static methods with `[McpServerTool(Name = "...")]` and `[Description("...")]`
