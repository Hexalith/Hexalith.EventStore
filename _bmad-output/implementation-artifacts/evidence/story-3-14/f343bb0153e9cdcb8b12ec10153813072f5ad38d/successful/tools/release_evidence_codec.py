...
def validate_identity(document, expected_package_ids, expected_manifest_sha256=None):
    ...
    # Recorded, not compared against the running CODEC_VERSION: the retained bytes below are the
    # binding, so a newer codec can still re-derive an older packet's identity.
    _positive_integer(codec["version"], "codec version is invalid")
    if _file_binding(codec["codec"])["file"] != RETAINED_CODEC_FILE or _file_binding(
        codec["verifier"]
    )["file"] != RETAINED_VERIFIER_FILE:
...
def _validate_authority(document, packet_root):
    ...
    if (
        ...
        or raw.get("html_url") != f"https://github.com/{REPOSITORY}/issues/346#issuecomment-{comment_id}"
    ...
    role_user = role.get("user")
    if (
        not isinstance(role_user, dict)
        or role_user.get("login") != "jpiquot"
        or role.get("permission") not in {"admin", "maintain", "write"}
    ):
    ...
    if (
        ...
        or receipt.get("html_url") != f"https://github.com/{REPOSITORY}/issues/346#issuecomment-{receipt_id}"
