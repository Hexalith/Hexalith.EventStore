...
type: 'bugfix'
created: '2026-09-01'
baseline_revision: f5bd22e1b02d54e5bb3311ec8482ffde9345fba9
...
| Completed-work re-drive | All Execution items are checked, their implementation is already present at `baseline_revision`, and the dispatch-start status snapshot is empty for the four Execution code paths | Treat the dispatch as verification/finalization-only, require no new implementation diff, rerun Verification at current `HEAD`, and return complete when every command satisfies its expected result | A blocked result must name an overlapping Execution path, the failed or unexecutable dispatch-start status command, or the exact failed or unexecutable Verification command and its concrete reason; any evidence-free generic blocked outcome, including one that caused the current pause, is not blocker evidence and cannot be carried forward |
...
| Unrelated dirty workspace | Pre-existing or concurrent changes exist outside the four Execution code paths | Leave those changes untouched and report the story complete after every specified verification passes, irrespective of repository-wide dirty status | Unrelated repository-wide dirtiness is informational and cannot override the scoped result |
...
- Given a session returns blocked, then it identifies either a staged, unstaged, or untracked dispatch-start entry in one of the four exact Execution code paths, the exact dispatch-start status command that could not execute, or an exact Verification command that could not execute or did not satisfy its expected result, and includes the concrete path/status or command failure evidence; a generic blocked outcome without that evidence is non-conforming.
- Given any evidence-free generic blocked outcome, including one that caused the current pause, when the story is freshly re-driven, then that outcome is non-conforming session output rather than blocker evidence, and only fresh owned-path overlap, dispatch-start status-command failure, or Verification-command failure or expected-result mismatch may block; if the fresh scoped snapshot is empty and every Verification command passes, the session must report the story complete.
- Given pre-existing or concurrent changes only outside the four Execution code paths, or the only owned artifact change is this resolver-approved spec edit, when every specified verification passes and finalization runs, then finalization reports the story complete while unrelated changes remain untouched; repository-wide dirtiness, committed implementation history, and the absence of a fresh diff do not block.
