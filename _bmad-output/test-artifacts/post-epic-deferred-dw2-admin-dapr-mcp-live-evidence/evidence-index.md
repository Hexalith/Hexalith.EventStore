...
|---:|---|---|---|---|---|---|---|---|
| 1 | `EnableKeycloak=false aspire run --detach --non-interactive --format Json --apphost src/Hexalith.EventStore.AppHost/Hexalith.EventStore.AppHost.csproj`; Aspire MCP `list_resources` | AppHost resources | Runtime baseline below; `aspire-list-resources-snapshot.json` | Runtime baseline lists command, flags, dashboard/API URLs, resource states, skipped Keycloak | Resources running/healthy; Keycloak skipped intentionally; URLs co-located in baseline | passed | Dashboard login token redacted | N/A |
| 2 | HTTP Admin DAPR smoke | `eventstore-admin` | `admin-dapr-smoke-summary.json`; `admin-dapr-components-response.json`; `admin-dapr-sidecar-response.json`; `admin-dapr-actors-response.json`; `admin-dapr-pubsub-response.json`; `admin-dapr-resiliency-response.json`; `admin-dapr-health-response.json`; `admin-dapr-health-dapr-response.json`; `admin-dapr-health-dapr-history-response.json` | components, sidecar, actors, pub/sub, resiliency, health history covered | 8/8 endpoints returned 200 | passed | JWT not stored; response scrubbed for secret-like keys | `STORY:post-epic-deferred-dw2-admin-dapr-mcp-live-evidence` |
...
| 11 | Markdown artifact review | evidence folder | this index | Required evidence tables and links are durable | All required tables present; every File List artifact linked from this index | passed | No secrets stored | `STORY:post-epic-deferred-dw2-admin-dapr-mcp-live-evidence` |
| 12 | `deferred-work.md` edit | DW2-relevant bullets | dispositions table below; `deferred-work.md` | Touched bullets carry clear disposition markers | DW2-relevant bullets dispositioned with `STORY:...`, `ACCEPTED-DEBT`, or `NO-ACTION`; unrelated DW6 governance items left untouched | passed | N/A | `STORY:post-epic-deferred-dw2-admin-dapr-mcp-live-evidence` |
| 13 | Manual redaction review | all artifacts | this index and linked artifacts | Tokens, secrets, payloads, and sensitive values omitted | Redaction policy applied; no JWT-shaped tokens, secrets, payloads, or raw state values in artifacts; stdout transcript body-shaped only | passed | Applies to all artifacts in this folder | `STORY:post-epic-deferred-dw2-admin-dapr-mcp-live-evidence` |
| 14 | Git diff review | topology/contracts | final diff | No topology/product contract change unless smoke-proven narrow defect | Diff scope limited to `_bmad-output/**`; no production code, no DAPR YAML, no Admin/MCP contract changes | passed | N/A | `STORY:post-epic-deferred-dw2-admin-dapr-mcp-live-evidence` |
| 15 | Story bookkeeping | story and sprint status | story file; sprint-status.yaml | Dev Agent Record, File List, Change Log, Verification Status updated; review status only after evidence | Dev Agent Record, File List, Change Log, Verification Status updated; sprint-status row moved `in-progress` → `review` only after evidence and validation | passed | N/A | `STORY:post-epic-deferred-dw2-admin-dapr-mcp-live-evidence` |
...
| 2026-05-05T13:51:36+02:00 | Aspire MCP `list_resources` | Aspire dashboard URL | Reachable HTTPS endpoint | `https://localhost:17017` | passed | N/A | Yes | `aspire-list-resources-snapshot.json` |
| 2026-05-05T13:51:36+02:00 | Aspire MCP `list_resources` | Admin Server URL (`eventstore-admin`) | Reachable HTTP endpoint for `/api/v1/admin/**` | `http://localhost:8090` | passed | N/A | Yes | `aspire-list-resources-snapshot.json` |
| 2026-05-05T13:51:36+02:00 | Aspire MCP `list_resources` | Admin UI URL (`eventstore-admin-ui`) | Reachable HTTPS endpoint for browser UI | `https://localhost:7197` (browser proof not used in this run) | passed | N/A | Yes | `aspire-list-resources-snapshot.json` |
| 2026-05-05T13:51:36+02:00 | Aspire MCP `list_resources` | CommandAPI URL (`eventstore`) | Reachable HTTP endpoint for `/api/v1/commands` | `http://localhost:8080` | passed | N/A | Yes | `aspire-list-resources-snapshot.json` |
| 2026-05-05T13:51:36+02:00 | Aspire MCP `list_resources` | Remote EventStore sidecar HTTP | Reachable for remote DAPR metadata | `http://localhost:3501` | passed | N/A | Yes | `aspire-list-resources-snapshot.json` |
| 2026-05-05T13:51:36+02:00 | Aspire MCP `list_resources` | eventstore / eventstore-admin / eventstore-admin-ui / sample / sample-blazor-ui / tenants | Running + Healthy | All listed project resources Running + Healthy | passed | N/A | Yes | `aspire-list-resources-snapshot.json` |
| 2026-05-05T13:51:36+02:00 | Aspire MCP `list_resources` | eventstore-dapr-cli / eventstore-admin-dapr-cli / sample-dapr-cli / tenants-dapr-cli | Running + Healthy | All listed DAPR CLI sidecars Running + Healthy | passed | N/A | Yes | `aspire-list-resources-snapshot.json` |
| 2026-05-05T13:51:36+02:00 | Aspire MCP `list_resources` | `statestore`, `pubsub` | Running + Healthy | Both DAPR components Running + Healthy | passed | N/A | Yes | `aspire-list-resources-snapshot.json` |
| 2026-05-05T13:51:36+02:00 | AppHost config | Keycloak | Skipped when `EnableKeycloak=false` | No `keycloak` resource present; symmetric JWT dev mode used | passed | N/A | Yes | `aspire-list-resources-snapshot.json` |
...
| 2026-05-05T13:52:51+02:00 | health history | `/api/v1/admin/health`, `/health/dapr`, `/health/dapr/history` | Component health timeline or empty-timeline classification | 200; current DAPR health and history `hasData=true`, 5 entries, not truncated | N/A | N/A | passed | `admin-dapr-health-response.json`; `admin-dapr-health-dapr-response.json`; `admin-dapr-health-dapr-history-response.json` |
...
|---|---|---|---|---|---|---|---|---|
| 2026-05-05T13:54:13+02:00 | timeline | Admin API | Cross-stream timeline shape or classified failure | 200; shape `totalCount,continuationToken,items` over the canonical seeded stream | shape artifact only | no timeout | passed | `debugging-timeline-shape.json` |
...
| 2026-05-05T13:54:13+02:00 | bisect | Admin API | Divergence search shape or classified failure | 200; shape includes `goodSequence,divergentSequence,totalSteps,isTruncated,steps`. NOTE: live URI used `good=0&bad=1` over the 5-event seed which exercises only a single-event boundary, not a multi-step binary search; recorded as evidence gap (DW2 review, P2). A follow-up live pass should use `good=0&bad=5` against the same canonical seeded stream. | shape artifact only | no timeout | passed; degenerate-range | `debugging-bisect-shape.json` |
...
| Time | Tool | Timer source | Timer scope | Cold/warm | Sample count | Retries included? | Initialization included? | Admin API latency included? | Durations | Local environment caveat | Classification | Artifact |
|---|---|---|---|---|---:|---|---|---|---|---|---|---|
| 2026-05-05T13:57:00+02:00 | `health-dapr` | Python `time.perf_counter` around JSON-RPC `tools/call` | Outer measurement wraps stdio write-and-read on MCP pipes; includes pipe latency + server handler + Admin API HTTP roundtrip; not server-side handler time alone | warm after initialize/tools-list | 3 | No | No | Yes | 8.378 ms, 1.895 ms, 1.411 ms; min 1.411 / avg 3.895 / max 8.378 | Local developer machine; not a product SLA; three-sample baseline implies no statistical confidence | sample-only | `mcp-smoke-summary.json` |
...
## Evidence Gaps Recorded by DW2 Code Review

These items were flagged during the post-evidence code review. They are recorded as known gaps rather than fabricated retroactively; closing them requires a follow-up live capture pass.

| Gap | AC | Detail | Required action for next pass |
|---|---|---|---|
| Bisect URI used `good=0&bad=1` (single-event boundary) | AC #5 | `debugging-bisect-shape.json` exercises only a one-event range over the 5-event seed; the divergence/binary-search branch is not actually walked | Re-run bisect with `good=0&bad=5` against the canonical seeded stream and refresh `debugging-bisect-shape.json`; mark `degenerate-range` removed |
| Aspire MCP `list_resources` raw response not byte-preserved | AC #1, AC #11 | Resource state was synthesized into `aspire-list-resources-snapshot.json` post-hoc from runtime baseline observations; the underlying MCP tool result envelope is not preserved | Persist the raw `list_resources` MCP response on the next capture pass and replace the synthesized snapshot |
| MCP write-preview non-mutation proof is `0 == 0` | AC #8 | Both `consistency-list` calls returned `totalCount=0`; the equality is trivial and does not discriminate "no mutation" from "system was already empty" | On a future pass, seed at least one consistency-check entry first so `before.totalCount` and `after.totalCount` are non-zero and the equality is meaningful |
| Session-fallback proof is shape-of-success only | AC #9 | `stream-list` after `session-set-context` returned `totalCount=112`; no comparison call (with explicit tenantId/domain) was recorded to confirm the session values were actually applied to filter | Add a paired call with explicit `tenantId=tenant-a&domain=counter` and assert the totalCount equals the session-context call |
| `notifications/initialized` lifecycle handshake not captured | AC #7 | Per MCP 2025-06-18 spec, the client should send a `notifications/initialized` notification after the initialize result. The current `mcp-stdout-transcript.jsonl` only records server→client responses (id 1..12); the client-side notification is absent from the smoke transcript | On the next pass, capture both client→server and server→client traffic so the lifecycle notification is visible (split harness logging or use a transcript that includes notifications) |
