...
| Story slicing | `epics.md` carries coordinated-slice gates for Stories 1.3, 1.6, 2.4, 3.7, 5.6, 7.2, 7.3, 7.4 with named owners, review boundaries, and validation commands. Implementation story files must split or carry those gates forward. |
| Backlog story shape | Story 7.5 is reclassified as backlog/planning work and names exact artifacts under `_bmad-output/planning-artifacts/backlog/`. |
...
| --- | --- |
| 1.3 | Split read-model store, cursor codec, and test fake/conflict semantics unless accepted as one coordinated slice; Tenants adoption moves to 1.6. |
...
| 2.4 | Split Tenants generated contract/API-host work from UI alignment and compatibility validation unless accepted as one coordinated slice. |
| 3.7 | Split CI gate migration from supply-chain backlog hardening unless accepted as one coordinated slice. |
...
| 7.2 | Split claims normalization, audit logging, deferred-operation UI/server honesty, and shared typed-client reduction unless accepted as one coordinated slice. |
| 7.3 | Split secret-store use, readiness/app-health checks, DAPR resiliency policy, and immutable image tags unless accepted as one coordinated slice. |
| 7.4 | Split CI integration-test lane recovery, persisted state evidence assertions, fake/integration test reclassification, and perf-lab workflow setup unless accepted as one coordinated slice. |
| 7.5 | Treat as backlog planning with exact deliverables for GDPR-1, IAM-1, KIT-1, and REST generator hardening. |
