...
| Architecture-to-epic handoff | `epics.md` preserves all existing story identities, resolves the conflicts in `architecture-reconciliation.md`, assigns focused ownership and evidence for AD-26 through AD-33, and records current PRD and architecture digests only after substantive reconciliation and renewed approval. Completed evidence is narrowed rather than rewritten; new obligations receive successor stories where necessary. |
...
| Erasure phases | Story 1.14 evidence is accepted only for AD-7 projection removal. Any AD-30 claim requires a domain-policy-owned workflow ID, fenced phases, and separate logical, projection, cryptographic, broker, backup, restore-point, cache, export, replica, and legal-hold results with no inferred overall completion. |
| Dead-letter capture | Every production subscriber acknowledges poison/terminal rejection only after the AD-31 sink durably accepts the tenant/domain `MessageId` record. `eventstore-operations` remains non-production until topology, release, AD-28, AD-29, tenant/target, common-catalog, quarantine, and replay evidence pass. |
| Correlation identity | `Contracts` enforces `X-Correlation-ID` as 1-128 ASCII alphanumeric/hyphen characters; only the first public boundary accepts or mints, downstream replacements are rejected, and correlation never selects command status or substitutes for `traceparent`. |
| Unified catalog | One retained-byte `deploy/dapr/eventstore-routing-catalog.json` joins route and idempotency facets with stable route-entry IDs and one root generation. Every required host validates identical root/facet digests through prepare/ready/commit or rolls back; non-Development overrides, partial generations, and mismatches fail readiness. |
| Deployed-parity handoff | Story 3.13 remains the negative `v3.94.1` disposition, Story 3.14 remains separately authorized corrective release evidence, and Story 3.15 alone may establish positive exact-lineage parity. Current Story 3.15 `in-progress` status grants no parity, deployment, or migration authority. |
...
| OQ8 platform | Stories 4.9-4.13 implement trusted admission, digest-directory rotation, state/fence behavior, expiry/tombstones, and legacy reconciliation. Stories 4.14-4.15 own only the EventStore source-side platform packet; Folders owns canonical `oq8-idempotency-evidence.yaml` and OQ8 closure. Story 4.14 evidence against `oq8-postgresql-v1` remains correctness evidence, not complete AD-26 production-profile authority; the Story 4.8 ledger carries no executable status. |
...
| Readiness rerun | Only after the PRD maintenance, architecture-to-epic handoff, lifecycle reconciliation, and every other PRD blocker close can a fresh assessment validate FR1-FR37, NFR1-NFR19, and AD-1 through AD-33. The assessment must report no stale production claim, ungoverned dependency, contradictory story rule, missing owner, or Development-only evidence used for production. Epic 8 remains separately gated post-MVP work. |
...
`requirements-traceability.md` may name additional supporting stories. It reflects the current
epic plan and is not completion authority for AD-26 through AD-33 until the required epic update
assigns those decisions to focused stories.
