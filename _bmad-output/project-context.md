...
- `KeycloakPersistent=true` opts the Keycloak container into persistent lifetime for faster `aspire run` restarts (default OFF — a documented exception to the "prefer non-persistent resources" rule). EXPERIMENTAL: persistent lifetime alone does NOT reuse the container — Aspire's random per-run endpoint host ports churn DCP's `lifecycle-key` and force a recreate, so this flag also pins Keycloak's `http`/`management` endpoints proxyless to fixed host ports `8180`/`8543` to stabilize the hash. The two ports are configurable via `KeycloakHttpPort`/`KeycloakManagementPort` (read only on the persistent path; validated fail-fast at AppHost build — integer 1..65535, distinct, not the EventStore `8080`); relocate them if `8180`/`8543` collide with host software (a fixed port can wedge Keycloak in `Created`). Do NOT add a runtime free-port probe — a warm reused container legitimately holds the ports between runs. A reused container skips realm re-import, so remove it after editing `KeycloakRealms/hexalith-realm.json`. Reuse + OIDC verified under `aspire run` AND under Tier 3 fixtures (`KEYCLOAK_TEST_REUSE=true`): two consecutive `dotnet test` runs reattach the byte-identical persistent container (same ID/`CreatedAt`/`lifecycle-key`, warm run ~26% faster) — `DistributedApplicationTestingBuilder`/`DisposeAsync` does NOT tear it down (dw19b 2026-05-25). Note: 5 authorized Tier 3 command-submission E2E tests return a pre-existing `500` in the actor/command pipeline identically on cold, warm, and default non-reuse runs — unrelated to reuse/ports (tracked separately).
- All Keycloak-dependent services must `WaitFor(keycloak)` (healthy), in every mode including tests — do NOT relax to `WaitForStart`. Tier 3 fixtures build this same AppHost in run mode, and a service reaching healthy before Keycloak can validate tokens causes OIDC-discovery races and flaky auth tests.
