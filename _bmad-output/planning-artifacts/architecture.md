...
- **Context:** The admin tool needs to surface traces, metrics, and logs. The observability stack (Zipkin/Jaeger, Prometheus/Grafana, Aspire Dashboard) already provides rich UIs for these.
- **Decision:** The admin Web UI displays event store domain-aware summary views (e.g., command latency per aggregate type, projection lag per tenant) and deep-links to the external observability tool for detailed exploration. It does not embed or replicate trace viewers, metric dashboards, or log explorers.
...
> **Note:** This section reflects 7 shipped packages (SignalR and Admin.Abstractions added post-initial architecture).
...
| ---------------------------------- | ----------------------------------------------------------- | ----------------------------------------- |
| `Hexalith.EventStore.Admin.Server` | Admin API + Blazor Web UI, connects to event store via DAPR | Aspire resource (container)               |
| `Hexalith.EventStore.Admin.Cli`    | CLI tool calling Admin API over HTTP                        | .NET global tool (`dotnet tool install`)  |
| `Hexalith.EventStore.Admin.Mcp`    | MCP server calling Admin API over HTTP                      | Stdio executable for AI agent integration |

**Peer Services (external bounded contexts, co-orchestrated via Aspire):**

| Service              | Purpose                                                                            | Integration                                |
| -------------------- | ---------------------------------------------------------------------------------- | ------------------------------------------ |
| `Hexalith.Tenants`   | Tenant lifecycle management (create, enable/disable, users, roles, configuration). Owns FR77 implementation. | Aspire peer resource with own DAPR sidecar. Admin UI/CLI/MCP consume its Client SDK |

Hexalith.Tenants is itself an event-sourced service built on Hexalith.EventStore. In the Aspire topology, it runs as an independent service with its own DAPR sidecar, state store, and pub/sub topics. EventStore admin tools (Web UI, CLI, MCP) consume Hexalith.Tenants.Client packages for tenant operations — they never bypass the Tenants command pipeline.
...
- semantic-release for automated SemVer from Conventional Commits
...
| D9  | NuGet Package Versioning            | semantic-release (Conventional Commits → SemVer, automated publish)        | --                                                                               |
...
- **Service discovery:** Resolver precedence is contractual: (1) exact static registration keyed by `tenant:domain:version`, (2) exact static registration keyed by `tenant|domain|version`, (3) pipe wildcard static registration keyed by `*|domain|version`, (4) sanitized wildcard static registration keyed by `wildcard_{domain}_{version}`, (5) opt-in DAPR config-store lookup when `ConfigStoreName` is non-empty, (6) convention fallback with `AppId = domain` and `MethodName = "process"`. The sanitized wildcard key remains `wildcard_{domain}_{version}` and is safe only while domains exclude `_` and versions match `^v[0-9]+$`; `DomainServiceResolver` validates this invariant before building the sanitized key. Config store is disabled by default (`ConfigStoreName = null`). Exact registrations always win over wildcards; wildcard registrations have their `TenantId` rewritten to the actual caller before being returned to downstream invokers.
...
#### D9: Package Versioning -- semantic-release

- **Tool:** semantic-release (derives SemVer from Conventional Commits)
- **Workflow:** Merging to main triggers semantic-release which analyzes commit messages (`feat:` = minor, `fix:` = patch, `BREAKING CHANGE:` = major), creates Git tag, builds, packs, and publishes NuGet packages
- **Monorepo strategy:** Single version injected via `-p:Version=` MSBuild property; all packages share version
- **Rationale:** Fully automated versioning and publishing, enforced commit conventions, auto-generated changelog

#### D10: CI/CD Pipeline -- GitHub Actions

- **Platform:** GitHub Actions (free for open-source repositories)
- **Pipelines:** Build + test + commitlint on PR, semantic-release on merge to main (version + pack + publish NuGet + GitHub Release), DAPR integration tests with containerized DAPR sidecar
...
| `no-tenant-user` | _(none)_           | orders            | command:submit                                | Tenant validation rejection  |
...
9. **CI/CD** -- GitHub Actions with semantic-release (D9, D10), three-tier test execution
...
EventStore query routing is a platform integration boundary. Domain services that serve queries through `POST /api/v1/queries` implement the Contracts-owned, implementation-neutral projection query method contract while runtime/host projects own any DAPR actor inheritance. Contracts owns `QueryEnvelope`, `QueryResult`, `QueryAdapterFailureReason`, and `IProjectionActor.QueryAsync(QueryEnvelope)` without referencing DAPR, gRPC, or protobuf packages; Server/runtime owns DAPR actor adapters and weak proxy invocation. The contract must define:
...
**QueryRouter weak ActorProxy invocation (post-Epic-22 R22A1, ES9).** `QueryRouter` invokes the generic projection query method through the **weak** DAPR proxy path — that is, `IActorProxyFactory.Create(ActorId, actorType, options?)` followed by `ActorProxy.InvokeMethodAsync<QueryEnvelope, QueryResult>(nameof(IProjectionActor.QueryAsync), envelope, cancellationToken)`. This is required because `IProjectionActor.QueryAsync(QueryEnvelope)` is part of the public Contracts surface and intentionally does not accept a `CancellationToken`, while the HTTP request scope must still be able to cancel the DAPR invocation. Strongly typed dispatch proxies returned by `IActorProxyFactory.CreateActorProxy<IDaprProjectionActor>(...)` are runtime-owned Server/host details and MUST NOT be cast back to `ActorProxy` for weak/JSON invocation — the dispatch proxy does not initialize the weak-path state and the cast produced a runtime `NullReferenceException` on every `list-tenants` call (see post-Epic-22 R22A1 sprint-change proposal). ES9 keeps the method-name anchor in Contracts while moving DAPR actor inheritance to Server/hosting code.
...
| D9 + D10      | semantic-release Conventional Commits integrated into GitHub Actions release pipeline | Yes         |
