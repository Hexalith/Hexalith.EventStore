...
**QueryRouter weak ActorProxy invocation (post-Epic-22 R22A1).** `QueryRouter` invokes the generic `IProjectionActor` through the **weak** DAPR proxy path — that is, `IActorProxyFactory.Create(ActorId, actorType, options?)` followed by `ActorProxy.InvokeMethodAsync<QueryEnvelope, QueryResult>(nameof(IProjectionActor.QueryAsync), envelope, cancellationToken)`. This is required because `IProjectionActor.QueryAsync(QueryEnvelope)` is part of the public contracts surface and intentionally does not accept a `CancellationToken`, while the HTTP request scope must still be able to cancel the DAPR invocation. Strongly typed dispatch proxies returned by `IActorProxyFactory.CreateActorProxy<IProjectionActor>(...)` remain valid for direct interface calls (`proxy.QueryAsync(envelope)`), but they MUST NOT be cast back to `ActorProxy` for weak/JSON invocation — the dispatch proxy does not initialize the weak-path state and the cast produced a runtime `NullReferenceException` on every `list-tenants` call (see post-Epic-22 R22A1 sprint-change proposal).
