...
- **Service discovery:** Convention-based by default — AppId = domain name, MethodName = "process" (zero configuration). Override hierarchy: (1) **exact** static registrations in appsettings.json keyed by `tenant\|domain\|version` (for local dev/test), (2) **wildcard tenant** static registrations keyed by `*\|domain\|version` — single entry covers all tenants for a given domain, useful when one DAPR app hosts multiple sample domains, (3) DAPR config store (opt-in via `DomainServiceOptions.ConfigStoreName`, for per-tenant routing in complex scenarios), (4) convention fallback. Config store is disabled by default (`ConfigStoreName = null`). Exact registrations always win over wildcards; wildcard registrations have their `TenantId` rewritten to the actual caller before being returned to downstream invokers.
