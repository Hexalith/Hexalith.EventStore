...
| OR11 | **Blocking.** Make the OQ8 design authority reproducible inside the EventStore evidence boundary using a permitted immutable copy, a complete approved normative projection, or a signed or content-addressed Folders attestation. PRD and architecture carry the repository/path/commit/digest; propagate the same complete identity and available attested bytes into `epics.md`, OQ8 packets, validator, and CI. `python3 tools/validate-oq8-platform-evidence.py --pre-review` currently exits 1 on `docs/guides/configuration-reference.md` drift; absence, mismatch, or red validation fails G-OQ8. | Architecture owner with Folders content, EventStore, and test owners | Before OQ8 closure, readiness `READY`, release, or consumer handoff |
...
| OR15 | **Blocking.** Resolve lifecycle contradictions for Stories 3.15, 4.5, 4.15, 5.2, 5.3, 5.4, and 6.1 across committed `sprint-status.yaml`, story wrappers/evidence, and `epics.md`. Story 3.15 must replace tracker subject `a5c07d178412d8fbac72ec660a3c0a94826a823f7376c61e0e7b98ea554c3448` with packet/spec subject `aafe9040786c4f3af496b7ecbe62282c89396a15362b668a7b81ee148fe3f9c5` and retain `in-progress` until G-RUNTIME-PARITY passes. Story 4.5 must separate historical FR31 race-observation completion from current-packet validation: its current acceptance says packet drift makes it `in-progress`, so tracker/epics `done` cannot supply NFR7/NFR16 evidence. Story 5.3's tracker/wrapper are now `done`, but epics remains stale. Story 5.4 now has wrapper `in-progress`, tracker `review`, and epics `backlog`; none supplies terminal evidence. Story 4.15 additionally requires `python3 tools/validate-oq8-platform-evidence.py --pre-review` to pass on the bound subject; its source-only completion flag grants no external authority. Add guarded subject and status-transition validation so the sources cannot diverge silently again. | Story owners with tracker and test owners | Before affected FR/NFR delivery claims or the next readiness re-run |
