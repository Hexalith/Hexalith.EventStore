...
implementation_readiness_last_assessed: 2026-09-09
implementation_readiness_last_baseline: 1b6f08d41de040615d3b08675d98e46cfa5bab0c
implementation_readiness_next_action: rerun-after-all-mandatory-gates-pass
prd_source_validation_assessed: 2026-09-10
prd_source_validation_baseline: 293c69c42d35dee26682d42f05c943c0b65786f4
prd_source_validation_baseline_status: examined-dirty-unapproved
prd_source_validation_grade: poor
prd_source_validation_report: _bmad-output/planning-artifacts/prds/prd-eventstore-2026-07-05/validation-report.md
prd_finalize_assessed: 2026-09-10
...
Document finality and implementation readiness are separate states. This document can be finalized as the truthful requirements baseline while implementation readiness remains blocked. The supplied validation report examined historical `HEAD` `293c69c42d35dee26682d42f05c943c0b65786f4` plus a dirty worktree and returned `Poor` / `Reject`. The finalize review examined later `HEAD` `dd55a6d1e128989777ae6c47459da0f746e84e53` plus this PRD worktree. Neither observation is a clean, atomic, or approved baseline, and one must not be substituted for the other. The executable gate contract in §11.4 currently computes `Reject`. This PRD therefore authorizes no `READY` verdict, Phase 4 MVP completion, release, deployment, consumer migration, or general dependent implementation handoff until every mandatory gate passes against one approved source identity and readiness is re-run.

A failed gate may authorize only an owner-approved, narrowly scoped corrective story or lane whose sole purpose is to produce the required fix and bound evidence. Before handoff, that exception requires a content-bound record at `_bmad-output/implementation-artifacts/evidence/corrective-work-authorizations/<gate>/<authorization-id>.json` and a passing `python3 tools/validate-corrective-work-authorization.py <record> --changed-paths-from <baseline-sha>`. The record must bind schema/version, failed gate, this PRD digest, baseline commit, input-subject digest, deterministic output-subject derivation rule, authorizing owner and role-registry identity, allowed paths and mutation classes, prohibited paths/actions, expected evidence and exact validator, issuance, expiry, revocation state, and `corrective-work-only: true` while every readiness/release/deployment/production/migration/removal flag is false. Missing, expired, revoked, mismatched, overbroad, or out-of-path records reject the handoff; a PRD, baseline, input subject, derivation rule, scope, owner, or validator change invalidates it. Postflight validation must cover the resulting diff and evidence and bind the derived output-subject digest. The failed gate can pass only on evidence bound to that postflight digest; any later output-subject change invalidates the record and requires a new authorization. This authorization does not itself change the failed gate, readiness result, lifecycle claim, or any prohibited authority.
...
For the Story 4.8 evidence ledger and active Stories 4.9-4.15, the approved 2026-07-20 OQ8 sprint change proposal and the Architecture + Security + Test-approved OQ8 design version 1.0.0 govern. This PRD, architecture, epics, and canonical SPEC package currently project that authority into EventStore, but those projections remain unreconciled and non-authorizing until G-BASELINE and G-OQ8 pass.
...
The journeys below restate existing requirements as cross-role, failure-aware flows. They do not add delivery scope or replace the final top-level `ux.md` handoff or its detailed `DESIGN.md` and `EXPERIENCE.md` artifacts, which remain draft.

- **UJ1 - Alex adopts the domain-service SDK.** Alex, a domain author, references the package-safe EventStore SDK, registers and maps the canonical host seams, and keeps the module limited to domain behavior. Guardrail and adoption evidence must expose remaining boilerplate rather than relabel it as domain logic; any residual router, state-store, projection, telemetry, health, or Aspire plumbing leaves FR1-FR10 and SM8 open. **Requirements:** FR1-FR10, NFR9, NFR12, NFR18, SM8.
- **UJ2 - Morgan exposes a generated external API.** Morgan, an external API host developer, supplies command/query contracts and generates controllers in a dedicated host. Before routing or state access, the host resolves exactly one authorized tenant, canonicalizes its case, and validates the canonical value; invalid tenant input fails closed. Accepted commands use a `MessageId` valid under the endpoint's declared contract version as the sole public status identity; missing or invalid identity omits `Location` or rejects safely, never substitutes `CorrelationId`. Invalid metadata, ambiguous routing, or unsafe errors remain support-safe and non-authorizing. **Requirements:** FR11-FR16, NFR1-NFR3, NFR5, NFR12-NFR14, G-TENANT, G-STATUS-ID.
- **UJ3 - Priya observes projection-confirmed success.** Priya, a Tenants UI maintainer, submits a command through the client library and first sees acceptance or evidence-pending state. Only projection-backed query metadata can move the UI to projection-confirmed success; stale, rebuilding, degraded, unavailable, local-only, handler-computed, or unknown-provenance results remain distinct and never become inferred success. **Requirements:** FR4, FR12, FR15-FR16, FR33-FR34, NFR8, NFR14-NFR15.
- **UJ4 - Nora investigates and recovers a failed operation.** Nora, an operator, follows tenant-scoped status, persisted event, admission, projection, checkpoint, and dead-letter evidence without exposing sensitive payloads or blindly retrying. A missing, conflicting, expired, corrupt, unavailable, or partially published state follows the bounded recovery contract; absent proof leaves the operation unresolved rather than successful. **Requirements:** FR23, FR26-FR30, FR32-FR34, NFR1-NFR2, NFR4, NFR6-NFR8, NFR10-NFR11, NFR14-NFR17.
- **UJ5 - Riley authorizes a release and consumer removal.** Riley, a release owner, evaluates one immutable release subject and its independent receipts. A consumer owner separately evaluates an exact consumer/removal subject. Missing, mixed-lineage, stale, unauthenticated, expired, or mismatched evidence authorizes neither release/deployment nor consumer infrastructure removal, even when source/package capability exists. **Requirements:** FR17, FR19, FR21-FR22, FR25, FR32, FR36, NFR11-NFR12, NFR16-NFR17, G-RUNTIME-PARITY, G-CONSUMER.
...
- **Publication Lifecycle** - An immutable subject progresses only through `built` → `evidence-candidate-published` → `evidence-validated` → `release-available` → `production-promoted`. `built` has no external authority. `evidence-candidate-published` requires the bounded Story 3.14 publication authority and exists only to collect evidence. `evidence-validated` requires the Story 3.15 validator and all three receipts to pass but still grants no release or deployment authority. `release-available` requires a separate authenticated release-owner record. `production-promoted` requires a separate authenticated deployment-owner record for each declared production profile. No state may be inferred or skipped, and any subject change restarts the lifecycle at `built`. Story 3.14 produces the corrective evidence candidate; Story 3.15 can establish `evidence-validated` but cannot itself authorize release or promotion.
- **Publication Authority Record** - For the current Story 3.15 subject, later-state authority must live at `_bmad-output/implementation-artifacts/evidence/publication-authority/aafe9040786c4f3af496b7ecbe62282c89396a15362b668a7b81ee148fe3f9c5/authority.json` and pass `python3 tools/validate-publication-authority.py _bmad-output/implementation-artifacts/evidence/publication-authority/aafe9040786c4f3af496b7ecbe62282c89396a15362b668a7b81ee148fe3f9c5/authority.json`. The versioned record must bind its schema, subject digest, source SHA, Story 3.14 packet and OCI/package identities, Story 3.15 validator/result/receipt-set digest, predecessor-state record digest, authenticated issuer identity and authentication-evidence digest, role-registry identity, state-specific outcome, issuance, expiry, revocation, and invalidation rules. A `release-available` entry requires the release-owner role. Each `production-promoted` entry additionally binds a profile from the record's complete production-profile inventory, immutable deployment identity, environment, and deployment-owner role. The validator must reject absent/unknown/duplicate/skipped states, incomplete profiles, unauthenticated or wrong-role issuers, expired/revoked records, predecessor or subject mismatch, and any authority whose bytes are not content-bound. The record and validator are currently absent, so no later state is authorized.
- **Canonical Production Profile Inventory** - AD-26 defines exactly one currently authorizing production profile: the versioned canonical bytes at `deploy/dapr/production-profile.yaml`. Its canonical-byte SHA-256, not a name or an authority-record self-declaration, is the complete production-profile inventory. The file is currently absent, so no profile is authorizing. Cosmos templates and Development/test profiles are explicitly excluded. Adding, replacing, or retiring an authorizing profile requires approved architecture change and a new canonical profile digest before the publication-authority validator, any promotion entry, or any consumer manifest can pass. The validator must compute the path's canonical-byte digest, reject an absent or under-declared path, and require every `production-promoted` and consumer entry to bind that exact digest.
...
- **MessageId Contract Version** - The endpoint or generated contract, never caller-controlled text, selects the accepted `MessageId` grammar. Version 2 is the canonical EventStore grammar: exactly 26 uppercase Crockford Base32 ULID characters (`0-9`, `A-H`, `J-N`, `P-T`, `V-Z`), accepted only when `Ulid.TryParse` succeeds and canonical round-trip text is byte-for-byte identical. Version 1 is the preserved legacy grammar: 1-128 ASCII alphanumeric or hyphen characters, with no leading or trailing hyphen, accepted only by endpoints or contracts that explicitly declare version 1 and preserved byte-for-byte. Both versions use `MessageId`, never `CorrelationId`, as the command-status selector. NFR12 governs version-1 deprecation or removal.
...
- **Owner Roles** - This PRD distinguishes six: the **EventStore owner**, who approves parity and EventStore evidence packets; the **release owner**, who authorizes `release-available`; the **deployment owner**, who authorizes `production-promoted`; the **consumer owner**, who authorizes removal of infrastructure from one bound consumer repository and commit; the **security approver**, who approves the payload-protection specification and G5; and the **product owner**, who owns scope and the forward backlog. One person may hold more than one role, so an approval by identity alone is not evidence of independent review, and an approval issued by the author of the work it approves satisfies the record but not the control. G-HIGH-RISK makes machine-verifiable non-authorship binding for every row classified high risk; today only Story 8.11 implements such a control, so the mandatory matrix and general control remain unimplemented and G-HIGH-RISK fails. OR10 owns that closure rather than weakening the requirement.
- **Parity** - Used in several senses, each with its own proof standard. **Source/package parity** - the EventStore libraries expose every capability a consumer needs, proven by Story 1.20's packet. **Deployed-runtime parity** - an evidence-candidate runtime demonstrably behaves as its packet claims, proven only by Story 3.15's three receipts; this is `evidence-validated`, not release or promotion authority. **Query parity** - a generic query path returns what a hand-written one did. **Topology parity** - AppHost, tests, and deploy templates assert the same posture. **Dual-provider parity** - two payload-protection providers interoperate byte-for-byte. **Tenant-filter parity** - admin queries filter by tenant identically across surfaces. Never treat evidence for one kind as evidence for another.
...
| FR12 | The REST API generator must discover command and query contracts and emit typed, OpenAPI-visible controllers that delegate to `IEventStoreGatewayClient` and forward canonical query metadata headers when the gateway supplies them. Every generated controller must enforce the canonical tenant boundary in NFR2 before routing or state access. The generator test suite must cover discovery, routing conventions, diagnostics, generated output, query metadata headers, `304`, safe problem-detail behavior, and compiled-controller tenant negatives. An accepted generated command may emit an absolute, gateway-authoritative command-status `Location` URI only when a `MessageId` valid under the endpoint's declared MessageId contract version selects the status resource. The endpoint or generated contract, not caller input, selects the version. `CorrelationId` remains diagnostic and must never select that resource. An absent, blank, invalid-for-version, unavailable, or version-ambiguous `MessageId` must omit `Location` or cause a safe rejection; it must never fall back to `CorrelationId` or emit a relative or dangling external-host URI. |
...
**Done evidence:** Generated controllers delegate through the gateway; Sample and Tenants UI hosts contain no generated or hand-written per-message MVC controllers; SignalR and optional DAPR notification paths prove scoped detail delivery and signal-only compatibility. Compiled and runtime controller tests must cover every NFR2 tenant case before downstream calls, including mixed-case normalization and rejection of missing, duplicate, conflicting-after-normalization, grammar-invalid, unauthorized, and reserved values. Command tests must prove that only a `MessageId` valid for the declared contract version can produce `Location`, including version-1 and version-2 positive cases and missing, invalid-for-version, caller-version-selection, version-ambiguous, and `MessageId`/`CorrelationId` divergence negatives. Current Stories 2.2, 2.4, 2.5, and 2.9 completion labels do not satisfy these corrective outcomes; Gate G-TENANT and Gate G-STATUS-ID remain failed until approved corrective stories or reopened stories pass.
...
| ID | Requirement |
| --- | --- |
| FR36 | Before a consuming module deletes local projection/query infrastructure, five independent outcomes must pass: EventStore source/package capability availability, deployed-runtime evidence validation, `release-available`, `production-promoted`, and per-consumer removal authorization. EventStore must produce an EventStore-owner-reviewed parity packet proving every required source/package capability through production paths and binding an exact runtime SHA. Story 3.15 must separately validate that deployed runtime through three packet-bound receipts. Release availability requires its own authenticated release-owner authority; production promotion requires a distinct authenticated deployment-owner authority for the same immutable subject and deployment profile. Evidence-candidate publication and evidence validation grant neither later authority. The consumer-removal manifest must enumerate every root-declared or Phase-4-referenced consumer, whether or not removal is proposed; a new consumer or removal proposal must register before code changes. Per-consumer removal requires an authenticated immutable consumer-owner receipt binding the consumer repository and commit, the applicable-mode matrix, the exact removal-subject digest, the role-registry identity, a `consumer-removal-authorized` outcome, issuance time, validity or expiry, and invalidation on any bound identity change or expiry. A manifest entry may be `N/A` only when no removal is proposed, a bound consumer diff proves no removal, and both the consumer owner and validator attest that fact; a removal proposal, missing entry, or unknown state never qualifies. The consumer's checked-out EventStore SHA and deployment profile must match the production-promoted authority. Missing or invalid consumer authority always prohibits removal even when platform capability, release, and promotion authority pass. |

**Done evidence:** FR36 carries independently failing capability, evidence-validation, release-availability, production-promotion, and consumer-removal states and is closed only when every applicable state is closed for the same immutable runtime and bound consumer subject.
...
- **Deployed-runtime parity - OPEN.** Story 3.13 records the immutable v3.94.1 candidate as rejected and non-authorizing. Story 3.14 produces a separately authorized corrective evidence candidate; Story 3.15 owns evidence-validation closure only. Story 3.15 is `in-progress` and its packet fails closed at 0 of 3 receipts, where the verifier exits 1 and grants nothing. Separate `release-available` and `production-promoted` authority records would still be required after evidence validation. The 2026-09-07 Epic 3 retrospective is `rejected` and records that FR36 is not closed.
- **Per-consumer removal authority - OPEN.** The required deterministic all-consumer manifest and validator do not exist. Parties is an applicable consumer with a removal proposal, but no authenticated consumer-owner receipt binds its repository/commit, deployment profile, exact removal subject, role-registry identity, validity, and `consumer-removal-authorized` outcome. Platform, runtime, release, or promotion evidence cannot substitute for this receipt.
...
| NFR2 | Tenant isolation must be preserved across state keys, actor IDs, topics, admin queries, generated REST APIs, SignalR groups, and deployment configuration. Every tenant-scoped request and `eventstore:tenant` grant must resolve exactly one explicit tenant through the shared canonicalizer, which performs invariant lowercase case normalization before comparison and authorization, then validates the 1-64-character grammar `^[a-z0-9]([a-z0-9-]*[a-z0-9])?$`. Mixed-case input is accepted only when this normalization yields the same single grammar-valid and authorized tenant. Missing, duplicate, conflicting-after-normalization, grammar-invalid, unauthorized, and reserved `system` tenant values must be rejected before routing, state access, status lookup, or resource-existence disclosure. Whitespace or other characters are not trimmed or repaired. Platform-wide operations use a distinct authenticated platform-operation scope; they must not synthesize a tenant. |
...
| FR12-C2 | corrective successor or reopened 2.2/2.4/2.5 | Compiled controllers canonicalize tenant case consistently and enforce every NFR2 rejection before downstream work; platform-wide operations do not synthesize `system`. |
| FR12-C3 | corrective successor or reopened 2.9 | Only a `MessageId` valid under the endpoint's declared contract version selects an absolute gateway-authoritative status `Location`; missing, invalid-for-version, or version-ambiguous identity omits it or rejects safely. |
...
| FR36-C1 | 1.20 | Source/package capability parity is owner-reviewed, production-path proven, and bound to one exact runtime SHA. |
| FR36-C2 | 3.15 | Deployed-runtime parity passes the exact three-receipt validator on one unchanged evidence candidate; candidate publication alone grants nothing. |
| FR36-C3 | corrective authority lane | Separate authenticated release-owner and deployment-owner records move the unchanged subject through `release-available` and `production-promoted`; neither state is inferred from evidence validation. |
| FR36-C4 | corrective consumer-authority lane | The deterministic manifest enumerates every applicable consumer/removal subject before code changes and rejects missing, unknown, or vacuous entries. |
| FR36-C5 | corrective consumer-authority lane | Each removal binds a valid consumer-owner receipt, repository/commit, mode matrix, removal digest, role registry, validity, and the same production-promoted runtime SHA; any bound change invalidates it. |
...
- Tenant access must be validated before status, idempotency, state, projection, admin, or generated REST data can disclose resource existence. Exactly one explicit tenant is lowercased by the shared canonicalizer and authorized under NFR2; missing, duplicate, conflicting-after-normalization, grammar-invalid, unauthorized, or reserved input fails before downstream work.
- A public command-status resource is selected only by a `MessageId` valid under the endpoint's declared contract version. `CorrelationId` remains diagnostic and cannot be substituted for status identity; callers cannot select the grammar version.
...
Phase 4 MVP delivery is total over FR1-FR36, NFR1-NFR18, and every stable §7.1 clause. Each item must have one primary owner, a reconciled lifecycle state, exact passing evidence, and approval in the G-MVP-COVERAGE manifest; a story-level `done`, an omitted item, or a passed subset cannot establish MVP completion.
...
Every metric is marked **achieved**, **partially met**, or **not met**. An achieved metric is retained for the record but no longer discriminates between success and failure for the remaining epics. The G-MVP-COVERAGE manifest owns the enumerable host, surface, and high-tier-evidence denominators used by SM9, SM10, and SM12; absence of that manifest keeps those metrics not met.
...
- **SM3 (not met; historical 2026-08-01 achievement invalidated):** High-risk NFRs NFR1-NFR4, NFR7, NFR10-NFR11, and NFR14-NFR17 must map to complete clause-level primary coverage. §11.2 carries declarations, but the corrected NFR2 tenant contract and NFR3 all-host contract lack approved corrective ownership, and NFR17-C5 is unassigned. Historical story mappings cannot satisfy the expanded, architecture-aligned outcomes.
...
- **SM2 (partially met):** MVP FR1-FR36 and committed post-MVP FR37 retain epic coverage, but §7.1 and §11 expose corrective or unassigned slices for the tenant boundary, status identity, NFR5, NFR12, NFR13, FR36-C3, FR36-C4, FR36-C5, NFR17-C5, and per-consumer removal authority. The 2026-09-08 OR12 repair remains historical; it does not close these later findings. SM2 cannot pass until `epics.md` owns every slice, the drift/closure guard passes, and the PRD, architecture, detailed UX, and epics are substantively reconciled and approved under OR8 and OR14.
- **SM4 (achieved):** The oversized stories identified by readiness review are decomposed per `_bmad-output/planning-artifacts/story-id-migration-2026-08-01.md`. Story 4.8 is now a non-executable evidence ledger decomposed into Stories 4.9-4.15 and must never carry an execution status. Stories 7.14 and 8.2 retain their numbers as the first focused child of their own decompositions - 7.14 as the Admin shell and canonical-route boundary, 8.2 as payload-protection contracts and golden vectors - with siblings 7.19-7.20 and 8.3-8.11. Both remain `backlog`, which is decomposition, not completion.
...
- **SM7 (not met; post-MVP):** Target: Story 8.11 records the payload-protection G5 packet as owner/security-approved `available`, exact source/package/backend identities are recorded, EventStore goldens and Parties dual-provider parity pass, and Story 8.10 rollback succeeds after `pdenc-v2` writes before Parties Story 8.7 resumes. Current G5 is `needs-additive-api`; no target outcome is authorized. Validates FR37/NFR19 without entering the Phase 4 MVP aggregate.
...
- **SM8 (not met; FR9, reuse):** Duplicated platform plumbing remaining in the Tenants domain module - request routers, projection actors, cursor codecs, state-store plumbing, telemetry registration, health checks, and per-domain Aspire wiring. Target zero. Currently non-zero: the Tenants domain-service host retains transitional `AddDaprClient`, `UseCloudEvents`, controller, MediatR, and router composition (DW-64).
- **SM9 (not met; FR13, NFR14, reuse):** Target: zero interactive UI hosts in the G-MVP-COVERAGE `interactive_ui_hosts` inventory contain generated or hand-written per-message MVC command/query controllers. The numerator is the count of inventory entries whose bound guardrail result detects one or more such controllers; the inventory or any result is currently absent or unapproved, so the metric cannot pass.
- **SM10 (not met; NFR1, hardening):** Target: 100% of entries in the G-MVP-COVERAGE `nfr1_surfaces` inventory carry at least one bound negative test proving anonymous or forged-flag access is rejected, with health/liveness/readiness probes recorded as the single pinned anonymous exception. The complete inventory and coverage fraction are not approved.
- **SM11 (not met; NFR7, hardening):** Silent-loss classes with an implemented guard or recovery that prevents loss in the supported operating envelope and is proven through production paths. Target five of five; deferral paperwork and scope exclusion never count. Currently four of five are delivered: class (a) staged-state flush loss is guarded by Story 5.1; class (b) stale pipeline records are guarded by Story 4.2; class (d) committed-but-unpublished events are recovered by Story 4.4; and class (e) duplicate side effects has implementation and closed source-only platform evidence from Stories 4.9-4.15. Class (c) has a reproduced `same-key-overwrite-raw-durable-write-lost` outcome and no fence or approved enforced operating envelope. Story 4.15 grants no broader readiness or external authority. See OR4.
- **SM12 (not met; NFR16, hardening):** Target: 100% of entries in the G-MVP-COVERAGE `high_tier_evidence_cases` inventory assert persisted state-store, read-model, marker, lifecycle, or checkpoint end-state rather than only HTTP status or mock call counts. The complete inventory and coverage fraction are not approved.
...
| FR36 | Epic 1 - projection/query parity source/package closure; Epic 3 - deployed-runtime parity closure; corrective release/promotion and consumer authority | FR36-C1: 1.20 source/package slice; FR36-C2: 3.15 deployed-runtime slice; FR36-C3: unassigned release/promotion authority slice; FR36-C4 and FR36-C5: unassigned consumer-manifest and per-consumer receipt slices |
...
Each row classifies what the story's own `epics.md` section declares. A declaration records ownership, not delivery. A literal range endpoint is distinguished from an interior ID inferred from range notation; inferred interiors are not counted as individual declarations. Semantic gaps remain blocking even where an older, narrower NFR has a primary owner.
...
| NFR2 | 5.2, 5.5-5.8, 5.10, 7.2 | 1.5, 1.9, 1.10, 1.14, 2.5, 3.10, 7.1, 7.3, 7.4, 7.19, 8.7 | Range-only interiors: 8.1, 8.5, 8.9, 8.11. The corrected complete tenant contract lacks an approved primary slice; blocking semantic gap. |
| NFR3 | 5.3 | 8.3, 8.6 | Range-only interiors: 8.1, 8.5, 8.9, 8.11. Story 5.3 does not own Tenants, generated-host, or future-host conformance; blocking semantic gap. |
| NFR4 | 5.3, 7.6 | 5.4, 8.1, 8.5, 8.6, 8.9, 8.11 | Declared; 8.1, 8.5, 8.9, and 8.11 are literal range endpoints. |
| NFR5 | **Unassigned** | 7.1 | Primary owner absent; blocking. Story 2.8 does not declare NFR5. |
...
| NFR12 | 1.17, 3.13, 3.15 | 1.20, 2.7, 2.8, 2.12, 3.16, 6.1-6.6, 7.5, 8.1, 8.2, 8.4, 8.9, 8.10, 8.11 | Existing declarations do not cover complete, disjoint primary slices for the expanded public-surface inventory; blocking semantic gap. 8.1 and 8.11 are literal range endpoints. |
| NFR13 | **Unassigned** | 2.4, 2.9 | Primary owner absent; blocking. Story 2.2 is a candidate generator-build owner but does not declare NFR13. |
...
The PRD, architecture, UX, epics, sprint status, and implementation evidence exist, but they do not form one clean, reviewed, approved baseline. The supplied validation examined historical `HEAD` `293c69c42d35dee26682d42f05c943c0b65786f4`; the finalize review examined later `HEAD` `dd55a6d1e128989777ae6c47459da0f746e84e53` plus this PRD worktree. Raw digest equality is integrity evidence only; it never proves substantive reconciliation, owner approval, or an atomic baseline. Implementation readiness stays blocked until §12 and every mandatory row in §11.4 pass. A readiness re-run is the final step, not a substitute for those corrections.
...
| `ux.md` | SHA-256 `2927f97d4fe7262ec5886084a974d9d5c21240560e6563ab84f84b90e4dd670a`; top-level status `final`. | `epics.md` matches this top-level digest, but the detailed DESIGN and EXPERIENCE artifacts both have `status: draft` and their recorded digests are stale: DESIGN recorded `76be269737ddacb5d5680862590893cd4e69d469222fd1eba554aa4084339a4f`, current `3f4f0181ea24b5ed7544b6cb8482cd73b1aadba7ddbef47fd135e080a2d8365d`; EXPERIENCE recorded `06de15b349224772146fa279f9a61ca6c0fffe9d17774588cedd330690042eb6`, current `11f754031cb5f7f8c376787573c32ba9ac0c68c4718e6b4b86281f579e8c9cab`. EXPERIENCE also retains two unconfirmed assumptions: restore/import remain hidden, and tenant provisioning remains absent from Tenants & Access. | UI handoff is not one content-bound approved source set; those assumptions cannot authorize UI scope or behavior. |
...
| Story 4.5 append-race evidence | Tracker and epics say `done`, while the current story acceptance requires `in-progress` when its packet fails source binding or runtime attribution; the current spec retains unresolved review items and reports source drift. | The historical race observation remains valid input for FR31, but current packet validation and NFR7/NFR16 closure are not established. | G-BASELINE and OR15 fail; the observation cannot authorize fencing, NFR7/NFR16 closure, release, or readiness. |
| `sprint-status.yaml` and story wrappers | Current `HEAD` plus this PRD worktree. | 3.15: tracker `in-progress` and names stale subject `a5c07d178412d8fbac72ec660a3c0a94826a823f7376c61e0e7b98ea554c3448`, while the spec and packet bind `aafe9040786c4f3af496b7ecbe62282c89396a15362b668a7b81ee148fe3f9c5`; 4.15: tracker, wrapper, epics, and bounded lifecycle record agree on `done`; 5.2: tracker `done`, wrapper `in-review`, epics `backlog`; 5.3: wrapper and committed tracker `done`, epics stale; 5.4: tracker `review`, wrapper `in-progress`, epics `backlog`; 6.1: tracker `review`, wrapper `done`, canonical spec `approved-authorized`, epics `backlog` and says absent. | Story 4.15's former conflict is resolved; the remaining conflicting lifecycle values or stale evidence subjects cannot supply delivery or gate evidence. |
| OQ8 evidence | Story 4.15 v4 default, closed, and historical validators pass over a fresh source-only subject while v3 remains byte-identical. The external governing design bytes are still unavailable inside EventStore and no approved normative projection or content-addressed Folders attestation is present. | Story 4.15's `eventStorePlatformComplete: true` is a bounded source-only packet claim; it grants no release, deployment, runtime-pin, consumer-migration, Folders-closure, or readiness authority. | G-OQ8 still fails on external design authority and OR11 remains open, independently of the now-consistent lifecycle. |
| Story 3.15 deployed-runtime parity | Packet `_bmad-output/implementation-artifacts/evidence/story-3-15/f343bb0153e9cdcb8b12ec10153813072f5ad38d/closure.json`; current subject SHA-256 `aafe9040786c4f3af496b7ecbe62282c89396a15362b668a7b81ee148fe3f9c5`; 0 of 3 packet-bound receipts. | The exact validator command in G-RUNTIME-PARITY exits 1 with `exactly three packet-bound receipts are required`; all mutation, publication, deployment, and consumer-removal authority flags remain false. | Deployed-runtime parity is unavailable; blocks FR36, SM6, G-CONSUMER, release, deployment, migration, and readiness. |

- Stories 1.20 and 3.12 remain `done` and are not reopened by the deployed-runtime correction. Story 1.20 remains the completed source/package parity gate. Story 3.13 records the immutable v3.94.1 candidate as rejected and non-authorizing because its config provenance is malformed and its retained authority forbids deployment. Story 3.14 produces the corrective `evidence-candidate-published` subject; Story 3.15 can establish only `evidence-validated`. Neither result reopens Story 1.20 or authorizes `release-available`, `production-promoted`, Parties 8.6, G5, deployment, or consumer migration. This planning update authorizes no release, deployment, Git, or submodule mutation; each later publication-lifecycle transition requires its own durable authority record.
...
- Story 5.3 reached `done` at review loop 8 in its wrapper, and the committed tracker now also says `done`; the `epics.md` narrative remains stale. This alignment does not establish an approved atomic baseline and cannot close NFR3 for Tenants, generated hosts, or future JWT-binding hosts.
...
| --- | --- | --- | --- | --- | --- |
| G-BASELINE | OR5, OR8, OR14, OR15, OR28; SM2 | One manifest binding final PRD, architecture, detailed UX, epics, sprint status, story wrappers, relevant evidence digests, and the corrective-work authorization registry after substantive reconciliation; a guard must compare FR/NFR text, §7.1/§11 ownership, artifact statuses, lifecycle values, corrective-work preflight/postflight results, and require every authorization to be closed or revoked before readiness. The exact baseline validator lane/command is not yet defined. | Product, architecture, epic, UX, test, and tracker owners; approval must follow validation of the same manifest. | **FAIL/BLOCKED.** Architecture and detailed UX remain draft with two unconfirmed UX assumptions, detailed digests are stale, this PRD now differs from the epics input digest, lifecycle values conflict, and the baseline and corrective-authorization guards are absent. Blocks every general downstream handoff; only a separately passing corrective-work authorization may open its bounded lane. | None. Any bound byte, status, mapping, authorization, or approval change invalidates the row. |
| G-OQ8 | FR27; NFR7, NFR16; OR11 | Available governing bytes by permitted immutable copy, complete approved normative projection, or signed/content-addressed Folders attestation, binding repository, path, commit, and SHA-256 in PRD, architecture, epics, packet, validator, and CI; the active Story 4.15 source-only packet must pass default validation. | Architecture, Security, Test, EventStore, and Folders content owners; non-authorship control from G-HIGH-RISK. | **FAIL/BLOCKED.** Story 4.15 v4 source-only validation and lifecycle closure pass, but governing design bytes remain unavailable inside EventStore and no approved normative projection or signed/content-addressed Folders attestation exists. Blocks broader OQ8 authority, readiness, release, and consumer handoff without reopening the bounded Story 4.15 result. | None. Any design identity, projected byte, public-document subject, packet, validator, or approval change invalidates the row. |
| G-TENANT | FR12-C2, FR15; NFR2; OR20 | Approved corrective or reopened story; compiled generated-controller and Tenants runtime tests for mixed-case request/grant normalization plus missing, duplicate, conflicting-after-normalization, grammar-invalid, unauthorized, and reserved `system` inputs, proving rejection where required before routing/state access and proving platform operations use distinct authenticated scope. Exact CI lane/command must be bound by that story. | EventStore, Tenants, Security, and Test owners with independent approval. | **FAIL/BLOCKED.** Current generated and Tenants paths return or route synthetic/raw tenant values while affected stories are labelled `done`. Blocks tenant-safety, readiness, release, and deployment claims. | None. Generator, canonicalizer, Tenants host, route, test, or approval changes invalidate the row. |
| G-STATUS-ID | FR12-C3, FR12-C4; AD-17/AD-32; OR21 | Approved corrective or reopened Story 2.9; a versioned contract manifest assigning MessageId grammar v1 or v2 to every affected endpoint/generated contract; compiled and runtime positive and negative tests for both grammars, undeclared/ambiguous/caller-selected versions, missing, blank, invalid-for-version, noncanonical-v2, and divergent `MessageId`/`CorrelationId`, proving only `MessageId` can select status `Location`. Exact CI lane/command must be bound by that story. | EventStore API and Test owners with independent approval. | **FAIL/BLOCKED.** Current public documentation and implementation do not bind one version per surface, and generator/tests pin a `MessageId ?? CorrelationId` fallback while Story 2.9 is labelled `done`. Blocks FR12, readiness, and external API release. | None. Version assignment, grammar, status route, identity builder, generator, tests, or approval changes invalidate the row. |
...
| G-CLAUSE | FR4, FR5, FR7, FR12, FR26, FR33, FR34, FR36, NFR17; OR7 | §7.1 must be mirrored in `epics.md`; each clause must bind a primary slice and exact passing evidence; an automated all-clauses-required validator must reject missing, duplicate, unsupported, or status-only closure. Exact validator lane/command is not yet defined. | Product, epic, architecture, and Test owners. | **FAIL/BLOCKED.** The PRD ledger now exists, but downstream clause ownership, evidence identities, and the validator are not reconciled; FR36-C3, FR36-C4, FR36-C5, and NFR17-C5 lack assigned corrective primary stories. Blocks parent-requirement closure and readiness. | None. Clause text, mapping, story, evidence, or validator changes invalidate the row. |
...
| G-AUTH-HOSTS | NFR1-NFR3; OR23 | One versioned JWT contract/fingerprint consumed by every externally reachable or JWT-binding host, including Tenants and generated-host fixtures, with negative Production/break-glass/algorithm/issuer/audience/signature/lifetime/role/tenant tests and exact release evidence. | Security, EventStore, Tenants, release, and Test owners with independent approval. | **FAIL/BLOCKED.** Story 5.3 is `done` for its bounded hosts, but Tenants hand-rolls a weaker subset and no all-host/future-host conformance gate exists. Blocks NFR3, readiness, release, and deployment. | None. Contract, host inventory, fingerprint, host code, tests, or approval changes invalidate the row. |
| G-RUNTIME-PARITY | FR36-C2; SM6; OR15 | `python3 tools/validate-corrected-deployed-runtime-parity.py _bmad-output/implementation-artifacts/evidence/story-3-15/f343bb0153e9cdcb8b12ec10153813072f5ad38d/closure.json --packet-root _bmad-output/implementation-artifacts/evidence/story-3-15/f343bb0153e9cdcb8b12ec10153813072f5ad38d` must pass with exactly three valid, packet-bound receipts over current subject `aafe9040786c4f3af496b7ecbe62282c89396a15362b668a7b81ee148fe3f9c5`; the tracker, spec, packet, role registry, and receipt directory must name that same subject. | EventStore owner, Test Architect, and release owner exactly as enumerated by the packet's role registry, subject to G-HIGH-RISK non-authorship control. A passing result establishes only `evidence-validated`; it does not establish `release-available` or `production-promoted`. | **FAIL/BLOCKED.** The command exits 1 with `exactly three packet-bound receipts are required`; receipt count is 0 of 3, tracker subject is stale, and authority flags remain false. Blocks FR36, SM6, G-CONSUMER, readiness, release, deployment, and migration. | None. Any source, package, OCI, Production-smoke, inventory, registry, verifier, decision, receipt policy, subject, or receipt change invalidates the row and requires a complete remint. |
| G-PUBLICATION-AUTH | FR36-C3; OR29 | The exact current-subject authority record and command in the Publication Authority Record glossary entry must pass. The validator must compute the canonical-byte SHA-256 of AD-26's sole authorizing `deploy/dapr/production-profile.yaml`, require that digest as the complete production-profile inventory, and reject absent, under-declared, unknown, or self-only profiles. The authority record must contain one valid predecessor-bound state chain, a release-owner-authenticated `release-available` entry, and a deployment-owner-authenticated `production-promoted` entry binding that canonical profile digest and an immutable deployment identity. | Release owner, deployment owner, Security, and Test owners with non-authorship control from G-HIGH-RISK. | **FAIL/BLOCKED.** The canonical production-profile file, versioned authority record, and validator do not exist; no release availability or production promotion is authorized. Blocks G-CONSUMER, readiness, release, deployment, migration, and removal. | None. Subject, source, package/OCI identity, Story 3.15 result or receipts, canonical profile bytes/digest, deployment identity, issuer/authentication, role registry, predecessor state, time validity, revocation, record, or validator changes invalidate the row. |
| G-CONSUMER | FR36-C4, FR36-C5; AD-22; OR24 | Passing G-RUNTIME-PARITY and G-PUBLICATION-AUTH, plus `_bmad-output/implementation-artifacts/evidence/consumer-removal-manifest.json` validated by `python3 tools/validate-consumer-removal-authority.py _bmad-output/implementation-artifacts/evidence/consumer-removal-manifest.json`. The manifest universe is every root-declared or Phase-4-referenced consumer, whether or not removal is proposed; a new consumer or proposal must register before code changes. Each entry binds repository/commit, the exact canonical `deploy/dapr/production-profile.yaml` digest, mode matrix, removal-subject digest, role-registry identity, consumer-owner receipt, decision, issuance, validity, and invalidation. `PASS` requires a valid `consumer-removal-authorized` receipt. `N/A` is allowed only when no removal is proposed, the bound consumer diff proves no removal, and both the consumer owner and validator attest that fact; a proposal can never be `N/A`, and missing, unknown, under-declared, or self-only profile entries fail. | EventStore owner, release owner, deployment owner, and each bound consumer owner; non-authorship control from G-HIGH-RISK. | **FAIL/BLOCKED.** Runtime parity, canonical production profile, release availability, and production promotion are unauthorized; the consumer manifest and validator are absent. Parties has an applicable removal proposal but no valid per-consumer authority, so it fails rather than receiving `N/A`. No consumer may remove local infrastructure. | None. Any bound source, package, runtime, publication state, consumer universe, canonical profile bytes/digest, mode, removal subject, role registry, receipt, decision, validator, or expiry change invalidates the applicable authority. |
| G-HIGH-RISK | Every mandatory gate; NFR7, NFR12, NFR16; OR10, OR13 | `_bmad-output/implementation-artifacts/evidence/phase-4-gate-risk-matrix.json` must enumerate G-BASELINE, G-OQ8, G-TENANT, G-STATUS-ID, G-APPEND, G-CLAUSE, G-NFR8, G-COMPAT, G-AUTH-HOSTS, G-RUNTIME-PARITY, G-PUBLICATION-AUTH, G-CONSUMER, G-HIGH-RISK, G-NFR18, G-NFR-OWNERSHIP, G-MVP-COVERAGE, and G-READINESS. `python3 tools/validate-phase-4-gate-risk-matrix.py _bmad-output/implementation-artifacts/evidence/phase-4-gate-risk-matrix.json` must reject omissions, unknown gates, and new unclassified gates. Each entry is `high-risk` or a reason-coded `standard-control`; standard classification is not omission or waiver. Every high-risk entry requires both an authenticated second identity independent from the author and sealed CI validation, and binds its exact command, trigger, subject/evidence identities, pass condition, author/evaluator roles, non-authorship result, and guarded transition. | Product, EventStore, Security where applicable, and Test owners; both controls must be machine-verifiable for each high-risk row. | **FAIL/BLOCKED.** The matrix, validator, classifications, second-identity controls, sealed validation, and guarded transitions are absent. Blocks high-risk evidence from authorizing readiness, release, deployment, or migration. | None. Gate inventory, classification, reason, subject, validator, identity, roster, result, or status changes invalidate the matrix and affected row. |
...
| G-NFR-OWNERSHIP | NFR2, NFR3, NFR5, NFR12, NFR13, NFR17-C5, NFR18; OR6, OR20, OR22, OR23, OR25, OR26 | Each requirement or semantic correction must have complete disjoint primary ownership, an owning story/successor, exact evidence and validator, and an all-owners-covered result in the MVP coverage manifest. | Product, architecture, epic, and Test owners plus capability owners named by the corrective story. | **FAIL/BLOCKED.** NFR5, NFR13, NFR17-C5, and NFR18 have no complete primary owner; NFR2, NFR3, and NFR12 have incomplete semantic ownership. Blocks readiness and MVP completion. | None. Requirement, ownership, story, evidence, or validator changes invalidate the row. |
| G-MVP-COVERAGE | FR1-FR36, NFR1-NFR18; OR27 | `_bmad-output/implementation-artifacts/evidence/phase-4-mvp-coverage.json` must enumerate every MVP FR/NFR and stable §7.1 clause with primary owner, lifecycle state, exact evidence digest, validator command/result, approval, and the mandatory gate that blocks it when not passed. It must also enumerate `interactive_ui_hosts`, `nfr1_surfaces`, and `high_tier_evidence_cases` so SM9, SM10, and SM12 have closed denominators. `python3 tools/validate-phase-4-mvp-coverage.py _bmad-output/implementation-artifacts/evidence/phase-4-mvp-coverage.json` must reject omissions, duplicate ownership, unsupported `done`, stale identities, unapproved or failed evidence, and incomplete metric inventories. `N/A` is forbidden for MVP IDs; non-runtime requirements pass only through their specified approved artifact evidence. | Product, epic, architecture, and Test owners with independent approval of the same manifest digest. | **FAIL/BLOCKED.** The manifest and validator do not exist, several FR/NFR outcomes are open, metric inventories are absent, and multiple mandatory gates fail. Blocks Phase 4 MVP completion and prevents G-READINESS from asserting total coverage. | None. Any MVP requirement, clause, story, lifecycle, evidence, validator, gate, metric inventory, or approval change invalidates the row. |
| G-READINESS | OR1 | A new persisted implementation-readiness report that consumes the single approved baseline manifest, the complete MVP coverage manifest, and all passing mandatory rows; it must explicitly compute the aggregate and bind its validator/evaluator identity. | Product owner after all preceding evaluators approve the same source set. | **FAIL/BLOCKED — aggregate `Reject`.** Every mandatory row above must pass first. The 2026-08-01 `READY` report is historical and non-authorizing. | None. Any mandatory-row invalidation immediately returns the aggregate to `Reject`. |
...
Epic 8 remains committed post-MVP and is not pulled into the Phase 4 MVP aggregate. This update does reopen or add corrective ownership where current implementation contradicts architecture-aligned requirements: tenant routing, public status identity, all-host JWT conformance, compatibility enforcement, clause closure, and consumer-removal authority. Product-policy, acceptance, authority, and cross-artifact questions remain open and are recorded as blockers instead of inferred answers. The remaining payload-protection design decisions stay gated by Story 8.1 and the approved security specification listed in §11.3.
...
| --- | --- | --- | --- |
| OR4 | **Blocking.** Resolve NFR7 class (c) and DW-326 through an approved tracker story owned by the EventStore persistence owner. Deliver and prove provider-portable append fencing, or mechanically enforce and prove a supported operating envelope in which a second writer cannot enter. Moving fencing into MVP implementation requires approved scope change. Risk acceptance or deferral alone cannot satisfy NFR7, SM11, or G-APPEND. | Product owner with architecture, EventStore persistence, and test owners | Before Phase 4 MVP completion, readiness `READY`, release, or deployment |
| OR10 | **Blocking.** Implement the binding G-HIGH-RISK non-authorship control: every high-risk row requires both an authenticated second identity independent from the author and sealed CI validation. The versioned matrix and validator must prove both controls and reject self-approval, identity aliasing, missing seals, omitted gates, and reasonless classifications. | Product owner with EventStore, security where applicable, and test owners | Before any high-risk evidence is used to authorize `READY`, release, deployment, or migration |
| OR11 | **Blocking.** Make the OQ8 design authority reproducible inside the EventStore evidence boundary using a permitted immutable copy, a complete approved normative projection, or a signed or content-addressed Folders attestation. Story 4.15 v4 closes the bounded EventStore source-only lifecycle but deliberately does not provide those external governing bytes or authority; absence, mismatch, or red validation of that future authority fails G-OQ8. | Architecture owner with Folders content, EventStore, and test owners | Before broader OQ8 authority, readiness `READY`, release, or consumer handoff |
...
| OR14 | **Blocking.** Perform substantive PRD-to-architecture-to-UX-to-epics reconciliation, then obtain approval of one content-bound manifest. The pre-update PRD/architecture digest match resulted from a prohibited hash-only refresh and did not confer approval; this PRD update makes the epics PRD digest stale again, and the draft detailed DESIGN/EXPERIENCE digests already differ. Architecture must add the Tenants boundary to AD-10 and align AD-11/AD-26 to the PRD's `built` → `evidence-candidate-published` → `evidence-validated` → `release-available` → `production-promoted` vocabulary; detailed UX must resolve its two open assumptions; architecture must complete reviewer closure and obtain explicit owner ratification of AD-26 or an approved replacement before epics renewal. Only then may input digests be refreshed. | Architecture, UX, epic, test, and product owners | Before downstream handoff or the next readiness re-run |
| OR15 | **Blocking.** Resolve the remaining lifecycle contradictions for Stories 3.15, 4.5, 5.2, 5.3, 5.4, and 6.1 across committed `sprint-status.yaml`, story wrappers/evidence, and `epics.md`. Story 3.15 must replace tracker subject `a5c07d178412d8fbac72ec660a3c0a94826a823f7376c61e0e7b98ea554c3448` with packet/spec subject `aafe9040786c4f3af496b7ecbe62282c89396a15362b668a7b81ee148fe3f9c5` and retain `in-progress` until G-RUNTIME-PARITY passes. Story 4.5 must separate historical FR31 race-observation completion from current-packet validation: its current acceptance says packet drift makes it `in-progress`, so tracker/epics `done` cannot supply NFR7/NFR16 evidence. Story 5.3's tracker/wrapper are now `done`, but epics remains stale. Story 5.4 now has wrapper `in-progress`, tracker `review`, and epics `backlog`; none supplies terminal evidence. Story 4.15 is resolved by its bounded v4 lifecycle record and closed default probe; its source-only completion flag still grants no external authority. Add guarded subject and status-transition validation for the remaining sources so they cannot diverge silently again. | Story owners with tracker and test owners | Before affected FR/NFR delivery claims or the next readiness re-run |
...
| # | Owed refinement | Owner | Trigger |
| --- | --- | --- | --- |
| OR7 | **Blocking.** §7.1 now defines stable subclauses and all-clauses-required closure for FR4, FR5, FR7, FR12, FR26, FR33, FR34, FR36, and NFR17. Reconcile those clauses into `epics.md`, assign FR36-C3, FR36-C4, FR36-C5, and NFR17-C5, bind exact evidence identities, and add the automated validator required by G-CLAUSE. PRD text alone does not close this refinement. | Product owner with epic/story and test owners | Before the next readiness re-run |
| OR17 | **Blocking.** §11.4 now defines the Phase 4 MVP exit decision and separates post-MVP G5. Complete every missing exact lane/command, evidence identity, and approval control recorded in its mandatory rows, and retain their current failed results until proof passes. | Product owner with every named gate evaluator | Before the next readiness re-run |
| OR20 | **Blocking.** Reopen Stories 2.2/2.4/2.5 or approve corrective successors that implement NFR2's shared lowercase canonicalization, post-normalization grammar/authorization checks, and distinct authenticated platform-operation scope, then bind compiled/runtime mixed-case positives and fail-closed negatives to G-TENANT. Existing `done` labels are non-authorizing for the corrected contract. | EventStore, Tenants, security, epic, and test owners | Before tenant-safety, readiness, release, or deployment claims |
| OR21 | **Blocking.** Reopen Story 2.9 or approve a corrective successor that assigns MessageId grammar v1 or v2 to every affected endpoint/generated contract, implements the glossary's exact grammars without caller-controlled version selection, makes valid-for-version `MessageId` the sole public command-status selector, preserves explicit v1 compatibility under NFR12, and removes every `CorrelationId` fallback. Bind the version manifest and compiled/runtime evidence to G-STATUS-ID. | EventStore API, epic, consumer, and test owners | Before FR12 closure, readiness, or external API release |
| OR22 | **Blocking.** Inventory every NFR12 public surface, establish source/binary API and wire baselines, define enforced SemVer/deprecation/removal policy, add representative package-only consumers, and bind the exact release validation lane and evidence to G-COMPAT. | EventStore API, release, consumer, and test owners | Before compatibility, readiness, or release claims |
| OR23 | **Blocking.** Replace host-local JWT subsets with one shared versioned contract/fingerprint for EventStore, Admin, Sample, Tenants, generated-host fixtures, and future JWT-binding hosts; add all-host conformance evidence to G-AUTH-HOSTS. Story 5.3 closure alone is insufficient. | Security, EventStore, Tenants, release, and test owners | Before NFR3, readiness, release, or deployment claims |
| OR24 | **Blocking.** Assign and implement the FR36 per-consumer authority slice and its deterministic universe. Create `_bmad-output/implementation-artifacts/evidence/consumer-removal-manifest.json` and `tools/validate-consumer-removal-authority.py`; enumerate every root-declared or Phase-4-referenced consumer whether or not removal is proposed, and register any new consumer or proposal before code changes; bind repository/commit, deployment profile, mode matrix, removal-subject digest, authenticated consumer-owner receipt and role registry, authorization outcome, validity, and invalidation; enforce the G-CONSUMER `PASS`/`N/A` rules. Parties is applicable and currently failed. Platform, runtime, release, and promotion parity cannot substitute. | EventStore, release, deployment, each consumer owner, and test owner | Before any consumer removes local projection/query infrastructure |
| OR25 | **Blocking.** Assign NFR5 one disjoint primary story or approved corrective successor for its existing SignalR detail-metadata contract only. The story must prove the configured maximum of 16 entries, 2048 total UTF-8 bytes, opaque-key treatment, and suppression of metadata values above Debug log level, bind the exact validation command, and appear in both `epics.md` and the G-MVP-COVERAGE manifest. This item does not apply NFR5 to DAPR, generated APIs, or other response types. | Product owner with SignalR transport, epic, and test owners | Before NFR5 coverage, Phase 4 MVP completion, or the next readiness re-run |
| OR26 | **Blocking.** Assign NFR13 one disjoint primary story or approved corrective successor for its existing build-quality contract only. The story must prove that generated code and source-generator packages build under warnings-as-errors and conform to EventStore code style, nullable, ULID, and `ConfigureAwait(false)` rules, bind the exact validation command, and appear in both `epics.md` and the G-MVP-COVERAGE manifest. Deterministic or byte-stable generator output is not added by this refinement. | Product owner with REST generator, epic, and test owners | Before NFR13 coverage, Phase 4 MVP completion, or the next readiness re-run |
| OR27 | **Blocking.** Create `_bmad-output/implementation-artifacts/evidence/phase-4-mvp-coverage.json` and `tools/validate-phase-4-mvp-coverage.py` exactly as specified by G-MVP-COVERAGE. Populate every FR1-FR36, NFR1-NFR18, stable §7.1 clause, and the already-defined SM9/SM10/SM12 denominators from reconciled lifecycle and evidence sources; omissions, duplicate owners, unsupported `done`, failed evidence, stale identity, or absent approval must keep the aggregate failed. | Product owner with epic, architecture, tracker, and test owners | Before Phase 4 MVP completion or the next readiness re-run |
| OR28 | **Blocking.** Implement and owner-approve the content-bound corrective-work authorization record and validator defined in §0. The validator must fail before handoff on missing, expired, revoked, mismatched, overbroad, or out-of-path authority; validate the resulting diff and evidence postflight; and make G-BASELINE reject any active, invalid, or unreconciled authorization. Until this exists, no gate-closing implementation handoff is authorized by the §0 exception. | Product owner with architecture, tracker, security where applicable, and test owners | Before any corrective implementation handoff and before the next readiness re-run |
| OR29 | **Blocking.** Implement and independently approve the Publication Authority Record schema, complete production-profile inventory, and exact validator defined in the glossary and G-PUBLICATION-AUTH. Bind authenticated release/deployment issuers, state predecessors, subject/package/OCI/receipt identities, profile-specific immutable deployment identities, issuance, expiry, revocation, and invalidation. Existing candidate-publication evidence cannot be relabeled as release availability or production promotion. | Release owner with deployment, security, architecture, and test owners | Before `release-available`, `production-promoted`, G-CONSUMER, or the next readiness re-run |

**Final readiness gate**

| # | Owed refinement | Owner | Trigger |
| --- | --- | --- | --- |
| OR1 | **Blocking.** Re-run implementation readiness only after every other blocking refinement and every mandatory §11.4 gate passes against one approved baseline. The 2026-08-01 `READY` verdict is historical; the 2026-09-10 reconciliation result remains `Poor` / `Reject`. | Product owner | After all blocking refinements close; also whenever FR/NFR text, a bound artifact/evidence identity, lifecycle state, approval, retrospective, or scope changes |
...
Identifiers OR2, OR3, and OR12 are retired, not reused. OR20-OR29 are the corrective blockers introduced by the 2026-09-10 review; existing identifiers remain unchanged so prior references continue to resolve.
...
- **OR2 (Story 5.3).** The 2026-09-08 correction retired the then-observed tracker error; later work changed the facts again. On 2026-09-10 the story spec is `done` at review loop 8 and the committed sprint tracker is now `done`, while `epics.md` and the former PRD narrative still describe loop-3 partial delivery. OR15 owns the remaining epics drift. Even after those sources align, Story 5.3 covers only its bound hosts and cannot close the broader NFR3 all-host contract; that separate gap is OR23.
- **OR3 (Stories 4.5, 4.6, 5.1).** The 2026-09-08 correction was resolved per story, not per source. Story 5.1 is genuinely `done` at review loop 7 with sealed, test-backed evidence. Story 4.5's historical FR31 race observation was accepted at review loop 4, but later acceptance text requires `in-progress` whenever its current packet fails source binding or runtime attribution; current tracker/epics `done` is therefore non-authorizing and the renewed conflict is owned by OR15. Story 4.6 is not done: its spec records `approval_state: absent` and `implementation_authorized: false` with three outstanding operator actions, so the tracker row flipped by commit `8d6f7dac` was corrected back to `awaiting-operator`. NFR7 class (a) is confirmed guarded (§7, SM11); class (c) is unchanged because the Story 4.5 observation grants no fencing, NFR7/NFR16, or readiness authority - that gap remains OR4.
