...
| Runtime baseline | Repository and machine use SDK `10.0.302` / runtime `10.0.9`; central ASP.NET pins remain `10.0.9` | Architecture AD-11's required ASP.NET `10.0.10` half of the `10.0.302` / `10.0.10` baseline is not satisfied (SDK half already current). |
...
- AD-11 security baseline: repository and machine are on SDK 10.0.302 /
  runtime 10.0.9 while required ASP.NET remains 10.0.10 (SDK half already
  current). Hard blocker before selecting or publishing the proof runtime.
