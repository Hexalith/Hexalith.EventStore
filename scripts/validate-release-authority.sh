...
role_allowlist="${HEXALITH_RELEASE_OWNER_ALLOWLIST_PATH:-_bmad-output/implementation-artifacts/1-20-github-approval-role-allowlist.json}"
...
[[ "$phase" =~ ^(verify|publish)$ ]] ||
  fail "Authority phase must be verify or publish."
...
[[ "$authority_url" =~ ^https://api\.github\.com/repos/Hexalith/Hexalith\.EventStore/issues/comments/[0-9]+$ ]] ||
  fail "HEXALITH_RELEASE_AUTHORITY_URL must be an EventStore GitHub issue-comment API URL."
...
[ -f "$role_allowlist" ] ||
  fail "The checked-in EventStore release-owner allowlist is unavailable."
