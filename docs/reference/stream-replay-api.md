...
Consumed-event idempotency is keyed by the EventStore `messageId`. The default marker store is in-memory; a completed marker means the message was already handled or terminally skipped, so duplicate deliveries within the process are acknowledged without deserializing payloads or invoking handlers. Services that need durable markers can explicitly register the DAPR marker store when their sidecar has access to the configured state store. DAPR marker keys are scoped by topic, subscription route, and message id. Acquisition and transition reads use strong consistency, and transitions use checked ETag writes with first-write concurrency; a rejected or unconfirmed write fails instead of fabricating success.

The two stores differ in their in-flight concurrency semantics, and opting into the DAPR store trades one guarantee for another rather than strictly strengthening dedup. The in-memory store mutually excludes a concurrently-processing duplicate within the same process (a second delivery is reported as retryable while the first is in flight). The DAPR store takes no pre-handler or in-progress lease; it persists only the post-handler `Dispatched` and terminal `Completed` states. Two concurrent deliveries of the same `messageId` (for example across replicas) can therefore both pass acquisition and dispatch handlers. Registering the DAPR store buys durability across process restarts and replicas, not stronger in-flight exclusion; handlers must be idempotent under at-least-once delivery in either configuration.

No durable in-progress lease is defined. Handler execution remains an at-least-once delivery boundary: if handler execution throws, the in-process marker is released and the exception propagates so DAPR can redeliver the message. After handlers succeed, built-in marker stores first persist `Dispatched` and then persist `Completed`, both independently of the caller cancellation token. A completion failure propagates through the retryable HTTP 500 path. Redelivery of a `Dispatched` marker performs completion only, before envelope decoding, and never invokes handlers again. Third-party stores compiled against the earlier interface use the compatibility default: the dispatch transition calls their existing completion method exactly once and reports that no second completion is needed. Those stores retain their prior durability limitations; a store that needs recoverable completion-only redelivery must override `MarkDispatchedAsync` and durably represent the intermediate state. The default in-memory marker store does represent `Dispatched`, but only in process memory: a restart between dispatch and completion loses the marker, so the next delivery reacquires and redispatches handlers. Only the DAPR marker store makes completion-only recovery survive a restart.

The DAPR state component used for markers must support strong-consistency reads, ETags, and first-write concurrency. Enabling the appended `Dispatched` state requires a coordinated rollout: drain or stop every old consumer before any new consumer can write or encounter that state, then deploy the compatible new consumer cohort. Do not run old and new consumers concurrently during this transition, because old readers do not understand completion-only reacquisition. Before rollback, let the new cohort finish every pending `Dispatched` marker to `Completed`; only then restart old consumers.
