...
#### Closed legacy inventory operation

An upgrade may set `RequireLegacyInventory=true` only with a tenant-scoped, versioned inventory process. Register every supported self-describing aggregate-local source entry under one `InventoryId` and positive `InventoryVersion`, then close the manifest with the exact entry count, the complete scanned digest-key-version set, and the canonical manifest digest. The digest binds tenant/source identity, source schema and evidence, protected aliases, retention tier, consumption/observation/expiry timestamps, exact logical result, execution identities, and inventory/migration identity; mutable migration phase and target checkpoints are deliberately separate. Closure is immutable: an open, missing, mismatched, unknown-version, corrupt, or unscanned-key-version manifest never proves absence, and admission remains `idempotency_unsafe_legacy_state`. A closed manifest may classify a missing protected alias as `NoLegacy` only when that alias's digest-key version is in the closed scan set; an unclosed manifest may not.

`RequireLegacyInventory=false` is the explicit clean-install policy. Use it only when the deployment has never admitted legacy aggregate-local keys. It is not an emergency bypass for incomplete scans, interrupted upgrades, unavailable state, or reconciliation failures.

Migration is crash-resumable and ordered: prepare a non-executable target, verify its hash-bound acknowledgement, persist the payload-free aggregate-source redirect, establish the pinned target as the tenant directory authority, and finally activate the target. Each checkpoint is durable and bound to the tenant, inventory, migration, source evidence, protected aliases, and pinned target. Before the source redirect, an operator may roll back only the exact unactivated prepared target. The source redirect is the irreversible boundary; after it recovery is forward-only, retains the original source evidence, and must finish the already-pinned target even if digest rotation starts meanwhile. A `Migrated` marker is accepted only after the source redirect, target acknowledgement and activation, and canonical directory are independently reproved.

An exact source whose replay interval already elapsed migrates directly to the fence-free expiry tombstone and never becomes replayable or executable. Tenant deletion serializes against migration and the existing 400-day/legal-hold governance retains inventory and alias references until final bounded purge. Do not retire a digest reader while any migration checkpoint or lifecycle reference names it.

Inventory and reconciliation tooling must emit only bounded classifications and non-secret schema, inventory, migration, and phase identifiers. Never include an opaque key, canonical intent, digest-key material, raw aggregate state key, verification/digest values, or protected result payload in logs, traces, metrics, errors, screenshots, or support evidence.
