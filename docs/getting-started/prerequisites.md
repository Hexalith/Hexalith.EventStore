...
Hexalith.EventStore targets .NET 10. You need the .NET 10 SDK (version 10.0.302 or later patch in the same feature band) to build and run the project.
...
Expected output: `10.0.302` or later patch in the same feature band.
...
```bash
$ dotnet --version    # 10.0.302 or later patch in the same feature band
...
If `dotnet --version` shows a version earlier than 10.0.302, download the latest .NET 10 SDK from the [.NET 10 download page](https://dotnet.microsoft.com/en-us/download/dotnet/10.0). Multiple SDK versions can coexist on the same machine.
