...
| Hub broadcast | Broadcast origin, `SignalRProjectionChangedBroadcaster.BroadcastChangedAsync` start and completion timestamps, group, success/fail-open log or trace, EventId, category, elapsed milliseconds, and exception type for fail-open failures. |
| Client receipt | UTC timestamp for client `ProjectionChanged(projectionType, tenantId)` receipt, exact two-field payload observed, EventId, category, connection state, callback count, and trace/span identifiers when present. |
...
| `broadcastStartUtc` | `SignalRProjectionChangedBroadcaster.BroadcastChangedAsync` entered. | Directly emitted by broadcaster EventId `1090` with category `Hexalith.EventStore.SignalRHub.SignalRProjectionChangedBroadcaster`, projection type, tenant id or safe alias, group, UTC timestamp, and trace/span identifiers when present. |
| `broadcastCompletedUtc` | Broadcast call returned or fail-open warning emitted. | Directly emitted by broadcaster EventId `1091` on success and EventId `1092` on fail-open failure with elapsed milliseconds, result classification, and exception type for failures. |
| `clientReceiptUtc` | Client callback received `ProjectionChanged(projectionType, tenantId)`. | Directly emitted before callbacks by `EventStoreSignalRClient` EventId `2090` with category `Hexalith.EventStore.SignalR.EventStoreSignalRClient`, projection type, tenant id or safe alias, group, connection state, callback count, UTC timestamp, and trace/span identifiers when present. |
...
Current repository defaults register tracing sources for the application name, `Hexalith.EventStore`, `Microsoft.AspNetCore.SignalR.Server`, and `Microsoft.AspNetCore.SignalR.Client`. Treat built-in SignalR spans as supporting diagnostics; direct broadcaster/client receipt logs remain the proof that a specific projection-change signal was emitted and observed.
...
| --- | --- | --- |
| Authenticated join and group | `ProjectionChangedHub.JoinGroup()` validates auth/tenant access, rejects colons, tracks group count, and logs join/leave/connect/disconnect at Debug. Tenant authorization denial uses Warning EventId `1084` and validator failure uses Error EventId `1085` from category `Hexalith.EventStore.SignalRHub.ProjectionChangedHub`. When filtering hub events, pair EventId with that category — `1084`/`1085` are hub-only and must not be conflated with future broadcaster (`1090`–`1092`) or client receipt (`2090`) IDs. | Capture Debug logs or add safe test harness metadata for connection/session aliases. |
| Broadcast success/failure | `SignalRProjectionChangedBroadcaster.BroadcastChangedAsync()` sends to `{projectionType}:{tenantId}` and emits direct structured evidence: EventId `1090` broadcast start, EventId `1091` completion, and EventId `1092` fail-open failure. Fields include projection type, tenant id or safe alias, group, UTC timestamp, elapsed milliseconds for result logs, result classification, exception type on failure, trace/span identifiers when present, and category `Hexalith.EventStore.SignalRHub.SignalRProjectionChangedBroadcaster`. The old 1084/1085 broadcaster collision is removed; hub 1084/1085 still exist for tenant authorization/validator events. | Use EventId `1090`/`1091`/`1092` plus category filters for broadcaster evidence. |
...
| Client receipt | `EventStoreSignalRClient` emits EventId `2090` before invoking callbacks for a subscribed group. Fields include projection type, tenant id or safe alias, group, receipt UTC timestamp, connection state, callback count, trace/span identifiers when present, and category `Hexalith.EventStore.SignalR.EventStoreSignalRClient`. Public callback signatures, reconnect/rejoin behavior, and hub payload remain unchanged. | Harnesses still own evidence run ids and stale/mismatched receipt rejection; do not add run ids to the public payload. |
...
| OpenTelemetry | Service defaults register `Hexalith.EventStore`, `Microsoft.AspNetCore.SignalR.Server`, `Microsoft.AspNetCore.SignalR.Client`, ASP.NET Core, HTTP client, runtime metrics, and JSON console logs. | Product-owned broadcaster spans use `Hexalith.EventStore`; built-in SignalR spans remain supporting diagnostics. |
...
| --- | --- | --- | --- | --- |
| Runtime proof harness still owns evidence run id propagation | QA/test tooling | Proof metadata, log scopes, test projection/tenant aliases, or harness-side validation state | Required because the public SignalR payload intentionally remains only `ProjectionChanged(projectionType, tenantId)` and must not carry run ids. | No (unit tests and evidence schema reject stale/mismatched receipts). |
...
| Hub EventIds 1084/1085 remain authorization-specific | EventStore server/observability | `src/Hexalith.EventStore/SignalRHub/ProjectionChangedHub.cs` only if future evidence wants a hub-specific renumbering pass | Broadcaster evidence now uses 1090-1092, so numeric EventId-only broadcaster filters are no longer ambiguous. Hub 1084/1085 are not broadcaster events. | No |
