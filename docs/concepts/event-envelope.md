...
The supported command path treats a persisted envelope as immutable: producers append through the `AggregateActor` and never update an existing event key. That is the required event-sourcing invariant, but the application does not currently supply a storage-level append fence. Story 4.5 live evidence showed that an adversarial direct actor-state transaction can write the same sequence key while an actor turn is in flight and that the actor can silently overwrite that durable write in the observed Dapr `1.18.1` `state.redis` / Redis `6` profile. Direct actor-state writes are unsupported, other providers are not characterized by that run, and provider-portable enforcement remains deferred pending the reviewed fencing design.
...
All supported commands for one aggregate are serialized by the actor's turn-based concurrency model, so two actor-dispatched commands do not interleave their persist operations. The events, metadata, and checkpoint from one turn are committed as one actor-state transaction. This actor serialization does not itself fence a second writer that calls the raw actor-state transaction endpoint; Story 4.5 observed such a writer claim sequence 1 and become durable before the actor silently replaced it at the same key in the tested Dapr/Redis profile.
...
The `sequenceNumber` deserves special attention: it is auto-incremented from `AggregateMetadata.CurrentSequence`, which is a watermark stored alongside the event stream in the DAPR state store. The `AggregateMetadata` record tracks the current sequence watermark, the last modification time, and an optional ETag. The AggregateActor reads the current watermark, increments it for each new event, and persists the updated watermark atomically with the events. This produces gapless, strictly ordered sequence numbers for actor-dispatched commands. Production currently writes the metadata ETag as `null` and supplies no first-write concurrency option; the tested Dapr/Redis profile therefore permitted an unsupported second writer to replace an existing sequence key, while other providers remain uncharacterized.
