...
internal static class RestApiControllerEmitter
{
    ...
    private static void AppendHeader(StringBuilder builder, string controllerNamespace)
    {
        ...
        builder.AppendLine("using Microsoft.Net.Http.Headers;");
        builder.AppendLine();
    ...
    private static void AppendControllerStart(StringBuilder builder, RestApiOptions options, string controllerName, bool hasCommandActions)
    {
        ...
        builder.AppendLine("    private const long MaxRetryAfterSeconds = 86_400;");
        builder.AppendLine("    private const int MaxWarningCodeCount = 16;");
        builder.AppendLine();
    ...
    private static void AppendCommandAction(StringBuilder builder, RestApiOptions options, RestApiMessageDescriptor message)
    {
        ...
        AppendCommandProblemProducesResponseTypes(builder);
        ...
        builder.AppendLine("        {");
        builder.AppendLine("            return MapGatewayException(ex, includeCommandOnlyStatusCodes: true);");
        builder.AppendLine("        }");
        builder.AppendLine("    }");
    }
    ...
    private static void AppendQueryAction(StringBuilder builder, RestApiOptions options, RestApiMessageDescriptor message)
    {
        ...
        AppendQueryProblemProducesResponseTypes(builder);
        builder.Append("    public async Task<IActionResult> ").Append(methodName).AppendLine("(");
        ...
        builder.AppendLine("                .ConfigureAwait(false);");
        builder.AppendLine();
        builder.AppendLine("            QueryResponseProvenance __hexalithProvenance = __hexalithResult.Metadata?.Provenance ?? QueryResponseProvenance.Unknown;");
        builder.AppendLine("            bool __hexalithProjectionBacked = __hexalithProvenance == QueryResponseProvenance.ProjectionBacked;");
        builder.AppendLine("            if (__hexalithResult.IsNotModified && !__hexalithProjectionBacked)");
        builder.AppendLine("            {");
        builder.AppendLine("                return CreateProblem(StatusCodes.Status502BadGateway, \"Bad Gateway\", \"Not-modified query response requires projection-backed provenance.\");");
        builder.AppendLine("            }");
        builder.AppendLine();
        builder.AppendLine("            string? __hexalithETag = __hexalithProjectionBacked");
        builder.AppendLine("                ? __hexalithResult.ETag ?? __hexalithResult.Metadata?.ETag");
        builder.AppendLine("                : null;");
        builder.AppendLine("            bool __hexalithHasStrongETag = false;");
        builder.AppendLine("            if (!string.IsNullOrWhiteSpace(__hexalithETag))");
        builder.AppendLine("            {");
        builder.AppendLine("                if (TryFormatStrongETag(__hexalithETag, out string? __hexalithBoundedETag))");
        builder.AppendLine("                {");
        builder.AppendLine("                    Response.Headers[\"ETag\"] = __hexalithBoundedETag;");
        builder.AppendLine("                    __hexalithHasStrongETag = true;");
        builder.AppendLine("                }");
        builder.AppendLine("            }");
        builder.AppendLine();
        builder.AppendLine("            if (__hexalithResult.IsNotModified && !__hexalithHasStrongETag)");
        builder.AppendLine("            {");
        builder.AppendLine("                return CreateProblem(StatusCodes.Status502BadGateway, \"Bad Gateway\", \"Not-modified query response requires a strong ETag.\");");
        builder.AppendLine("            }");
        builder.AppendLine();
        builder.AppendLine("            AppendQueryMetadataHeaders(__hexalithResult.Metadata);");
        builder.AppendLine();
        ...
        builder.AppendLine("        catch (EventStoreGatewayException ex)");
        builder.AppendLine("        {");
        builder.AppendLine("            return MapGatewayException(ex, includeCommandOnlyStatusCodes: false);");
        builder.AppendLine("        }");
    ...
    private static void AppendCommandProblemProducesResponseTypes(StringBuilder builder)
    {
        builder.AppendLine("    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status401Unauthorized, \"application/problem+json\")]");
        builder.AppendLine("    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status400BadRequest, \"application/problem+json\")]");
        builder.AppendLine("    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status403Forbidden, \"application/problem+json\")]");
        builder.AppendLine("    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status404NotFound, \"application/problem+json\")]");
        builder.AppendLine("    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status409Conflict, \"application/problem+json\")]");
        builder.AppendLine("    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status422UnprocessableEntity, \"application/problem+json\")]");
        builder.AppendLine("    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status429TooManyRequests, \"application/problem+json\")]");
        builder.AppendLine("    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status502BadGateway, \"application/problem+json\")]");
        builder.AppendLine("    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status500InternalServerError, \"application/problem+json\")]");
        builder.AppendLine("    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status503ServiceUnavailable, \"application/problem+json\")]");
    }

    private static void AppendQueryProblemProducesResponseTypes(StringBuilder builder)
    {
        builder.AppendLine("    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status401Unauthorized, \"application/problem+json\")]");
        builder.AppendLine("    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status400BadRequest, \"application/problem+json\")]");
        builder.AppendLine("    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status403Forbidden, \"application/problem+json\")]");
        ...
        builder.AppendLine("    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status502BadGateway, \"application/problem+json\")]");
        builder.AppendLine("    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status500InternalServerError, \"application/problem+json\")]");
        builder.AppendLine("    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status501NotImplemented, \"application/problem+json\")]");
        builder.AppendLine("    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status503ServiceUnavailable, \"application/problem+json\")]");
    }
    ...
    private static void AppendHelpers(StringBuilder builder, RestApiOptions options)
    {
        ...
        builder.AppendLine("    }");
        builder.AppendLine();
        builder.AppendLine("    private void AppendQueryMetadataHeaders(QueryResponseMetadata? metadata)");
        builder.AppendLine("    {");
        builder.AppendLine("        Response.Headers.Remove(ProjectionLifecyclePolicy.HeaderName);");
        builder.AppendLine("        QueryResponseProvenance provenance = metadata?.Provenance ?? QueryResponseProvenance.Unknown;");
        builder.AppendLine("        Response.Headers[\"X-Hexalith-Query-Provenance\"] = GetCanonicalQueryProvenanceName(provenance);");
        builder.AppendLine();
        ...
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        if (provenance == QueryResponseProvenance.ProjectionBacked");
        builder.AppendLine("            && TryGetBoundedHeaderValue(metadata.ProjectionVersion, out string? projectionVersion))");
        builder.AppendLine("        {");
        builder.AppendLine("            Response.Headers[\"X-Hexalith-Projection-Version\"] = projectionVersion;");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        ProjectionLifecycleState lifecycle = ProjectionLifecyclePolicy.Normalize(metadata.Lifecycle, provenance);");
        builder.AppendLine("        bool? isStale = ProjectionLifecyclePolicy.ProjectIsStale(lifecycle, metadata.IsStale);");
        builder.AppendLine("        bool? isDegraded = ProjectionLifecyclePolicy.ProjectIsDegraded(lifecycle, metadata.IsDegraded);");
        builder.AppendLine("        if (lifecycle != ProjectionLifecycleState.Unknown)");
        builder.AppendLine("        {");
        builder.AppendLine("            Response.Headers[ProjectionLifecyclePolicy.HeaderName] = GetCanonicalProjectionLifecycleName(lifecycle);");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        if (metadata.ServedAt is not null)");
        builder.AppendLine("        {");
        builder.AppendLine("            Response.Headers[\"X-Hexalith-Served-At\"] = metadata.ServedAt.Value.ToString(\"O\", CultureInfo.InvariantCulture);");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        if (provenance == QueryResponseProvenance.ProjectionBacked && isStale is not null)");
        builder.AppendLine("        {");
        builder.AppendLine("            Response.Headers[\"X-Hexalith-Is-Stale\"] = isStale.Value ? \"true\" : \"false\";");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        if (isDegraded is not null)");
        builder.AppendLine("        {");
        builder.AppendLine("            Response.Headers[\"X-Hexalith-Is-Degraded\"] = isDegraded.Value ? \"true\" : \"false\";");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        if (TryFormatWarningCodes(metadata.WarningCodes, out string? warningCodes))");
        builder.AppendLine("        {");
        builder.AppendLine("            Response.Headers[\"X-Hexalith-Warning-Codes\"] = warningCodes;");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        QueryPagingMetadata? paging = metadata.Paging;");
        builder.AppendLine("        if (paging is null)");
        builder.AppendLine("        {");
        builder.AppendLine("            return;");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        if (paging.PageSize > 0 && paging.PageSize <= QueryPolicyLimits.MaxPageSize)");
        builder.AppendLine("        {");
        builder.AppendLine("            Response.Headers[\"X-Hexalith-Page-Size\"] = paging.PageSize.ToString(CultureInfo.InvariantCulture);");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        if (paging.Offset.HasValue && paging.Offset.Value >= 0)");
        builder.AppendLine("        {");
        builder.AppendLine("            Response.Headers[\"X-Hexalith-Page-Offset\"] = paging.Offset.Value.ToString(CultureInfo.InvariantCulture);");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        if (paging.TotalCount.HasValue && paging.TotalCount.Value >= 0)");
        builder.AppendLine("        {");
        builder.AppendLine("            Response.Headers[\"X-Hexalith-Page-Total-Count\"] = paging.TotalCount.Value.ToString(CultureInfo.InvariantCulture);");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        if (paging.HasMore is not null)");
        builder.AppendLine("        {");
        builder.AppendLine("            Response.Headers[\"X-Hexalith-Page-Has-More\"] = paging.HasMore.Value ? \"true\" : \"false\";");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        if (TryGetBoundedHeaderValue(paging.NextCursor, out string? nextCursor))");
        builder.AppendLine("        {");
        builder.AppendLine("            Response.Headers[\"X-Hexalith-Next-Cursor\"] = nextCursor;");
        builder.AppendLine("        }");
        builder.AppendLine("    }");
        builder.AppendLine();
        builder.AppendLine("    private static string GetCanonicalQueryProvenanceName(QueryResponseProvenance provenance)");
        builder.AppendLine("        => provenance switch");
        builder.AppendLine("        {");
        builder.AppendLine("            QueryResponseProvenance.ProjectionBacked => nameof(QueryResponseProvenance.ProjectionBacked),");
        builder.AppendLine("            QueryResponseProvenance.HandlerComputed => nameof(QueryResponseProvenance.HandlerComputed),");
        builder.AppendLine("            _ => nameof(QueryResponseProvenance.Unknown),");
        builder.AppendLine("        };");
        builder.AppendLine();
        builder.AppendLine("    private static string GetCanonicalProjectionLifecycleName(ProjectionLifecycleState lifecycle)");
        builder.AppendLine("        => lifecycle switch");
        builder.AppendLine("        {");
        builder.AppendLine("            ProjectionLifecycleState.Current => nameof(ProjectionLifecycleState.Current),");
        builder.AppendLine("            ProjectionLifecycleState.Stale => nameof(ProjectionLifecycleState.Stale),");
        builder.AppendLine("            ProjectionLifecycleState.Rebuilding => nameof(ProjectionLifecycleState.Rebuilding),");
        builder.AppendLine("            ProjectionLifecycleState.Degraded => nameof(ProjectionLifecycleState.Degraded),");
        builder.AppendLine("            ProjectionLifecycleState.Unavailable => nameof(ProjectionLifecycleState.Unavailable),");
        builder.AppendLine("            ProjectionLifecycleState.LocalOnly => nameof(ProjectionLifecycleState.LocalOnly),");
        builder.AppendLine("            _ => nameof(ProjectionLifecycleState.Unknown),");
        builder.AppendLine("        };");
        builder.AppendLine();
        builder.AppendLine("    private static bool TryFormatWarningCodes(IReadOnlyList<string>? warningCodes, out string? headerValue)");
        builder.AppendLine("    {");
        builder.AppendLine("        headerValue = null;");
        builder.AppendLine("        if (warningCodes is null || warningCodes.Count == 0 || warningCodes.Count > MaxWarningCodeCount)");
        builder.AppendLine("        {");
        builder.AppendLine("            return false;");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        var boundedCodes = new List<string>(warningCodes.Count);");
        builder.AppendLine("        foreach (string warningCode in warningCodes)");
        builder.AppendLine("        {");
        builder.AppendLine("            if (!TryGetBoundedHeaderValue(warningCode, out string? boundedCode))");
        builder.AppendLine("            {");
        builder.AppendLine("                return false;");
        builder.AppendLine("            }");
        builder.AppendLine();
        builder.AppendLine("            if (!IsWarningCodeToken(boundedCode))");
        builder.AppendLine("            {");
        builder.AppendLine("                return false;");
        builder.AppendLine("            }");
        builder.AppendLine();
        builder.AppendLine("            boundedCodes.Add(boundedCode);");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        string value = string.Join(\",\", boundedCodes);");
        builder.AppendLine("        if (value.Length > MaxHeaderValueLength)");
        builder.AppendLine("        {");
        builder.AppendLine("            return false;");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        headerValue = value;");
        builder.AppendLine("        return true;");
        builder.AppendLine("    }");
        builder.AppendLine();
        builder.AppendLine("    private static bool TryGetBoundedHeaderValue(string? value, [NotNullWhen(true)] out string? headerValue)");
        builder.AppendLine("    {");
        builder.AppendLine("        headerValue = null;");
        ...
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        if (value.Length == 0 || value.Length > MaxHeaderValueLength)");
        builder.AppendLine("        {");
        builder.AppendLine("            return false;");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        if (ContainsHeaderControlCharacter(value))");
        builder.AppendLine("        {");
        builder.AppendLine("            return false;");
        builder.AppendLine("        }");
        builder.AppendLine();
        ...
        builder.AppendLine("    }");
        builder.AppendLine();
        builder.AppendLine("    private static bool IsWarningCodeToken(string value)");
        builder.AppendLine("    {");
        builder.AppendLine("        foreach (char character in value)");
        builder.AppendLine("        {");
        builder.AppendLine("            bool isAsciiLetter = character is >= 'A' and <= 'Z' or >= 'a' and <= 'z';");
        builder.AppendLine("            bool isAsciiDigit = character is >= '0' and <= '9';");
        builder.AppendLine("            if (!isAsciiLetter && !isAsciiDigit && character != '-' && character != '_' && character != '.')");
        builder.AppendLine("            {");
        builder.AppendLine("                return false;");
        builder.AppendLine("            }");
        builder.AppendLine("        }");
        ...
        builder.AppendLine("    }");
        builder.AppendLine();
        builder.AppendLine("    private static bool ContainsHeaderControlCharacter(string value)");
        builder.AppendLine("    {");
        builder.AppendLine("        foreach (char character in value)");
        builder.AppendLine("        {");
        builder.AppendLine("            if (char.IsControl(character))");
        builder.AppendLine("            {");
        builder.AppendLine("                return true;");
        builder.AppendLine("            }");
        builder.AppendLine("        }");
        ...
        builder.AppendLine("    }");
        builder.AppendLine();
        builder.AppendLine("    private ObjectResult MapGatewayException(EventStoreGatewayException exception, bool includeCommandOnlyStatusCodes)");
        builder.AppendLine("    {");
        builder.AppendLine("        int statusCode = NormalizeGatewayExceptionStatusCode(exception.StatusCode, includeCommandOnlyStatusCodes);");
        builder.AppendLine("        if (IsRetryableGatewayStatusCode(statusCode) && TryGetSafeRetryAfterHeaderValue(exception.RetryAfter, out string? retryAfter))");
        builder.AppendLine("        {");
        builder.AppendLine("            Response.Headers[\"Retry-After\"] = retryAfter;");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        var problem = new ProblemDetails");
        builder.AppendLine("        {");
        builder.AppendLine("            Status = statusCode,");
        builder.AppendLine("            Title = GetSafeProblemDisplayText(exception.Title) ?? \"Gateway Error\",");
        builder.AppendLine("            Type = GetSafeProblemDisplayText(exception.Type) ?? \"about:blank\",");
        builder.AppendLine("            Detail = GetSafeProblemDisplayText(exception.Detail),");
        builder.AppendLine("        };");
        builder.AppendLine();
        builder.AppendLine("        AddSafeProblemTokenExtension(problem, \"correlationId\", exception.CorrelationId);");
        builder.AppendLine("        AddSafeProblemTokenExtension(problem, \"tenantId\", exception.TenantId);");
        builder.AppendLine("        AddSafeProblemTokenExtension(problem, \"reason\", exception.Reason);");
        builder.AppendLine("        AddSafeProblemTokenExtension(problem, \"reasonCode\", exception.ReasonCode);");
            ...
            builder.AppendLine("        {");
        builder.AppendLine("            StatusCode = problem.Status,");
        ...
        builder.AppendLine("    }");
        builder.AppendLine();
        builder.AppendLine("    private static void AddSafeProblemTokenExtension(ProblemDetails problem, string key, string? value)");
        builder.AppendLine("    {");
        builder.AppendLine("        if (TryGetBoundedHeaderValue(value, out string? token) && IsWarningCodeToken(token))");
        builder.AppendLine("        {");
        builder.AppendLine("            problem.Extensions[key] = token;");
        builder.AppendLine("        }");
        builder.AppendLine("    }");
        builder.AppendLine();
        builder.AppendLine("    private static string? GetSafeProblemDisplayText(string? value)");
        builder.AppendLine("    {");
        builder.AppendLine("        if (string.IsNullOrWhiteSpace(value)");
        builder.AppendLine("            || value.Length > MaxHeaderValueLength");
        builder.AppendLine("            || ContainsHeaderControlCharacter(value))");
        builder.AppendLine("        {");
        ...
        builder.AppendLine("        {");
        builder.AppendLine("            if (safeErrors.Count >= MaxProblemErrorCount)");
        builder.AppendLine("            {");
        ...
        builder.AppendLine("    }");
        builder.AppendLine();
        builder.AppendLine("    private static bool TryFormatStrongETag(string etag, [NotNullWhen(true)] out string? headerValue)");
        builder.AppendLine("    {");
        builder.AppendLine("        headerValue = null;");
        builder.AppendLine("        if (!string.Equals(etag, etag.Trim(), StringComparison.Ordinal))");
        builder.AppendLine("        {");
        builder.AppendLine("            return false;");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        string value = etag.Trim();");
        builder.AppendLine("        if (!TryGetBoundedHeaderValue(value, out string? boundedValue))");
        builder.AppendLine("        {");
        builder.AppendLine("            return false;");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        if (ContainsWhitespace(boundedValue))");
        builder.AppendLine("        {");
        builder.AppendLine("            return false;");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        string candidate = boundedValue.Length >= 2 && boundedValue[0] == '\"' && boundedValue[boundedValue.Length - 1] == '\"'");
        builder.AppendLine("            ? boundedValue");
        builder.AppendLine("            : \"\\\"\" + boundedValue + \"\\\"\";");
        builder.AppendLine("        if (!EntityTagHeaderValue.TryParse(candidate, out EntityTagHeaderValue? parsed) || parsed.IsWeak)");
        builder.AppendLine("        {");
        builder.AppendLine("            return false;");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        headerValue = parsed.ToString();");
        builder.AppendLine("        return true;");
        builder.AppendLine("    }");
        builder.AppendLine();
        builder.AppendLine("    private static bool ContainsWhitespace(string value)");
        builder.AppendLine("    {");
        builder.AppendLine("        foreach (char character in value)");
        builder.AppendLine("        {");
        builder.AppendLine("            if (char.IsWhiteSpace(character))");
        builder.AppendLine("            {");
        builder.AppendLine("                return true;");
        builder.AppendLine("            }");
        builder.AppendLine("        }");
        ...
        builder.AppendLine("    }");
        builder.AppendLine();
        builder.AppendLine("    private static bool TryGetSafeRetryAfterHeaderValue(string? value, [NotNullWhen(true)] out string? headerValue)");
        builder.AppendLine("    {");
        builder.AppendLine("        headerValue = null;");
        builder.AppendLine("        if (!TryGetBoundedHeaderValue(value, out string? boundedValue))");
        builder.AppendLine("        {");
        ...
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        if (long.TryParse(boundedValue, NumberStyles.None, CultureInfo.InvariantCulture, out long delaySeconds) && delaySeconds is >= 0 and <= MaxRetryAfterSeconds)");
        builder.AppendLine("        {");
        builder.AppendLine("            headerValue = delaySeconds.ToString(CultureInfo.InvariantCulture);");
        builder.AppendLine("            return true;");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        if (DateTimeOffset.TryParseExact(boundedValue, \"r\", CultureInfo.InvariantCulture, DateTimeStyles.AssumeUniversal, out DateTimeOffset retryDate))");
        builder.AppendLine("        {");
        builder.AppendLine("            headerValue = retryDate.ToUniversalTime().ToString(\"r\", CultureInfo.InvariantCulture);");
        builder.AppendLine("            return true;");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        return false;");
        builder.AppendLine("    }");
        builder.AppendLine();
        builder.AppendLine("    private static bool IsRetryableGatewayStatusCode(int statusCode)");
        builder.AppendLine("        => statusCode is StatusCodes.Status409Conflict");
        builder.AppendLine("            or StatusCodes.Status429TooManyRequests");
        builder.AppendLine("            or StatusCodes.Status500InternalServerError");
        builder.AppendLine("            or StatusCodes.Status502BadGateway");
        builder.AppendLine("            or StatusCodes.Status503ServiceUnavailable;");
        builder.AppendLine();
        builder.AppendLine("    private static int NormalizeGatewayExceptionStatusCode(int statusCode, bool includeCommandOnlyStatusCodes)");
        builder.AppendLine("        => statusCode is StatusCodes.Status400BadRequest");
        builder.AppendLine("            or StatusCodes.Status401Unauthorized");
        builder.AppendLine("            or StatusCodes.Status403Forbidden");
        builder.AppendLine("            or StatusCodes.Status404NotFound");
        builder.AppendLine("            or StatusCodes.Status429TooManyRequests");
        builder.AppendLine("            or StatusCodes.Status500InternalServerError");
        builder.AppendLine("            or StatusCodes.Status501NotImplemented");
        builder.AppendLine("            or StatusCodes.Status502BadGateway");
        builder.AppendLine("            or StatusCodes.Status503ServiceUnavailable");
        builder.AppendLine("            || (includeCommandOnlyStatusCodes && statusCode is StatusCodes.Status409Conflict or StatusCodes.Status422UnprocessableEntity)");
        builder.AppendLine("                ? statusCode");
        builder.AppendLine("                : StatusCodes.Status502BadGateway;");
    }
