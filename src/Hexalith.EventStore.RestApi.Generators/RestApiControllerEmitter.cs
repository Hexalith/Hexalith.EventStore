...
internal static class RestApiControllerEmitter
{
    ...
    private static void AppendHelpers(StringBuilder builder, RestApiOptions options)
    {
        ...
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        if (provenance == QueryResponseProvenance.ProjectionBacked && metadata.IsStale is not null)");
        builder.AppendLine("        {");
        builder.AppendLine("            Response.Headers[\"X-Hexalith-Is-Stale\"] = metadata.IsStale.Value ? \"true\" : \"false\";");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        if (metadata.IsDegraded is not null)");
        builder.AppendLine("        {");
        builder.AppendLine("            Response.Headers[\"X-Hexalith-Is-Degraded\"] = metadata.IsDegraded.Value ? \"true\" : \"false\";");
        builder.AppendLine("        }");
        builder.AppendLine();
