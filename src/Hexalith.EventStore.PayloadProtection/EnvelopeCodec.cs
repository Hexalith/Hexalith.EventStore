...
internal static class EnvelopeCodec {
    ...
    internal static PayloadProtectionEnvelope Read(ReadOnlySpan<byte> value) {
        ...
        return new PayloadProtectionEnvelope(
            ...
            dekVersion,
            ordinal,
            value.Slice(54, PayloadProtectionLimits.NonceBytes).ToArray(),
            value.Slice(66, ciphertextBytes).ToArray(),
            value.Slice(66 + ciphertextBytes, PayloadProtectionLimits.TagBytes).ToArray());
    }
