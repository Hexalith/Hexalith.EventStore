...
public partial class AggregateActor(
    ...
    internal const string EventBatchCommitWitnessKey = "event_batch_commit_witness";
    ...
    private JsonSerializerOptions ActorStateJsonSerializerOptions
        => (serviceProvider?.GetService(typeof(IOptions<ActorRuntimeOptions>))
                as IOptions<ActorRuntimeOptions>)?.Value.JsonSerializerOptions
        ...
        IdempotencyLegacySourceRequest request)
    {
        ...
        try
        {
            IdempotencyLegacySourceInspection inspection = await checker
                .InspectLegacySourceAsync(request)
                .ConfigureAwait(false);
            if (inspection.Decision == IdempotencyLegacySourceDecision.Unavailable)
            {
        ...
        IdempotencyLegacySourceRedirectRequest request)
    {
        ...
        bool redirectSaveAttempted = false;
        try
        {
            expectedRedirectDigest = IdempotencyChecker.ComputeLegacyRedirectEvidence(request);
            (IdempotencyLegacySourceInspection staged, bool mutationStaged) = await checker
                .SetLegacySourceRedirectAsync(request)
                .ConfigureAwait(false);
            if (!mutationStaged)
            {
                if (staged.Decision == IdempotencyLegacySourceDecision.Unavailable)
                {
                    _ = await TryDiscardFailedBatchAsync().ConfigureAwait(false);
                }
        ...
        catch (Exception exception)
        {
            ...
            if (!redirectSaveAttempted)
            {
                return new IdempotencyLegacySourceInspection(IdempotencyLegacySourceDecision.Unavailable);
            }

            IdempotencyLegacySourceInspection observed;
            try
            {
                observed = await checker
    ...
    private async Task<CommandProcessingResult> ProcessCommandCoreAsync(
        ...
        using (processActivity) {
            ...
            using (Activity? activity = EventStoreActivitySource.Instance.StartActivity(
                ...
                IdempotencyRecord? expectedMigratedRecord = null;
                try
                {
                    ...
                    {
                        ConditionalValue<IdempotencyRecord> stagedMigration = await StateManager
                            .TryGetStateAsync<IdempotencyRecord>(
                                IdempotencyChecker.GetRecordKey(command.MessageId))
                            .ConfigureAwait(false);
                        expectedMigratedRecord = stagedMigration.HasValue
                            ? stagedMigration.Value
                            : throw new InvalidOperationException(
                                "Legacy migration did not stage the message-keyed record.");
                ...
                catch (Exception migrationException)
                {
                    ...
                    bool leftoverCausationPlusMiss = false;
                    try
                    {
                        ConditionalValue<IdempotencyRecord> observed = await StateManager
                            .TryGetStateAsync<IdempotencyRecord>(
                                IdempotencyChecker.GetRecordKey(command.MessageId))
                            .ConfigureAwait(false);
                        bool legacyKeyRemoved = true;
                        if (!string.IsNullOrWhiteSpace(command.CausationId)
                            && !string.Equals(command.MessageId, command.CausationId, StringComparison.Ordinal))
                        {
                            ConditionalValue<IdempotencyRecord> leftoverLegacy = await StateManager
                                .TryGetStateAsync<IdempotencyRecord>(
                                    IdempotencyChecker.GetRecordKey(command.CausationId))
                                .ConfigureAwait(false);
                            legacyKeyRemoved = !leftoverLegacy.HasValue;
                        }

                        if (observed.HasValue
                            && observed.Value == expectedMigratedRecord
                            && legacyKeyRemoved)
                        {
                            leftoverCausationPlusMiss = false;
                        }
                        else if (!observed.HasValue && !legacyKeyRemoved)
                        {
                            leftoverCausationPlusMiss = true;
                        }
                        else
                        {
                            _stateCacheUnsafe = true;
                            throw CreateStateRemediationException(
                                ...
                                new InvalidOperationException("Legacy migration durable state is ambiguous."),
                                discardExceptionType,
                                failedBatchDiscarded: true,
                                durableStateObservation: "AmbiguousLegacyMigration");
                        }
                    }
                    ...
                    if (leftoverCausationPlusMiss)
                    {
                        throw;
                    }

                    idempotencyCheck = new IdempotencyCheckResult(
                        IdempotencyCheckOutcome.LegacyMigration,
                        expectedMigratedRecord?.ToResult()
                            ?? throw new InvalidOperationException(
                                "Legacy migration expected record was not captured."));
                }
                ...
                if (existingPipeline is not null) {
                    ...
                    else
                    {
                        ...
                        catch (Exception cleanupSaveException)
                        {
                            ...
                            if (cleanupSaveException is OperationCanceledException)
                            {
                                if (cleanupCommitted)
                                {
                                    // Cleanup committed before cancellation, but ownership was not
                                    // observed yet. Force the next turn to normalize the pending
                                    // projection from the durable publication index.
                                    _pendingCountReconciliationRequired = true;
                                }
            ...
            try {
                ...
                using (Activity? activity = EventStoreActivitySource.Instance.StartActivity(
                    ...
                    try {
                        ...
                        // One actor-scoped opaque token proves which complete atomic event batch
                        // committed without rereading every event after an ambiguous save.
                        string eventBatchAttemptToken = Guid.NewGuid().ToString("N");
                ...
                if (publishResult.Success) {
                    ...
                    UnpublishedPublicationIndex originalPublicationIndex = await ReadPublicationIndexAsync()
                        .ConfigureAwait(false);
                    int originalPendingCount = await ReadPendingCommandCountAsync().ConfigureAwait(false);
                    bool recoveryEntryTracked = await StoreDrainRecordAndRegisterReminderAsync(
                        command.MessageId,
                        unpublishedRecord).ConfigureAwait(false);
                    UnpublishedPublicationIndex expectedPublicationIndex = await ReadPublicationIndexAsync()
                        .ConfigureAwait(false);
                    ...
                    try {
                        ...
                        (bool recoveryBatchCommitted, bool recoveryOwnerCommitted) =
                            await InspectPublicationRecoverySaveFailureAsync(
                                ...
                                pipelineKeyPrefix,
                                ex,
                                originalRecoveryPipeline,
                                originalPublicationIndex,
                                originalPendingCount,
                                expectedPublicationIndex).ConfigureAwait(false);
    ...
    private bool SnapshotRecordsMatch(SnapshotRecord observed, SnapshotRecord expected)
        => observed.SequenceNumber == expected.SequenceNumber
            ...
            && JsonSerializer.SerializeToUtf8Bytes(observed.State, ActorStateJsonSerializerOptions)
                .AsSpan()
                .SequenceEqual(JsonSerializer.SerializeToUtf8Bytes(
                    expected.State,
                    ActorStateJsonSerializerOptions));
    ...
    private async Task DrainUnpublishedEventsAsync(string trackingId) {
        ...
        try {
            ...
            ProtectedDataDiagnosticRedactor.RecordActivityException(activity, exception, "drain");
            _ = (activity?.SetTag("eventstore.failure_reason", "actor_state_remediation_failed"));
            _ = (activity?.SetStatus(ActivityStatusCode.Error, "ActorStateRemediationFailed"));
            throw;
        }
        catch (Exception ex) {
        ...
        if (!record.DeadLettered) {
            ...
            catch (Exception markerSaveException)
            {
                ...
                bool markerCommitted = await InspectDrainMarkerSaveFailureAsync(
                    ...
                    markedRecord,
                    markerSaveException).ConfigureAwait(false);
                if (markerSaveException is OperationCanceledException)
                {
                    throw;
                }
        ...
        UnpublishedEventsRecord committedRecord)
    {
        ...
        catch (Exception saveException)
        {
            ...
            catch (OperationCanceledException) when (saveException is OperationCanceledException)
            {
                throw;
            }
            catch (Exception inspectionException)
    ...
    private async Task<CommandProcessingResult> CompletePublishFailedAsync(
        ...
        UnpublishedPublicationIndex? originalPublicationIndex = null;
        int originalPendingCount = 0;
        bool recoverySaveAttempted = false;

        try
        {
        originalPublicationIndex = await ReadPublicationIndexAsync().ConfigureAwait(false);
        originalPendingCount = await ReadPendingCommandCountAsync().ConfigureAwait(false);
        var publishFailedState = new PipelineState(
            command.CorrelationId,
            CommandStatus.PublishFailed,
            command.CommandType,
        ...
        if (eventCount > 0) {
            ...
            if (hasRange
                ...
                shouldRegisterReminder = priorDrain?.ReminderArmedAt is null;
            }
            else {
                throw new InvalidOperationException(
        ...
        catch (Exception ex)
        {
            ...
            if (committedDrainRecord is not null)
            {
                (committed, _) = await InspectPublicationRecoverySaveFailureAsync(
                    ...
                    existingPipeline,
                    originalPublicationIndex
                        ?? throw new InvalidOperationException("Publication recovery original index was not captured."),
                    originalPendingCount,
            ...
            catch (Exception recoveryException)
            {
                ...
                if (recoveryDiscarded)
                {
                    try
                    {
                        ...
                        {
                            if (recoveryException is OperationCanceledException)
                            {
                                throw;
                            }
                    ...
                    catch (OperationCanceledException) when (recoveryException is OperationCanceledException)
                    {
                        throw;
                    }
    ...
    private async Task<(bool BatchCommitted, bool RecoveryOwnerCommitted)>
        InspectPublicationRecoverySaveFailureAsync(
            ...
            string pipelineKeyPrefix,
            Exception saveException,
            PipelineState originalPipeline,
            UnpublishedPublicationIndex originalPublicationIndex,
            int originalPendingCount,
            UnpublishedPublicationIndex expectedPublicationIndex)
    {
        (bool discarded, string discardExceptionType) = await TryDiscardFailedBatchAsync()
            .ConfigureAwait(false);
        if (!discarded)
        ...
        try
        {
            ...
            bool exactExpectedIndex = PublicationIndexesMatch(index, expectedPublicationIndex)
            ...
            // A still-present exact original checkpoint is the pre-commit witness, including a
            // timestamp-refreshed index that has not yet committed. An already committed recovery
            // owner may pre-date this resume attempt and must not be finalized as though this turn
            // had acquired a new pending slot.
            if (pipeline.HasValue
                && pipeline.Value == originalPipeline
                && PublicationIndexesMatch(index, originalPublicationIndex)
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            ConditionalValue<PipelineState> pipeline = await StateManager
                ...
                .ConfigureAwait(false);
            ConditionalValue<string> attemptToken = await StateManager
                .TryGetStateAsync<string>(EventBatchCommitWitnessKey)
                .ConfigureAwait(false);
            UnpublishedPublicationIndex index = await ReadPublicationIndexAsync().ConfigureAwait(false);
            ...
            if (pipeline.HasValue
                ...
                && exactRecoveryOwner
                && attemptToken.HasValue
                && string.Equals(attemptToken.Value, expectedAttemptToken, StringComparison.Ordinal))
            {
                return true;
            }
    ...
    private async Task ReconcilePendingCommandCountAsync()
    {
        ...
        catch (Exception saveException)
        {
            ...
            try
            {
                ...
                durableReconciled = durableCount == durableIndex.OwnerCount;
            }
            catch
            {
                _stateCacheUnsafe = true;
