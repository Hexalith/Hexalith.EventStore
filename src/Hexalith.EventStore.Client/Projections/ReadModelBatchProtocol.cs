...
internal sealed class ReadModelBatchProtocol {
    ...
    private async Task<ReadModelBatchResult> RunResumableAsync(CancellationToken cancellationToken) {
        ...
        if (marker is not null) {
            ...
            switch (marker.Status) {
                ...
                case ReadModelBatchMarkerStatus.Committed:
                    await CompactAndCompleteAsync(markerEtag, cancellationToken).ConfigureAwait(false);
                    return ReadModelBatchResult.Completed(_fingerprint);
                case ReadModelBatchMarkerStatus.Aborting:
                    await CompensateAsync(_batch.Operations.Count, markerEtag, cancellationToken).ConfigureAwait(false);
        ...
        (_, markerEtag, _) = await ReadMarkerAsync(cancellationToken).ConfigureAwait(false);
        await CompactAndCompleteAsync(markerEtag, cancellationToken).ConfigureAwait(false);
    ...
    private async Task<ReadModelBatchResult?> InstallAsync(CancellationToken cancellationToken) {
        for (int ordinal = 0; ordinal < _batch.Operations.Count; ordinal++) {
            ...
            if (foreignEnvelope || !SatisfiesConcurrency(operation, current)) {
                await CompensateAsync(ordinal, await CurrentMarkerEtagAsync(cancellationToken).ConfigureAwait(false), cancellationToken).ConfigureAwait(false);
                ReadModelBatchLog.Compensated(_logger, _scopeHash, _profileName);
                return ReadModelBatchResult.OptimisticConflict(_fingerprint, "pre-commit-conflict");
            }
            ...
            if (!installed) {
                await CompensateAsync(ordinal, await CurrentMarkerEtagAsync(cancellationToken).ConfigureAwait(false), cancellationToken).ConfigureAwait(false);
                ReadModelBatchLog.Compensated(_logger, _scopeHash, _profileName);
                return ReadModelBatchResult.OptimisticConflict(_fingerprint, "install-race");
            }
    ...
    private async Task CompactAndCompleteAsync(string markerEtag, CancellationToken cancellationToken) {
        for (int ordinal = 0; ordinal < _batch.Operations.Count; ordinal++) {
            if (_injector is not null) {
            ...
            if (TryGetOwnEnvelope(current, ordinal, out ReadModelBatchEnvelope? envelope)) {
                if (envelope!.IsDelete) {
                    await _accessor.DeleteAsync(operation.Key, cancellationToken).ConfigureAwait(false);
                }
                else {
                    await _accessor.WriteAsync(operation.Key, envelope.CandidateBytes(), cancellationToken).ConfigureAwait(false);
                }
            }
        ...
        // The terminal receipt is written last and is idempotent; a fresh etag guard avoids clobbering a
        // concurrent transition, falling back to an unconditional terminal write.
        bool ok = await _accessor
            .TryWriteAsync(_markerKey, SerializeMarker(receipt), markerEtag, cancellationToken)
            .ConfigureAwait(false);
        if (!ok) {
            await _accessor.WriteAsync(_markerKey, SerializeMarker(receipt), cancellationToken).ConfigureAwait(false);
        }
    }

    private async Task CompensateAsync(int installedCount, string markerEtag, CancellationToken cancellationToken) {
        ...
        if (marker is not null && marker.Status == ReadModelBatchMarkerStatus.Prepared) {
            marker.Status = ReadModelBatchMarkerStatus.Aborting;
            _ = await _accessor.TryWriteAsync(_markerKey, SerializeMarker(marker), currentEtag, cancellationToken).ConfigureAwait(false);
        }
        ...
        for (int ordinal = installedCount - 1; ordinal >= 0; ordinal--) {
            ...
            if (envelope!.PreviousBase64 is null) {
                ...
                await _accessor.WriteAsync(operation.Key, envelope.PreviousBytes(), cancellationToken).ConfigureAwait(false);
            }
        }

        (ReadModelBatchMarker? aborting, string abortingEtag, _) = await ReadMarkerAsync(cancellationToken).ConfigureAwait(false);
        if (aborting is not null && aborting.Status is ReadModelBatchMarkerStatus.Aborting or ReadModelBatchMarkerStatus.Prepared) {
            aborting.Status = ReadModelBatchMarkerStatus.Aborted;
            _ = await _accessor.TryWriteAsync(_markerKey, SerializeMarker(aborting), abortingEtag, cancellationToken).ConfigureAwait(false);
        }
    }
        ...
        try {
            ...
            switch (marker.Status) {
                case ReadModelBatchMarkerStatus.Completed:
                    return ReadModelBatchResult.Completed(_fingerprint);
                case ReadModelBatchMarkerStatus.Committed:
                    await CompactAndCompleteAsync(markerEtag, token).ConfigureAwait(false);
                    return ReadModelBatchResult.Completed(_fingerprint);
                case ReadModelBatchMarkerStatus.Aborting:
                    await CompensateAsync(_batch.Operations.Count, markerEtag, token).ConfigureAwait(false);
    ...
    private async Task<string> CurrentMarkerEtagAsync(CancellationToken cancellationToken) {
        (_, string etag, _) = await ReadMarkerAsync(cancellationToken).ConfigureAwait(false);
        return etag;
    }
