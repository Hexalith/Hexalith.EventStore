...
/// <summary>
/// Identifies the stream read event metadata id being generated by a deterministic id factory.
/// </summary>
public enum StreamReadEventIdKind {
    /// <summary>
    /// Message identifier.
    /// </summary>
    Message,
...
public sealed class StreamReadPageBuilder {
    ...
    public StreamReadPageBuilder WithDeterministicIds(Func<long, string> idFactory) {
        ...
        _idFactory = (sequence, kind) => string.Concat(idFactory(sequence), "-", kind.ToString().ToLowerInvariant());
        return this;
    }
    ...
    /// Configures a deterministic id factory for MessageId/CorrelationId/CausationId.
    /// </summary>
    public StreamReadPageBuilder WithDeterministicIds(Func<long, StreamReadEventIdKind, string> idFactory) {
        ArgumentNullException.ThrowIfNull(idFactory);
    ...
    public StreamReadPageBuilder AddEvent(long sequenceNumber, string eventTypeName = "TestEvent", byte[]? payload = null) {
        ...
        string messageId = _idFactory?.Invoke(sequenceNumber, StreamReadEventIdKind.Message) ?? UniqueIdHelper.GenerateSortableUniqueStringId();
        string correlationId = _idFactory?.Invoke(sequenceNumber, StreamReadEventIdKind.Correlation) ?? UniqueIdHelper.GenerateSortableUniqueStringId();
        string causationId = _idFactory?.Invoke(sequenceNumber, StreamReadEventIdKind.Causation) ?? UniqueIdHelper.GenerateSortableUniqueStringId();
