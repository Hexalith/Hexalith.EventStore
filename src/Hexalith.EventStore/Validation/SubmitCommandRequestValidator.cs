...
public partial class SubmitCommandRequestValidator : AbstractValidator<SubmitCommandRequest> {
    ...
    private const int MaxIdempotencyKeyBytes = 4_096;
    ...
    public SubmitCommandRequestValidator() {
        ...
        _ = RuleFor(x => x.IdempotencyKey)
            .Must(key => key is null
                || (!string.IsNullOrWhiteSpace(key)
                    && Encoding.UTF8.GetByteCount(key) <= MaxIdempotencyKeyBytes))
            .WithMessage($"IdempotencyKey must be non-blank and no larger than {MaxIdempotencyKeyBytes} UTF-8 bytes when supplied.");
    }
