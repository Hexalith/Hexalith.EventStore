...
public static partial class ProfileManager
{
    ...
    {
        "version", "activeProfile", "profiles", "url", "token", "format",
    };
    ...
    public static ProfileStore Load(string? profilePath = null)
    {
        ...
        if (!File.Exists(path))
        {
            return new ProfileStore(ProfileStore.CurrentVersion, null, new Dictionary<string, ConnectionProfile>());
        }
        ...
        try
        {
            ...
            return store with { Profiles = store.Profiles ?? new Dictionary<string, ConnectionProfile>() };
        }
        catch (JsonException)
        {
